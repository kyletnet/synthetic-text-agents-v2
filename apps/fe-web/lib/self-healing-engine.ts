...
import { autoDetectionEngine, DetectionResult, SystemHealth } from './auto-detection-engine';
...
export interface HealingAction {
  id: string;
  type: 'api_key_rotation' | 'process_cleanup' | 'mock_recovery' | 'preventive_maintenance' | 'system_restart';
...
export class SelfHealingEngine {
  ...
  constructor() {
    // Feature Flag 체크
    this.autoHealingEnabled = process.env.FEATURE_AUTO_HEALING_ENABLED !== 'false';
    ...
    if (this.autoHealingEnabled) {
      ...
      console.log('🔄 [SelfHealing] Self-Healing Engine initialized (AUTO ENABLED)');
    } else {
      console.log('⏸️ [SelfHealing] Self-Healing Engine initialized (AUTO DISABLED)');
    }
  }
  ...
  async performAutomaticHealing(): Promise<HealingResult[]> {
    // Dormant Mode 체크 (최우선)
    if (this.dormantMode) {
      console.log('🛌 [SelfHealing] System in dormant mode - healing suspended');
      console.log(`   Reason: ${this.dormantMode.reason}`);
      console.log(`   Triggered at: ${this.dormantMode.timestamp.toLocaleString()}`);
    ...
    try {
      return await withCircuitBreaker('self-healing-main', async () => {
        ...
        permanentOpenConditions: ['no active api keys', 'api key', 'unauthorized']
      });
    } catch (error) {
      ...
      console.log(`🛡️ [SelfHealing] Circuit breaker blocked healing: ${errorMessage}`);
  ...
  private async performAutomaticHealingInternal(): Promise<HealingResult[]> {
    ...
    if (healingActions.length === 0) {
      console.log('🔄 [SelfHealing] No healing actions required - system healthy');
      return [];
    }

    console.log(`🔄 [SelfHealing] Found ${healingActions.length} healing actions to perform`);
    ...
    for (const action of healingActions) {
      if (this.activeHealings.has(action.id)) {
        console.log(`🔄 [SelfHealing] Skipping ${action.type} - already in progress`);
        continue;
      }
      ...
      if (result.success && result.followUpRequired) {
        console.log(`🔄 [SelfHealing] Executing follow-up actions for ${action.type}`);
    ...
    console.log(`🔄 [SelfHealing] Healing cycle completed: ${successCount}/${results.length} successful`);

    // 성공 시 연속 실패 카운터 리셋
    if (successCount > 0) {
      this.consecutiveFailures = 0;
      this.stats.consecutiveFailures = 0;
      console.log('✅ [SelfHealing] Consecutive failures reset due to successful healing');
    } else {
      // 모든 healing action이 실패한 경우
      this.consecutiveFailures++;
      this.stats.consecutiveFailures = this.consecutiveFailures;
      console.warn(`⚠️ [SelfHealing] All healing actions failed (consecutive failures: ${this.consecutiveFailures}/${this.maxConsecutiveFailures})`);
      ...
      if (this.consecutiveFailures >= this.maxConsecutiveFailures) {
        ...
        console.error('🛌 [SelfHealing] Entered dormant mode - manual intervention required');
      }
    }
  ...
  private identifyHealingActions(systemHealth: SystemHealth): HealingAction[] {
    ...
    const criticalIssues = systemHealth.detections.filter(d => d.severity === 'emergency' || d.severity === 'critical');
    ...
    for (const issue of criticalIssues) {
      switch (issue.category) {
        ...
        case 'Port & Process Health':
          if (issue.message.includes('conflict') || issue.message.includes('multiple')) {
    ...
    // 우선순위별로 정렬
    return actions.sort((a, b) => this.getActionPriority(b) - this.getActionPriority(a));
  }
  ...
  private async executeHealingAction(action: HealingAction): Promise<HealingResult> {
    ...
    console.log(`🔄 [SelfHealing] Executing healing action: ${action.type} - ${action.description}`);
    ...
    try {
      ...
      switch (action.type) {
        case 'api_key_rotation':
          ({ success, details, followUpRequired, followUpActions } = await this.performAPIKeyRotation());
          break;
        ...
        case 'mock_recovery':
          ({ success, details, followUpRequired } = await this.performMockRecovery());
          break;
        ...
        case 'system_restart':
          ({ success, details, followUpRequired } = await this.performSystemRestart());
          break;
      ...
      if (success) {
        ...
        console.log(`✅ [SelfHealing] Healing action succeeded: ${action.type} (${duration}ms)`);
      } else {
        this.stats.failedHealings++;
        console.warn(`❌ [SelfHealing] Healing action failed: ${action.type} (${duration}ms)`);
      }
      ...
      console.error(`❌ [SelfHealing] Healing action error: ${action.type}`, error);
   ...
   * 🔑 API Key 로테이션 실행
   */
  private async performAPIKeyRotation(): Promise<{ success: boolean; details: any; followUpRequired: boolean; followUpActions: HealingAction[] }> {
    try {
      // 현재 키 상태 확인
      ...
      if (stats.activeKeys > 0) {
        ...
        const errorMessage = 'No API keys found in environment - requires manual configuration';
        console.error(`🛌 [SelfHealing] Unrecoverable issue detected: ${errorMessage}`);
   ...
   * 🧹 프로세스 정리 실행
   */
  private async performProcessCleanup(): Promise<{ success: boolean; details: any }> {
    try {
   ...
   */
  private async performMockRecovery(): Promise<{ success: boolean; details: any; followUpRequired: boolean }> {
    try {
      // 1. API 키 상태 재확인
      ...
      if (!keyRotationResult.success) {
        return {
          ...
          details: {
            ...
            reason: 'Cannot recover from mock contamination without valid API keys',
      ...
      return {
        ...
        details: {
          ...
          result: 'Mock contamination resolved - system ready for real LLM calls'
        },
   ...
   * 🔧 예방적 유지보수 실행
   */
  private async performPreventiveMaintenance(): Promise<{ success: boolean; details: any }> {
    try {
   ...
   * 🔄 시스템 재시작 실행
   */
  private async performSystemRestart(): Promise<{ success: boolean; details: any; followUpRequired: boolean }> {
    try {
      ...
      // 시스템 상태가 개선되었는지 확인
      const isHealthy = integrityReport.status === 'healthy' || integrityReport.status === 'degraded';
  ...
  private createAPIKeyRotationAction(severity: 'emergency' | 'critical' = 'critical'): HealingAction {
    return {
  ...
  private createMockRecoveryAction(): HealingAction {
    return {
      ...
      type: 'mock_recovery',
      severity: 'critical',
      description: 'Recover from mock contamination and restore real LLM execution',
      automated: true,
      estimatedDuration: 10000,
      requirements: ['Valid API keys', 'System restart capability'],
      risksAndSideEffects: ['System restart required', 'Active requests may be interrupted']
    };
  }
  ...
  private createProcessCleanupAction(): HealingAction {
    return {
      ...
      severity: 'medium',
      description: 'Clean up conflicting processes and restore clean environment',
      automated: true,
  ...
  private createSystemRestartAction(): HealingAction {
    return {
      ...
      risksAndSideEffects: ['All active sessions will be interrupted', 'Brief service downtime']
    };
  }
   ...
   * 🔮 예방적 액션 식별
   */
  private identifyPreventiveActions(systemHealth: SystemHealth): HealingAction[] {
    const actions: HealingAction[] = [];

    // 경고 수준 이슈들에 대한 예방적 조치
    const warnings = systemHealth.detections.filter(d => d.severity === 'warning');
  ...
  private startAutomaticHealing(): void {
    ...
    autoDetectionEngine.onAlert(async (result: DetectionResult) => {
      if (result.severity === 'emergency' || result.severity === 'critical') {
        console.log(`🚨 [SelfHealing] Critical alert detected - scheduling healing with backoff`);

        // Exponential backoff 적용
        const backoffDelay = this.calculateBackoffDelay();
        this.stats.backoffDelay = backoffDelay;

        console.log(`🔄 [SelfHealing] Scheduled healing in ${backoffDelay}ms (consecutive failures: ${this.consecutiveFailures})`);
    ...
    const preventiveHealingEnabled = process.env.FEATURE_PREVENTIVE_HEALING_ENABLED !== 'false';
    ...
    if (preventiveHealingEnabled) {
      backgroundTaskManager.registerInterval(
        ...
        async () => {
          ...
          if (systemHealth.overall === 'degraded') {
            console.log('🔄 [SelfHealing] System degraded - performing preventive healing');
            await this.performAutomaticHealing();
          }
        },
        600000, // 10 minutes
      ...
      console.log('🔄 [SelfHealing] Preventive healing scheduled (10min interval)');
    }
  ...
  private calculateBackoffDelay(): number {
    ...
    const exponentialDelay = this.baseBackoffDelay * Math.pow(2, this.consecutiveFailures);
  ...
  private enterDormantMode(reason: string, triggeredBy: string): void {
    ...
    const canceledCount = backgroundTaskManager.cancelTasksByPattern('healing-alert-*');
    ...
    console.error(`   Timestamp: ${this.dormantMode.timestamp.toLocaleString()}`);
  ...
  private updateStats(actionType: string, duration: number): void {
    ...
    this.stats.healingsByType[actionType] = (this.stats.healingsByType[actionType] || 0) + 1;
   ...
   * 수동 치유 실행
   */
  async manualHeal(actionType?: HealingAction['type']): Promise<HealingResult[]> {
    if (actionType) {
      console.log(`🔄 [SelfHealing] Manual healing requested for: ${actionType}`);
      // 특정 타입의 액션만 실행
      const systemHealth = await autoDetectionEngine.performFullHealthCheck();
      const actions = this.identifyHealingActions(systemHealth).filter(a => a.type === actionType);
