...
export class ProcessOverloadProtection {
  ...
  constructor() {
    console.log('🛡️ [ProcessProtection] Process Overload Protection initialized');
  }
  ...
  async detectDevProcesses(): Promise<ProcessInfo[]> {
    try {
      const { stdout } = await execAsync(`ps aux | grep -E "(next dev|npm.*dev|node.*dev)" | grep -v grep`);
  ...
  async collectProcessStats(): Promise<ProcessStats> {
    ...
    const highCpuProcesses = devProcesses.filter(p => parseFloat(p.cpu) > this.maxCpuThreshold);
    const highMemoryProcesses = devProcesses.filter(p => parseFloat(p.memory) > this.maxMemoryThreshold);
   ...
   * 🚨 과부하 상태 감지
   */
  async detectOverload(): Promise<{ isOverloaded: boolean; reason: string; severity: 'low' | 'medium' | 'high' | 'critical' }> {
   ...
   * 🧹 자동 프로세스 정리
   */
  async performAutomaticCleanup(): Promise<{ success: boolean; cleaned: number; details: string[] }> {
    const cleanupDetails: string[] = [];
    let cleanedCount = 0;

    try {
      console.log('🧹 [ProcessProtection] Starting automatic process cleanup...');

      // Circuit Breaker 적용으로 정리 작업 보호
      const result = await circuitBreakerRegistry.get('process-cleanup', {
        failureThreshold: 2,
        timeoutWindow: 30000,
        ...
        if (devProcesses.length > this.maxDevProcesses) {
          ...
          for (const process of processesToKill) {
            try {
              ...
              cleanupDetails.push(`Terminated process ${process.pid}: ${process.command}`);
              cleanedCount++;
              ...
              try {
                ...
                cleanupDetails.push(`Force killed stubborn process ${process.pid}`);
              } catch {
                // 프로세스가 정상적으로 종료됨
              }

            } catch (error) {
              cleanupDetails.push(`Failed to kill process ${process.pid}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
          }
        }
        ...
        // 2. 좀비 프로세스 정리
        try {
          await execAsync(`ps aux | awk '$8 ~ /^Z/ { print $2 }' | xargs -r kill -KILL`);
      ...
      console.log(`🧹 [ProcessProtection] Cleanup completed: ${result.cleanedCount} processes cleaned`);
      ...
      return {
        ...
        details: [...cleanupDetails, `Cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`]
      };
    }
  }
  ...
  startMonitoring(intervalMs: number = 60000): void {
    // Feature Flag 체크
    const monitoringEnabled = process.env.FEATURE_PROCESS_MONITORING_ENABLED !== 'false';

    if (!monitoringEnabled) {
      console.log('⏸️ [ProcessProtection] Process monitoring disabled by feature flag');
      return;
    }

    console.log(`🔄 [ProcessProtection] Starting process monitoring (every ${intervalMs/1000}s)`);
    ...
    backgroundTaskManager.registerInterval(
      ...
      async () => {
        ...
        if (overloadStatus.isOverloaded) {
          console.log(`🚨 [ProcessProtection] ${overloadStatus.severity.toUpperCase()}: ${overloadStatus.reason}`);

          if (overloadStatus.severity === 'critical' || overloadStatus.severity === 'high') {
            console.log('🧹 [ProcessProtection] Triggering automatic cleanup...');
...
// 싱글톤 인스턴스 내보내기
export const processOverloadProtection = ProcessOverloadProtection.getInstance();