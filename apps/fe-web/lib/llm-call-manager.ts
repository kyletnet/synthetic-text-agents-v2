...
class LLMCallTracker {
  static recordCall(result: LLMCallResult) {
    ...
    if (!result.success && result.error?.type === 'auth') {
      console.error(`🚨 [CRITICAL] API Key authentication failed: ${result.error.message}`);
    } else if (result.source === 'fallback') {
      console.warn(`⚠️ [FALLBACK] LLM call failed, using template: ${result.error?.message || 'unknown reason'}`);
    }

    // 성공적인 LLM 호출 로깅
    if (result.success && result.source === 'llm') {
      console.log(`✅ [LLM] Successful ${result.metadata.model} call (${result.metadata.duration}ms)`);
    }
  }
  ...
  static getStats(timeWindow?: number): LLMCallStats {
    ...
    const recentCalls = tracker.calls.filter(call =>
      now - new Date(call.metadata.timestamp).getTime() < windowMs
    );

    const totalCalls = recentCalls.length;
    const successfulCalls = recentCalls.filter(c => c.success && c.source === 'llm').length;
    const fallbackCalls = recentCalls.filter(c => c.source === 'fallback').length;
    const errorCalls = recentCalls.filter(c => !c.success).length;

    const errorBreakdown: Record<string, number> = {};
    recentCalls.filter(c => !c.success).forEach(call => {
      const errorType = call.error?.type || 'unknown';
      errorBreakdown[errorType] = (errorBreakdown[errorType] || 0) + 1;
    });

    const totalResponseTime = recentCalls.reduce((sum, call) => sum + call.metadata.duration, 0);
  ...
  static getDiagnostics(): {
    ...
    if (stats.fallbackRate > 0.5) {
      issues.push(`높은 Fallback 발생률: ${Math.round(stats.fallbackRate * 100)}%`);
      recommendations.push('ANTHROPIC_API_KEY 설정 확인 및 네트워크 상태 점검');
    }

    if (stats.successRate < 0.7 && stats.totalCalls > 10) {
      issues.push(`낮은 LLM 호출 성공률: ${Math.round(stats.successRate * 100)}%`);
...
export class LLMCallManager {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (!this.isRetryable(errorType)) {
          console.error(`❌ [NO-RETRY] ${errorType} error - 재시도 중단: ${error.message}`);
          break;
        }
        ...
        if (attempt < this.maxRetries) {
          ...
          console.log(`⏳ [BACKOFF] ${errorType} error - ${delay}ms 후 재시도 (${attempt}/${this.maxRetries})`);
  ...
  private static categorizeError(error: any): 'network' | 'auth' | 'rate_limit' | 'format' | 'timeout' | 'unknown' {
    ...
    if (code === 401 ||
      ...
      console.error(`🚨 [CRITICAL] API Key authentication failed: ${error.message}`);
      return 'auth';
    }
    ...
    // 알 수 없는 오류 - 추가 분석 필요
    console.warn(`❓ [UNKNOWN] Unclassified error: ${error.message} (code: ${code})`);
    return 'unknown';
  }

  private static isRetryable(errorType: 'network' | 'auth' | 'rate_limit' | 'format' | 'timeout' | 'unknown'): boolean {
    switch (errorType) {
      case 'network':
      case 'timeout':
      case 'unknown':
   ...
   * 알림 전송 (오류 유형별 처리)
   */
  private static async sendAlert(errorType: string, message: string, sessionId: string): Promise<void> {
    try {
      // CRITICAL 및 HIGH 우선순위 오류에 대해서만 알림 전송
      if (errorType === 'auth') {
        console.error(`🚨 [ALERT-CRITICAL] API Key 인증 실패 - 즉시 조치 필요: ${message}`);
        ...
        console.warn(`⚠️ [ALERT-HIGH] Rate Limit 도달 - 요청 빈도 조절 필요: ${message}`);
  ...
  private static async handleErrorByType(
    errorType: 'network' | 'auth' | 'rate_limit' | 'format' | 'timeout' | 'unknown',
    error: any,
    sessionId: string
  ): Promise<void> {
    switch (errorType) {
      ...
      case 'timeout':
        // 네트워크/타임아웃 - 재시도 로직 적용
        console.log(`🔄 [RETRY] ${errorType} error - 재시도 적용: ${error.message}`);
        break;

      case 'format':
        // 포맷 오류 - 재시도 하지 않고 로깅
        console.error(`❌ [NO-RETRY] Format error - 재시도하지 않음: ${error.message}`);
        break;
