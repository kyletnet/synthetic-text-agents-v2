...
import { DataAugmentationSystem, AugmentationRequest } from './augmentation-utils';
...
export class SmartAugmentationSystem {
  ...
  static analyzeInput(input: string): InputAnalysis {
    ...
    const hasCodePatterns = /```|function|class|import|export|console\.log/.test(trimmed);
    ...
    const recommendedAugmentations = this.getRecommendations(type, characteristics);
  ...
  // 타입별 추천 증강 방법
  private static getRecommendations(type: InputAnalysis['type'], characteristics: string[]): RecommendedAugmentation[] {
  ...
  // 스마트 증강 실행
  static async performSmartAugmentation(input: string, options?: {
    ...
    // 결과 평가
    const evaluation = this.evaluateAugmentations(input, augmentations, analysis);
  ...
  private static evaluateAugmentations(
    ...
    const qualityScore = Math.min(Math.max(
      allResults.reduce((sum, r) => sum + (r.quality?.score || 0.7), 0) / allResults.length, 0), 1);
    const relevanceScore = this.calculateRelevance(original, allResults, analysis);
    const usefulnessScore = Math.min(Math.max(
      allResults.reduce((sum, r) => sum + (r.quality?.metrics?.usefulness || 0.7), 0) / allResults.length, 0), 1);

    const overallScore = (diversityScore + qualityScore + relevanceScore + usefulnessScore) / 4;
    ...
    return {
      ...
      metrics: {
        diversity: {
          ...
          description: '생성된 변형들의 표현 다양성 정도. 높을수록 더 다양한 스타일과 구조를 가짐.'
        },
        quality: {
          score: Math.round(qualityScore * 100) / 100,
          description: '언어적 품질과 자연스러움. 문법, 어휘 선택, 문체의 적절성을 종합 평가.'
        },
        relevance: {
          score: Math.round(relevanceScore * 100) / 100,
          description: '원본 내용과의 의미적 관련성. 핵심 메시지와 맥락이 얼마나 잘 보존되는지 측정.'
        },
        usefulness: {
          score: Math.round(usefulnessScore * 100) / 100,
          description: '실제 사용 가능성과 실용적 가치. 교육, 업무, 소통 등에서의 활용도.'
        },
      },
      recommendations,
      bestVariants,
  ...
  // 관련성 계산
  private static calculateRelevance(_original: string, results: any[], analysis: InputAnalysis): number {
    // 입력 타입에 따른 기대 관련성 조정
    ...
    // 최종 점수도 0-1 범위로 제한
    return Math.min(Math.max(baseScore * (typeWeights[analysis.type] || 0.75), 0), 1);
  }
  ...
  // 개선 추천사항 생성
  private static generateRecommendations(overallScore: number, metrics: any): string[] {
    const recommendations: string[] = [];

    if (overallScore < 0.6) {
      recommendations.push('전반적인 증강 품질이 낮습니다. 더 구체적이고 명확한 입력을 제공해보세요.');
    }

    if (metrics.diversity < 0.5) {
      recommendations.push('생성된 변형의 다양성이 부족합니다. 다른 증강 타입을 시도하거나 증강 개수를 늘려보세요.');
    }

    if (metrics.quality < 0.7) {
      recommendations.push('언어적 품질을 개선하기 위해 더 명확하고 문법적으로 올바른 입력을 사용해보세요.');
    }

    if (metrics.relevance < 0.6) {
      recommendations.push('원본과의 관련성이 낮습니다. RAG 컨텍스트를 활용하거나 더 관련성 높은 문서를 업로드해보세요.');
    }

    if (metrics.usefulness < 0.7) {
      recommendations.push('실용성을 높이기 위해 구체적인 사용 목적에 맞는 증강 타입을 선택해보세요.');
    }

    if (recommendations.length === 0) {
      recommendations.push('훌륭한 증강 결과입니다! 생성된 변형들을 다양한 용도로 활용해보세요.');
    }
