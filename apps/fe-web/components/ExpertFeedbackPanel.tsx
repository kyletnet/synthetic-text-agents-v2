...
const FOCUS_AREAS = [
  { id: 'diversity', label: '다양성', description: '표현 방식, 문체, 구조의 다양화' },
...
const PRIORITY_LEVELS = [
  { value: 'high', label: '높음', color: 'bg-red-100 text-red-800', description: '즉시 개선 필요' },
  { value: 'medium', label: '보통', color: 'bg-yellow-100 text-yellow-800', description: '점진적 개선' },
  { value: 'low', label: '낮음', color: 'bg-green-100 text-green-800', description: '선택적 개선' }
];
...
export default function ExpertFeedbackPanel({ sessionId, currentResult, onFeedbackSubmit }: ExpertFeedbackProps) {
      ...
      <CardContent className="space-y-6">
        ...
        {weakAreas.length > 0 && (
          <Alert>
            ...
            <AlertDescription>
              <strong>개선 추천 영역:</strong> {weakAreas.map(area => area.name).join(', ')}
              <div className="mt-2 text-sm">
                이 영역들에 대한 구체적인 피드백을 제공하면 더 효과적인 개선이 가능합니다.
              </div>
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-3">
          ...
          {showExamples && (
            <div className="bg-gray-50 p-4 rounded-lg space-y-4">
              {FEEDBACK_EXAMPLES.map((category, idx) => (
                <div key={idx}>
                  <h5 className="font-medium text-sm text-gray-700 mb-2">{category.category}</h5>
          ...
          <div className="text-sm text-gray-600">
            💡 구체적이고 실행 가능한 개선 방향을 제시해주세요. 시스템이 자동으로 전략을 수립합니다.
          </div>
        </div>
        ...
        <Alert>
          ...
          <AlertDescription>
            <strong>다음 단계:</strong> 제출된 피드백을 8-Agent 시스템이 분석하여 구체적인 개선 전략을 수립하고,
            자동으로 개선된 증강 결과를 생성합니다.
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
