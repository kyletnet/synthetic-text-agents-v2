...
export default function QualitySummaryDashboard({ result }: QualitySummaryProps) {
  // 전체 증강 결과 통계 계산
  const allResults = result.augmentations.flatMap(aug => aug.results);
  const totalVariants = allResults.length;

  // 품질 분포 계산
  const qualityDistribution = allResults.reduce((acc, item) => {
  ...
  const getMetricStatus = (score: number) => {
    if (score >= 0.8) return { status: 'excellent', icon: CheckCircle2, color: 'text-green-600' };
    if (score >= 0.6) return { status: 'good', icon: TrendingUp, color: 'text-blue-600' };
    if (score >= 0.4) return { status: 'needs-improvement', icon: AlertTriangle, color: 'text-yellow-600' };
  ...
  const weakAreas = Object.entries(result.evaluation.metrics)
    ...
    .map(([key, metric]) => ({
      name: key === 'diversity' ? '다양성' :
            key === 'quality' ? '품질' :
            key === 'relevance' ? '관련성' : '유용성',
      score: metric.score,
      key
    }));
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{distributionPercentages.high}%</div>
              <div className="text-sm text-gray-600">고품질 (0.8+)</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-600">{distributionPercentages.medium}%</div>
              <div className="text-sm text-gray-600">중품질 (0.6-0.8)</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{distributionPercentages.low}%</div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-green-700">고품질</span>
                  <span className="text-sm">{qualityDistribution.high}개</span>
                </div>
                <Progress value={distributionPercentages.high} className="h-3" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-yellow-700">중품질</span>
                  <span className="text-sm">{qualityDistribution.medium}개</span>
                </div>
                <Progress value={distributionPercentages.medium} className="h-3" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-red-700">저품질</span>
              ...
              return (
                <div key={key} className="space-y-3 p-4 border rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={metric.score >= 0.6 ? 'default' : 'secondary'}>
              ...
              {weakAreas.map((area, index) => (
                <Alert key={area.key} variant="destructive">
                  ...
                  <AlertDescription>
                    ...
                    이 영역에 대한 구체적인 피드백을 제공하면 시스템이 자동으로 개선 전략을 수립합니다.
                  </AlertDescription>
                </Alert>
              ))}
            </div>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <h4 className="font-medium text-green-700">효과적인 피드백 예시</h4>
                <ul className="text-sm text-gray-600 space-y-1">
                  <li>• "다양성을 높이려면 문체를 격식체와 반말체로 섞어서 생성해줘"</li>
                  <li>• "질문의 난이도를 초급/중급/고급으로 나누어서 만들어줘"</li>
                  <li>• "실무에서 바로 사용할 수 있는 구체적인 예시를 더 포함해줘"</li>
                  <li>• "전문 용어 설명을 더 자세히 추가해서 이해하기 쉽게 해줘"</li>
                </ul>
              </div>
              <div className="space-y-2">
