  ...
  try {
    ...
    const breakerStates = allBreakers.map(breaker => {
      ...
      const timeSinceFailure = state.lastFailureTime ?
        Math.round((Date.now() - state.lastFailureTime) / 1000) : undefined;
      ...
      return {
        ...
        permanentOpenSince: state.permanentOpenTimestamp ?
          new Date(state.permanentOpenTimestamp).toISOString() : undefined
      };
    });
    ...
    if (healingStats.isDormant) {
      ...
      recommendations.push("Call POST /api/system/heal/resume with valid reason to resume");
      recommendations.push("Verify API keys are configured before resuming");
      overallStatus = "critical";
    }

    // Circuit Breaker PERMANENT_OPEN 체크
    const permanentOpenBreakers = breakerStates.filter(b => b.state === 'PERMANENT_OPEN');
    if (permanentOpenBreakers.length > 0) {
      permanentOpenBreakers.forEach(breaker => {
        issues.push(`CRITICAL: Circuit Breaker '${breaker.name}' PERMANENTLY OPEN - ${breaker.permanentOpenReason}`);
      });
      recommendations.push("Manual intervention required - check circuit breaker status");
      if (overallStatus !== "critical") overallStatus = "critical";
    }

    // Background Task 과부하 체크
    if (taskStats.totalTasks > 10) {
      issues.push(`WARNING: Excessive background tasks (${taskStats.totalTasks}/10 limit)`);
      recommendations.push("Review background task list - possible memory leak");
      if (overallStatus === "healthy") overallStatus = "warning";
    }

    // 연속 실패 체크
    if (healingStats.consecutiveFailures >= 5) {
      issues.push(`WARNING: Self-Healing has ${healingStats.consecutiveFailures} consecutive failures`);
      recommendations.push("System approaching Dormant Mode threshold (10 failures)");
      if (overallStatus === "healthy") overallStatus = "warning";
    }
    ...
    if (mockContaminationRisk === "critical") {
      if (!envPolicy.strictMode && process.env.NODE_ENV === 'production') {
        ...
        issues.push("CRITICAL: No API keys configured - system cannot function");
      ...
      issues.push(`WARNING: ${keyStats.failedKeys}/${keyStats.totalKeys} API keys failed - reduced capacity`);
      ...
      issues.push(`INFO: ${keyStats.failedKeys} API keys disabled but system functional`);
    }

    // Feature Flag 상태 추가
    if (!healingStats.isDormant && process.env.FEATURE_AUTO_HEALING_ENABLED === 'false') {
      recommendations.push("Auto-healing is disabled - manual intervention may be required");
    }
