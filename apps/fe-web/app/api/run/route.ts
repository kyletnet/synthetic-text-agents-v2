...
function runBestEffortCleanup() {
  ...
  try { cleanupOldFiles(runLogsDir, keepDays); } catch {}
  try { cleanupOldFiles(decisionsDir, keepDays); } catch {}
}
// ===== end helpers =====
...
async function mockProvider(request: RunRequest): Promise<RunResult> {
  try {
    ...
    const templatePath = path.join(process.cwd(), "../../docs/TEMPLATES/expected_run_result.json")
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const outDir = path.join(process.cwd(), 'apps', 'fe-web', 'docs', 'RUN_LOGS');
      ...
      const samplesCount = Array.isArray(result?.samples) ? result.samples.length : 0;

      const guidelines = typeof request?.inputs?.guidelines === 'string'
      ...
      try { fs.writeFileSync(fpath, md, 'utf-8'); } catch (_) {}
    } catch (_) {
      // no-op
    }
    ...
    try {
      ...
      const enablePlan = String(process.env.IMPROVEMENT_ENABLE_PLAN ?? 'true') === 'true';

      const samples = Array.isArray(result?.samples) ? result.samples : [];
      const failed = samples.filter((s:any) => (s?.status && String(s.status).toLowerCase() !== 'passed'));

      if (failed.length > 0) {
        // sort: worst first (lowest score), then highest latency, then id
        failed.sort((a:any,b:any) => {
          const sa = Number(a?.score ?? Infinity), sb = Number(b?.score ?? Infinity);
          if (sa !== sb) return sa - sb;
          const la = Number(a?.latencyMs ?? 0), lb = Number(b?.latencyMs ?? 0);
        ...
        const issueToExpert = (issue:string) => {
          ...
          if (t.includes('halluc')) return { expert: 'FactChecker', actions: ['verify sources','add citations','re-answer'] };
          if (t.includes('format'))  return { expert: 'Formatter', actions: ['apply schema','fix formatting','validate output'] };
          if (t.includes('easy'))    return { expert: 'DifficultyTuner', actions: ['raise constraints','increase complexity'] };
          if (t.includes('unclear')) return { expert: 'Clarifier', actions: ['tighten wording','add examples'] };
          if (t.includes('long'))    return { expert: 'Conciseness', actions: ['compress','bulletize','limit length'] };
          return { expert: 'GeneralReviewer', actions: ['review','patch','retest'] };
        };

        const plan = enablePlan ? topK.map((s:any) => {
          const issues = Array.isArray(s.issues) ? s.issues : [];
          const mappings = issues.length ? issues.map((iss:string) => issueToExpert(iss)) : [issueToExpert('general')];
          return {
            sampleId: s.id,
            issues: issues,
            modules: mappings.map((m: any) => m.expert),
            actions: Array.from(new Set(mappings.flatMap((m: any) => m.actions))),
          };
        }) : [];
        ...
        const decisionsDir = path.join(process.cwd(), 'apps', 'fe-web', 'docs', 'DECISIONS');
        fs.mkdirSync(decisionsDir, { recursive: true });

        const fpath = path.join(decisionsDir, `improvement_${yyyy}-${mm}-${dd}.md`);

        const hdr = `# Improvement Plan — ${yyyy}-${mm}-${dd} ${now.toISOString()}\n`;
        const sec1 = [
          `## Top-${topK.length} Failed (K=${k})`,
          '```json',
          JSON.stringify({ runTs: now.toISOString(), topKFailed: topK }, null, 2),
          '```',
          ''
        ].join('\n');
    ...
    try { runBestEffortCleanup(); } catch {}
