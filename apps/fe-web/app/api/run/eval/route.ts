    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        for (const line of lines) {
          if (line.includes('Overall score') || line.includes('Quality score')) {
            const match = line.match(/(\d+(?:\.\d+)?)\%?/)
            if (match) overallScore = parseFloat(match[1])
          }
          if (line.includes('Quality level') || line.includes('Recommendation')) {
        ...
        resolve({
          ...
          result: {
            ...
            qualityLevel: qualityMapping[qualityLevel as keyof typeof qualityMapping] || qualityLevel,
            issuesCount,
            exitCode: code,
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[Eval API] Starting quality evaluation with eval ID ${evalId}`)
