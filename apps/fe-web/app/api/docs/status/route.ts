...
export async function GET() {
  ...
  if (stamp && head) {
    const diff = execSync(`git diff --name-only ${stamp.head}..${head}`, {
      cwd: repoRoot,
    })
      .toString()
      .split("\n")
      .filter(Boolean);
    const baseWatched =
      Array.isArray(stamp.watched) && stamp.watched.length
        ? stamp.watched
        : watched;
    const touched = diff.filter((f) =>
      baseWatched.some((w: string) => f.startsWith(w) || f === w),
    );
    if (touched.length > 0) {
      stale = true;
