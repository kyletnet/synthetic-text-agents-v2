...
async function smartAugmentHandler(request: NextRequest) {
  ...
  try {
    ...
    // 스마트 증강 실행
    const session = await SmartAugmentationSystem.performSmartAugmentation(
      input,
      {
    ...
    if (envPolicy.warnings.length > 0) {
      envPolicy.warnings.forEach((warning) =>
        console.warn(`⚠️ [ExecutionVerifier] ${warning}`),
      );
    }
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        totalVariants: session.augmentations.reduce(
          (sum, aug) => sum + aug.results.length,
          0,
        ),
