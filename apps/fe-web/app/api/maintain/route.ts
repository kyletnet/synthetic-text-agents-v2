...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // 최근 실패 분석
    const lastFailure = llmStats.recentFailures.length > 0
      ? {
          type: llmStats.recentFailures[0].error?.type || 'unknown',
          message: llmStats.recentFailures[0].error?.message || 'Unknown error',
    ...
    const worstErrorTypes = Object.entries(llmStats.errorBreakdown)
      .map(([type, count]) => ({
        type,
        count,
        percentage: llmStats.totalCalls > 0
    ...
    if (fallbackRate > 30) {
      recommendations.push("HIGH: Fallback rate는 30% 이하로 유지해야 합니다. API 키와 네트워크 상태를 점검하세요.");
    }

    if (mockContaminationRisk === "critical") {
      recommendations.push("CRITICAL: Production 환경에서 Strict Mode가 비활성화되었습니다. 즉시 API 키를 설정하세요.");
    }

    if (llmStats.errorBreakdown.auth > 0) {
      recommendations.push("URGENT: API 키 인증 오류가 발생하고 있습니다. ANTHROPIC_API_KEY를 확인하세요.");
    }

    if (llmStats.errorBreakdown.rate_limit > 0) {
      recommendations.push("WARNING: Rate limit에 도달했습니다. 요청 빈도를 조절하거나 API 플랜을 확인하세요.");
    }

    if (llmStats.averageResponseTime > 10000) {
      recommendations.push("INFO: 평균 응답 시간이 10초를 초과합니다. 네트워크 상태를 점검하세요.");
    }
    ...
    const maintenanceReport: MaintenanceReport = {
      ...
      environmentStatus: {
        apiKeyConfigured: process.env.ANTHROPIC_API_KEY ?
          !process.env.ANTHROPIC_API_KEY.includes('your_api_key_here') : false,
    ...
    // 상태 코드 결정
    const statusCode = mockContaminationRisk === "critical" || llmDiagnostics.status === "critical"
      ? 503
      : mockContaminationRisk === "high" || llmDiagnostics.status === "warning"
