...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`💬 Expert feedback received for session ${feedback.sessionId}:`);
    ...
    return NextResponse.json({
      ...
      message: '피드백이 성공적으로 접수되었습니다. 3-Agent 시스템이 개선 전략을 수립 중입니다.',
      nextSteps: [
        '1. 피드백 해석 및 분석',
        '2. 개선 전략 수립',
        '3. 자동 개선 실행',
...
function detectIntent(text: string) {
  ...
  if (lowerText.includes('다양') || lowerText.includes('여러') || lowerText.includes('섞어')) {
    return 'diversity_improvement';
  }
  if (lowerText.includes('품질') || lowerText.includes('정확') || lowerText.includes('자연스럽')) {
    return 'quality_enhancement';
  }
  if (lowerText.includes('실용') || lowerText.includes('활용') || lowerText.includes('사용')) {
    return 'usefulness_boost';
  }
  if (lowerText.includes('관련') || lowerText.includes('맥락') || lowerText.includes('연결')) {
...
function estimateImpact(priority: string, areaCount: number) {
  ...
  switch (priority) {
    case 'high': baseImpact = 80; break;
    case 'medium': baseImpact = 60; break;
    case 'low': baseImpact = 40; break;
  }
