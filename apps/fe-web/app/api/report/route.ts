...
async function parseSessionReport(filePath: string): Promise<Partial<QualityMetrics> | null> {
  try {
    ...
    if (jsonMatches) {
      for (const match of jsonMatches) {
        try {
          ...
          if (data.quality_score_summary || data.metric_scores || data.overall_score) {
            return data
          }
        } catch (parseError) {
          continue
...
async function parseBaselineReport(filePath: string): Promise<Partial<QualityMetrics> | null> {
  try {
    const content = await fs.readFile(filePath, 'utf-8')
    ...
    if (lines.length > 0) {
      ...
      if (data.quality_summary || data.metrics) {
        return {
          overall_score: data.overall_score || data.quality_summary?.overall_score || 0,
          metric_scores: data.metrics || data.quality_summary?.metric_scores || {}
        }
      }
    }
...
async function findReportFiles(): Promise<{ sessionReport?: string; baselineReport?: string }> {
