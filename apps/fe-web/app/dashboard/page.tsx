  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      >
        ...
        <TabsContent value="quality">{renderQualityDashboard()}</TabsContent>

        <TabsContent value="abtests">{renderABTestDashboard()}</TabsContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-center">
                <div
                  className={`text-4xl font-bold mb-2 ${getScoreColor(metrics.overall_score)}`}
                >
                ...
                <div className="space-y-1">
                  {metrics.threshold_validation.p0_violations.length > 0 && (
                    <Badge variant="destructive" className="text-xs">
                      {metrics.threshold_validation.p0_violations.length}{" "}
                      Critical
                    </Badge>
                  )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${
                      metrics.metric_scores.duplication_rate <= 0.15
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${
                      metrics.metric_scores.evidence_presence_rate >= 0.8
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
                    {(
                      metrics.metric_scores.evidence_presence_rate * 100
                    ).toFixed(1)}
                    %
                  </span>
                </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${
                      metrics.metric_scores.hallucination_rate <= 0.05
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
                    {(metrics.metric_scores.hallucination_rate * 100).toFixed(
                      1,
                    )}
                    %
                  </span>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${
                      metrics.metric_scores.coverage_score >= 0.8
                        ? "text-green-600"
                        : "text-yellow-600"
                    }`}
                  >
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-semibold ${
                      metrics.metric_scores.pii_violations === 0
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`font-semibold ${
                        metrics.metric_scores.inference_type_ratio >= 0.1
                          ? "text-green-600"
                          : "text-yellow-600"
                      }`}
                    >
                      {(
                        metrics.metric_scores.inference_type_ratio * 100
                      ).toFixed(1)}
                      %
                    </span>
                  </div>
              ...
              <div className="space-y-4">
                ...
                {metrics.threshold_validation.p0_violations.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {metrics.threshold_validation.p0_violations.map(
                        (issue, idx) => (
                          <li
                            key={idx}
                ...
                {metrics.threshold_validation.p1_warnings.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {metrics.threshold_validation.p1_warnings.map(
                        (warning, idx) => (
                          <li
                            key={idx}
                ...
                {metrics.threshold_validation.p2_issues.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {metrics.threshold_validation.p2_issues.map(
                        (issue, idx) => (
                          <li
                            key={idx}
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {getMostCostEfficient()?.variantName}
                  </p>
                </div>
                  ...
                  {abTestResults.map((result) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <Badge
                          variant={
                            result.avgQualityScore >= 8.5
                              ? "default"
                              : result.avgQualityScore >= 7.5
                                ? "secondary"
                                : "destructive"
                          }
                        >
            ...
            <div className="space-y-3">
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <p className="text-green-700">
                  <strong>{getBestVariant()?.variantName}</strong> shows the
                  highest quality scores with{" "}
                  {getBestVariant()?.avgQualityScore.toFixed(1)}/10 average
                  rating.
                </p>
              </div>
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <p className="text-blue-700">
                  ...
                  <strong>{getFastestVariant()?.variantName}</strong>
                  with {getFastestVariant()?.avgProcessingTime}ms average
                  processing time.
                </p>
              </div>
              ...
              <div className="p-4 bg-purple-50 border border-purple-200 rounded-lg">
                ...
                <p className="text-purple-700">
                  <strong>{getMostCostEfficient()?.variantName}</strong> offers
                  the best quality-to-cost ratio at $
                  {getMostCostEfficient()?.totalCost.toFixed(2)} total cost.
                </p>
              </div>
            </div>
          </CardContent>
