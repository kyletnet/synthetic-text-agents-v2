...
try {
  ...
  console.log(
    `📁 Loaded ${fixtures.length} test fixtures from ${
      config.extended ? "extended" : "standard"
    } set`,
  );
} catch (error) {
...
console.log(
  `  Cases: ${metrics.cases_passed}/${metrics.cases_total} passed (${(
...
if (
  ...
  violations.push(
    `Pass rate: ${metrics.pass_rate.toFixed(
      3,
    )} vs baseline ${baselineMetrics.pass_rate.toFixed(3)} (tolerance: ±${
      tolerance.pass_rate
    })`,
  );
}
...
if (
  ...
  violations.push(
    `Mean score: ${metrics.mean_score.toFixed(
      3,
    )} vs baseline ${baselineMetrics.mean_score.toFixed(3)} (tolerance: ±${
      tolerance.mean_score
    })`,
  );
}
...
if (
  ...
  violations.push(
    `P50 latency: ${metrics.p50_ms}ms vs baseline ${
      baselineMetrics.p50_ms
    }ms (tolerance: +${tolerance.p50_ms * 100}%)`,
  );
}
...
if (
  ...
  violations.push(
    `P95 latency: ${metrics.p95_ms}ms vs baseline ${
      baselineMetrics.p95_ms
    }ms (tolerance: +${tolerance.p95_ms * 100}%)`,
  );
}
...
if (config.freeze) {
  ...
  console.log(
    `   Mean score: ${metrics.mean_score.toFixed(
      3,
    )}, Pass rate: ${metrics.pass_rate.toFixed(3)}`,
  );
...
${results
  .map(
    (r) =>
      `| ${r.id} | ${r.score.toFixed(3)} | ${r.status} | ${r.duration_ms}ms |`,
  )
  .join("\n")}
