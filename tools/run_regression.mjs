...
console.log(`Target: ${config.target}, Mode: ${config.mode}, Offline: ${config.offline}, Extended: ${config.extended}, Freeze: ${config.freeze}`);

// Load test fixtures
let fixtures;
try {
  const activeFixturesPath = config.extended ? EXTENDED_FIXTURES_PATH : FIXTURES_PATH;
  const fixturesContent = readFileSync(activeFixturesPath, 'utf8');
  fixtures = fixturesContent.trim().split('\n').map(line => JSON.parse(line));
  console.log(`📁 Loaded ${fixtures.length} test fixtures from ${config.extended ? 'extended' : 'standard'} set`);
} catch (error) {
...
try {
  ...
  console.log(`📊 Loaded baseline metrics (last updated: ${baseline.last_updated})`);
} catch (error) {
...
const tempInputContent = fixtures.map(f => ({
  question: f.question,
  answer: f.answer,
  context: f.context
...
console.log(`  Cases: ${metrics.cases_passed}/${metrics.cases_total} passed (${(metrics.pass_rate * 100).toFixed(1)}%)`);
...
if (Math.abs(metrics.pass_rate - baselineMetrics.pass_rate) > tolerance.pass_rate) {
  violations.push(`Pass rate: ${metrics.pass_rate.toFixed(3)} vs baseline ${baselineMetrics.pass_rate.toFixed(3)} (tolerance: ±${tolerance.pass_rate})`);
}

if (Math.abs(metrics.mean_score - baselineMetrics.mean_score) > tolerance.mean_score) {
  violations.push(`Mean score: ${metrics.mean_score.toFixed(3)} vs baseline ${baselineMetrics.mean_score.toFixed(3)} (tolerance: ±${tolerance.mean_score})`);
}

// For latency metrics, only flag if performance got WORSE (higher latency)
if ((metrics.p50_ms - baselineMetrics.p50_ms) / baselineMetrics.p50_ms > tolerance.p50_ms) {
  violations.push(`P50 latency: ${metrics.p50_ms}ms vs baseline ${baselineMetrics.p50_ms}ms (tolerance: +${tolerance.p50_ms * 100}%)`);
}

if ((metrics.p95_ms - baselineMetrics.p95_ms) / baselineMetrics.p95_ms > tolerance.p95_ms) {
  violations.push(`P95 latency: ${metrics.p95_ms}ms vs baseline ${baselineMetrics.p95_ms}ms (tolerance: +${tolerance.p95_ms * 100}%)`);
}
...
if (config.freeze) {
  ...
  console.log(`   Mean score: ${metrics.mean_score.toFixed(3)}, Pass rate: ${metrics.pass_rate.toFixed(3)}`);
...
// Write JSON summary
writeFileSync(`${REPORTS_DIR}/regression_summary.json`, JSON.stringify(summary, null, 2));
