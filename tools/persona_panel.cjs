...
function pickSamples(inFile, n){
  ...
  const sel = [];
  for(let i=0;i<lines.length && sel.length<n;i+=step){ sel.push(lines[i]); }
  return sel.map(l=>JSON.parse(l));
}

function rotate(keys){ let i=0; return ()=> keys[(i++)%keys.length]; }
...
async function main(){
  ...
  const keys = (process.env.ANTHROPIC_API_KEYS||'').split(',').map(s=>s.trim()).filter(Boolean);
  if(!IN || !keys.length){ console.log('skip'); process.exit(0); }
...
- 결과는 한국어로 간결하게.`;
  ...
  for(const s of samples){
    ...
    const key = nextKey();
    const msg = await callAnthropic(key, model, [{role:'user', content:text}]).catch(e=>`(API 에러) ${e.message}`);
