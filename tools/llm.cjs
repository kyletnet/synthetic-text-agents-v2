const crypto=require('crypto'); const fs=require('fs'); const path=require('path'); require('dotenv').config();
...
async function throttle(){ if(inflight<MAX_CONC){inflight++; return;}
  await new Promise(r=>queue.push(r)); inflight++;
}
function release(){ inflight--; const n=queue.shift(); if(n) n(); }

function withTimeout(p,ms){ return Promise.race([p,new Promise((_,rej)=>setTimeout(()=>rej(new Error('TIMEOUT')),ms))]); }

function tokenGuard(s, max=8000){ if(!s) return s; const words=s.split(/\s+/); if(words.length>max){ return words.slice(0,max).join(' ')+' ...[TRUNC]'; } return s; }
...
async function callLLM({prompt,system}){
  ...
  try{
    ...
    const inTok = json?.usage?.input_tokens || Math.round((prompt.length+(system||'').length)/4);
...
function saveRunLog(entry){
  ...
  let arr=[]; if(fs.existsSync(p)) arr=JSON.parse(fs.readFileSync(p,'utf8'));
  const safe={...entry};
  if(safe.prompt) safe.prompt=maskPII(safe.prompt).slice(0,500);
  if(safe.evidence_excerpt) safe.evidence_excerpt=maskPII(safe.evidence_excerpt).slice(0,300);
  arr.push(safe);
