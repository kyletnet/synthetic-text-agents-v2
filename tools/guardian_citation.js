...
export function guardianCitation(payload){
  ...
  let best=0; for(const c of cites){ const s=alignmentScore(answer,c?.snippet||''); if(s>best) best=s; }
  const result={ citation_presence:presence, snippet_alignment:Number(best.toFixed(4)) };
  ...
  if(payload.log!==false){
    ...
    const line={
      ...
      routing_path: '-',
      quality_score: null, p95_ms: null, cost: null, cost_cap_hit: 0,
