  ...
  for(const rel of candidates){
    ...
    if(fs.existsSync(abs) && abs.endsWith('.js')){
      try{
        ...
        console.warn(`[warn] RAG service import failed at ${rel}: ${e?.message||e}`);
      }
    }
  }
  console.log('[info] RAG service not found; skipping actual RAG call (router demo only).');
  return { ok:true, note:'rag-service-missing' };
}

// Node <20 compatibility for file URL helper without adding deps
function pathToFileURL(p){ const u = new URL('file://'); u.pathname = path.resolve(p).replace(/\\/g,'/'); return u; }
...
(async function main(){
  ...
  if (enableRag && seedDoc){
    if (fs.existsSync(seedDoc)){
      ...
      try{
        ...
        console.warn('[warn] RAG call failed, falling back to legacy:', e?.message||e);
  ...
  console.log(JSON.stringify({ run_id: runId, decided_routing: routing, note: resultNote }, null, 2));
