...
if (!fs.existsSync("docs/sample.txt")) {
  fs.writeFileSync(
    "docs/sample.txt",
    "유지관리 계약서: 30일 사전통지, 99.9% SLA, 현재지번 면책, 조기해지 위약금 50%.",
  );
}
...
const hasKey = !!(
  ...
  (fs.existsSync(".env") &&
    /ANTHROPIC_API_KEY=\S+/.test(fs.readFileSync(".env", "utf8")))
);

if (!hasKey) {
  console.log(
    "[INFO] Skip LLM call (no API key). Add ANTHROPIC_API_KEY then re-run.",
  );
  process.exit(0);
}
