...
const {
  inFile, outJson, runLog, model, panelSize, passThreshold,
  mode, sampleN, seed, maxRetries, reqTimeoutMs
} = CONFIG;

const keys = (process.env.ANTHROPIC_API_KEYS || '')
  .split(',').map(s => s.trim()).filter(Boolean);
if (!keys.length && process.env.KEY_FILE && fs.existsSync(process.env.KEY_FILE)) {
  const fileKeys = fs.readFileSync(process.env.KEY_FILE, 'utf8')
...
function persona(i){
  const presets = [
    {role:"교육 평가전문가", focus:"정확한 개념 이해, 학습 목표 적합성, 학년 적정 난이도"},
    {role:"데이터 검수 리드", focus:"형식 준수(Q/A), 근거 일관성, 중복/누락"},
    {role:"LLM 프로덕트 오너", focus:"사용자 가치, 답변 간결성, 리스크(환각/근거 미스)"}
  ];
...
function safeParseScore(s){
  ...
  try {
    ...
    return { score: sc, comment: (j.comment||'').toString().trim().slice(0,400) };
  } catch {
    ...
    while(attempt <= maxRetries){
      try{
        ...
        if(attempt>maxRetries){
          panel.push({score:0.0, comment:`API 실패: ${String(e).slice(0,150)}`});
          break;
        }
...
(async function main(){
  ...
  const out = {
    meta:{ inFile, total: all.length, used: results.length, panelSize, model, mode, sampleN, passThreshold },
    diversity:{}, // 4.1b에서 계산된 지표는 보고서에서 링크로 안내
    score:{ mean, passRate, bins, scores },
    results
  };
  fs.writeFileSync(outJson, JSON.stringify(out,null,2));
  fs.writeFileSync(runLog, JSON.stringify({ts:Date.now(), id:process.env.RUN_ID, config:CONFIG, usedKeys: keys.length}, null, 2));
