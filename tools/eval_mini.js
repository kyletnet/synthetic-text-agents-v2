...
const cases = JSON.parse(fs.readFileSync(path.join(root,'dev/runs/cases.json'),'utf8'));
const resultsPath = path.join(root,'dev/runs/results_sample.json');
if(!fs.existsSync(resultsPath)) {
  console.error('[hint] Create dev/runs/results_sample.json from your RAG output to evaluate.');
  process.exit(1);
}
const res = JSON.parse(fs.readFileSync(resultsPath,'utf8'));
const byId = Object.fromEntries(res.map(r=>[r.id,r]));
let hit=0, total=cases.length, lat=[], cost=[], cpHit=0;
for (const c of cases) {
  const r = byId[c.id]; if(!r){console.log(`[miss] ${c.id} no-result`); continue;}
  ...
  if(found) hit++;
  lat.push(r.latency_ms||0); cost.push(r.cost||0);
...
const p95 = (arr)=>{ if(arr.length===0) return 0; const a=[...arr].sort((a,b)=>a-b); const idx=Math.ceil(0.95*a.length)-1; return a[idx]; };
