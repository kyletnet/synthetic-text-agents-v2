...
function quantiles(nums){
  ...
  const q=(p)=>{ const i=(a.length-1)*p; const lo=Math.floor(i), hi=Math.ceil(i);
...
function sparkline(arr){
  // 8단계 블럭으로 간단 시각화
  const ticks = "▁▂▃▄▅▆▇█";
  const mn=Math.min(...arr,0), mx=Math.max(...arr,1e-9);
  const span = (mx-mn)||1;
  return arr.map(v=>{
...
module.exports = { tokenizeKoEn, ngrams, uniq, mean, quantiles, sparkline, mdTable };
