...
const bar = bins.map((n,i)=>`${(i/10).toFixed(1)}–${((i+1)/10).toFixed(1)}│${'█'.repeat(Math.min(40, n))} ${n}`).join('\n');
...
const worst = results.slice().sort((a,b)=>a.agg.median-b.agg.median).slice(0, Math.min(10, results.length));
const best  = results.slice().sort((a,b)=>b.agg.median-a.agg.median).slice(0, Math.min(5, results.length));

const casebook = `# Casebook (Hard cases)
${worst.map((r,idx)=>`## ${idx+1}. score=${r.agg.median.toFixed(2)} ${r.agg.pass?'(PASS)':'(FAIL)'}
**Q**: ${r.q}
**A**: ${r.a}
...
# Good examples
${best.map((r,idx)=>`## Top ${idx+1}. score=${r.agg.median.toFixed(2)}
**Q**: ${r.q}
**A**: ${r.a}
`).join('\n')}
`;
