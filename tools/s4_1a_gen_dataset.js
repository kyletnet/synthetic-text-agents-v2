...
function sentences(s){ return s.split(/(?<=[\.!\?]|다\.)\s+/).map(x=>x.trim()).filter(Boolean); }
...
function pickQuestion(txt){
  const t = txt;
  if(/[물|수증기|얼음|상태|응결|증발]/.test(t)) return "이 문단은 물의 상태 변화와 관련해 무엇을 설명하나요?";
  if(/원인|영향|요인|조건/.test(t)) return "이 문단에서 핵심 원인과 조건은 무엇인가요?";
...
function lenScore(ans){ // 40~160자 권장
  const n = ans.length;
  if(n<20) return 0.2;
  if(n>220) return 0.3;
...
function coverScore(chunk,a){
  const tkC = toks(chunk), tkA = toks(a);
  return overlap(tkC, tkA); // 0~1
}
function qualityScore(chunk, q, a){
  const wC=0.5, wF=0.2, wL=0.3;
...
function main(){
  ...
  md.push(`- **score avg**: ${avg.toFixed(3)}  |  **p50**: ${(qtl.p50??0).toFixed(3)}  |  **p95**: ${(qtl.p95??0).toFixed(3)}  |  **p99**: ${(qtl.p99??0).toFixed(3)}`);
  md.push(``);
  ...
  md.push(`|---|---|---|---:|---:|---:|---:|`);
  hi.forEach((x,i)=>{
    const m=x.meta, s=m.scores;
    md.push(`| ${i+1} | ${x.q} | ${x.a} | ${m.score.toFixed(2)} | ${s.coverage.toFixed(2)} | ${s.length.toFixed(2)} | ${s.format.toFixed(2)} |`);
  });
  md.push(``);
  md.push(`## Notes (for non-engineers)`);
  md.push(`- 이 리포트는 **LLM 호출 없이** 규칙만으로 만든 베이스라인 Q/A 품질을 가늠합니다.`);
  md.push(`- **score**는 덮어씀(답변-원문 겹침), 형식 적합성(물음표/문장), 길이 적정성(너무 짧거나 김) 등을 조합한 0~1 점수입니다.`);
  md.push(`- 이 분포를 베이스라인-제로로 저장해두고, 이후 LLM/다양성 지표/근거체크 등을 붙였을 때 개선폭을 비교하세요.`);
