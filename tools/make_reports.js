...
const { mdTable } = require('./_utils_diversity');
...
function main(){
  ...
  const i=args.indexOf('--in'); const inFile= i>=0? args[i+1]: null;
  ...
  if(!inFile) { console.error('need --in json'); process.exit(2); }
  ...
  sumMD.push(`- Pass threshold: **${meta.passThreshold}**  ·  Pass rate: **${(diversity.score.passRate*100).toFixed(1)}%**`);
  sumMD.push(`- Score mean/p50/p95: **${diversity.score.mean.toFixed(2)} / ${diversity.score.p50.toFixed(2)} / ${diversity.score.p95.toFixed(2)}**`);
  sumMD.push(`- Answer length (mean/p50/p95): **${diversity.lenA.mean.toFixed(1)} / ${diversity.lenA.p50.toFixed(0)} / ${diversity.lenA.p95.toFixed(0)}**`);
  sumMD.push(`- Lexical diversity (TTR mean): **${diversity.ttrMean.toFixed(2)}**`);
  sumMD.push(`- 3-gram overlap (dup-rate): **${(diversity.dup3Rate*100).toFixed(1)}%**  (낮을수록 중복↓)`);
  sumMD.push(`- Question type distribution: \`${Object.entries(diversity.qtypeDist).map(([k,v])=>`${k}:${v}`).join(', ')}\``);
  ...
  const mid = take(sorted.slice(Math.floor(sorted.length/2)), Math.ceil(samples/3));
  ...
  // 페르소나 노트가 있으면 첨부
  const pn = fs.readdirSync('reports').filter(f=>/^_PERSONA_NOTES_/.test(f)).sort().pop();
  if(pn){
    cb.push(`\n## 전문가 페르소나 노트`);
