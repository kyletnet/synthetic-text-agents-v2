...
for (let i = 0; i < args.length; i++) {
  switch (args[i]) {
    ...
    default:
      if (
        (!args[i].startsWith("--") && args[i].endsWith(".json")) ||
        args[i].endsWith(".jsonl")
      ) {
        config.files.push(args[i]);
      }
      break;
...
console.log(
  `🔍 Validating ${config.files.length} file(s) against ${config.schema} schema...`,
);
  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (isValid) {
        ...
        if (config.verbose) {
          console.log(
            `  ❌ Record ${i + 1} (${record.id || "no id"}): INVALID`,
          );
