...
export function recallAtK(goldDocIds, retrievedDocIds, k=5){
  ...
  const ret  = Array.from(new Set((retrievedDocIds||[]).map(String))).slice(0, k);
  let found = 0;
  for(const g of gold){ if(ret.includes(g)) found++; }
  const total = gold.length || 1;
  const recall = found / total;
  const hit = found > 0 ? 1 : 0;
  return { recall_at_k: Number(recall.toFixed(4)), hit_at_k: hit, found, total };
}
 ...
 */
export function citationPrecisionProxy(answer, citations, threshold=0.30){
  ...
  return { precision_proxy: Number(prec.toFixed(4)), positives: pos, total: arr.length, threshold };
}

/** Percentile utility (e.g., p=0.95) */
export function percentile(values, p=0.95){
  const v = (values||[]).map(Number).filter(n=>Number.isFinite(n)).sort((a,b)=>a-b);
