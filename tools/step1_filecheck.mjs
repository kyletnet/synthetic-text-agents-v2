...
function detectExampleJSON() {
  const candidates = [
    ...
    "example.json",
    "examples.json",
    ...walk(process.cwd()).filter((p) =>
      /[/\\](예제|예시|example)s?\.json$/i.test(p),
    ),
  ];
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i < 3)
      samples.push({
        i,
        len,
        preview: (s || "").slice(0, 80).replace(/\s+/g, " ").trim(),
      });
  }
...
function writeReadableMD(p, data) {
  ...
  if (data.example.found) {
    ...
    lines.push(
      `  - 평균/최소/최대 길이: ${ex.analysis.avg_len}/${ex.analysis.min_len}/${ex.analysis.max_len}`,
    );
    if (ex.analysis.notes?.length)
  ...
  if (!data.example.found)
    lines.push(
      "- 예제.json을 프로젝트 루트에 두거나 파일명을 예제.json으로 맞춰주세요.",
    );
  if (data.outputs.probe && data.outputs.probe.fail > 0)
    lines.push(
      "- outputs/*.jsonl 일부 줄이 손상되어 보입니다. 생성 루틴 재점검을 권장합니다.",
    );
  if (data.logs.probe && data.logs.probe.fail > 0)
    lines.push(
      "- RUN_LOGS/*.jsonl 일부 줄이 손상되어 보입니다. 로깅 시 JSON 직렬화 확인이 필요합니다.",
    );
  if (
    data.example.found &&
    data.example.analysis.valid &&
    data.example.analysis.count < 4
  )
    lines.push("- 멀티지문 QA 목표라면 최소 4개 청크 준비가 필요합니다.");
  lines.push("");
  lines.push("참고:");
...
function writeTechMD(p, data) {
  ...
  if (data.example.found) {
    ...
    if (data.example.analysis.notes?.length)
      lines.push(`- notes: ${data.example.analysis.notes.join("; ")}`);
  }
  lines.push("");
  ...
  if (data.outputs.probe)
    lines.push(
      `- latest_lines: ${data.outputs.probe.total}, ok: ${data.outputs.probe.ok}, fail: ${data.outputs.probe.fail}`,
    );
  lines.push("");
  ...
  if (data.logs.probe)
    lines.push(
      `- latest_lines: ${data.logs.probe.total}, ok: ${data.logs.probe.ok}, fail: ${data.logs.probe.fail}`,
    );
  lines.push("");
  lines.push("## decision");
...
(function main() {
  ...
  const outOK =
    !outProbe ||
    outProbe.fail <= Math.max(1, Math.floor(outProbe.total * 0.05));
  const logOK =
    !logProbe ||
    logProbe.fail <= Math.max(1, Math.floor(logProbe.total * 0.05));
  ...
  fs.writeFileSync(
    path.join(REPORT_DIR, "_STEP1_FILECHECK.json"),
  ...
  if (!ex.found)
    console.log(
      " - 예제.json을 찾지 못했습니다. 루트에 예제.json을 두거나 파일명을 맞춰주세요.",
    );
  if (ex.found && !ex.analysis.valid)
    console.log(
      ` - 예제.json 구조 감지 실패(shape=${ex.analysis.shape}). chunks 배열 형태인지 확인하세요.`,
    );
  if (outProbe && outProbe.fail > 0)
    console.log(
      ` - outputs 최신 JSONL 파싱 실패 줄: ${outProbe.fail}/${outProbe.total}`,
    );
  if (logProbe && logProbe.fail > 0)
    console.log(
      ` - RUN_LOGS 최신 JSONL 파싱 실패 줄: ${logProbe.fail}/${logProbe.total}`,
    );
})();
