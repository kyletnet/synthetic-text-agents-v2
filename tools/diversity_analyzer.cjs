...
function parseLine(line) {
  ...
  try {
    ...
    if (obj.q || obj.a) {
      return { Q: obj.q, A: obj.a, meta: obj.meta || {} };
    }
    // 기타 포맷 호환은 필요 시 추가
    return obj;
  } catch (e) {
...
function scoreOne(rec, docTopTokens) {
  const Q = rec.Q || "";
  const A = rec.A || "";
  const qt = tokenizeKoEn(Q),
    at = tokenizeKoEn(A);
  const lenQ = qt.length,
    lenA = at.length;
  const ttr = uniq(at).length / Math.max(1, at.length); // 어휘 다양도
  const okFormat =
    (/q[:：]/i.test(Q) || /\?$/.test(Q)) &&
    (/^a[:：]/i.test(A) || at.length > 0)
      ? 1
      : 0;
  // 키워드 커버리지: 문서 상위 토큰과 답변의 교집합 비율
  const cover = docTopTokens.length
    ? uniq(at).filter((t) => docTopTokens.includes(t)).length /
      Math.max(1, uniq(docTopTokens).length)
    : 0.0;
  ...
  const score =
    (0.4 * okFormat + 0.25 * cover + 0.2 * ttr + 0.15 * lenFit) * typeAdj;
...
function main() {
  ...
  const dup3Cnt = Object.values(triCount)
    .filter((c) => c > 1)
    .reduce((x, y) => x + y, 0);
  ...
  const passRate = scores.length
    ? scores.filter((x) => x >= passTh).length / scores.length
    : 0;
  ...
  const out = {
    ...
    diversity: {
      ...
      score: {
        ...
        mean: mean(scores),
        passRate,
        spark: sparkline(scores.map((x) => Math.round(x * 100) / 100)),
      },
    },
    per,
  };
