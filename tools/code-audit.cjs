...
function walk(dir){
  for(const name of fs.readdirSync(dir,{withFileTypes:true})){
    if(name.isDirectory()){
      ...
      if(/fetch\(.+?\)\s*;?\s*$/.test(s) && /timeout|AbortController/.test(s)===false){
        findings.push({file:fp, issue:'No timeout around fetch (consider AbortController/LLM_TIMEOUT_MS)'});
      }
      if(/Promise\.all\s*\(/.test(s) && /LLM_MAX_CONCURRENCY/.test(s)===false){
        findings.push({file:fp, issue:'Potential unbounded concurrency (guard with concurrency limiter)'});
      }
    }
  }
}
...
fs.writeFileSync('reports/CODE_AUDIT.md',
  `# Code Audit\n\nScore: ${out.score}/10\n\n` +
  (findings.length? findings.map((f,i)=>`- [${i+1}] ${f.file} — ${f.issue}`).join('\n') : '- No risky patterns detected.') + '\n'
);
