...
export async function runOnce(){
  ...
  const gold = JSON.parse(safeRead(path.join(root,'dev/runs/gold/p13_gold.json'))||'{"gold_doc_ids":["docA"],"thresholds":{"citation_precision":0.30}}');
  ...
  const { ms: latency_ms, value: enriched } = await timeIt(async () => {
    ...
    guardianCitation({ run_id, answer: e.answer, citations: e.citations, log: true });
    return e;
  });

  // compute recall@5 over doc ids
  const recall = recallAtK(gold.gold_doc_ids || ['docA'], retrievedTopK.map(r=>r.doc_id), 5);

  // compute citation precision proxy w.r.t. the final answer
  const cpp = citationPrecisionProxy(answer, enriched.citations, Number(gold?.thresholds?.citation_precision ?? 0.30));
...
if (import.meta.url === pathToFileURL(process.argv[1]).href){
  runOnce().catch(e=>{ console.error('[eval_runner_p13] error:', e); process.exit(1); });
}
