...
function readTodayLatencies(){
  ...
  for(const line of lines){
    try { const obj = JSON.parse(line); if(typeof obj.latency_ms === 'number') vals.push(obj.latency_ms); }
    catch {}
  }
  return vals;
}
...
async function main(){
  ...
  console.log(JSON.stringify({
    runs_in_session: N,
    session_p50_ms: Number(percentile(latencies,0.50).toFixed(3)),
...
main().catch(e=>{ console.error('[eval_runner_multi_p13] error:', e); process.exit(1); });
