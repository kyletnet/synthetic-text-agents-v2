...
export async function generateJSON(args) {
    ...
    const metaLog = (payload) => appendJSONL(`RUN_LOGS/${runId}.jsonl`, payload);
    if (DRY) {
        ...
        return { text: mock, json: undefined, usedDryRun: true };
    }
    ...
    while (attempt <= maxRetries) {
        ...
        try {
            ...
            const callOptions = {
                ...
                prompt: [
                    args.schemaHint
                        ? `Return ONLY JSON matching: ${args.schemaHint}`
                        : "",
                    args.prompt,
                ]
                    .filter(Boolean)
            ...
            return { text, json: parsed, usedDryRun: false };
        }
        catch (err) {
            lastErr = err;
            metaLog({
