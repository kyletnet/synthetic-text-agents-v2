...
export class LogForwarder extends EventEmitter {
        ...
        if (target.authentication) {
            switch (target.authentication.type) {
                case "apikey":
                    ...
                    break;
                case "basic": {
                    const basic = Buffer.from(`${target.authentication.credentials.username}:${target.authentication.credentials.password}`).toString("base64");
    ...
    checkRateLimit(target) {
        ...
        limiter.tokens = Math.min(target.rateLimiting.burstSize, limiter.tokens + tokensToAdd);
