...
export class ConfigService {
    ...
    static async loadConfig(configPath) {
        ...
        return {
            features: {
                rag: {
                    enabled: loadedConfig.features?.rag?.enabled ??
                        DEFAULT_CONFIG.features.rag.enabled,
                    topK: loadedConfig.features?.rag?.topK ??
                        DEFAULT_CONFIG.features.rag.topK,
                    minScore: loadedConfig.features?.rag?.minScore ??
    ...
    getRAGConfig() {
        ...
        return {
            ...
            indexPaths: ["docs/", "src/", "CLAUDE.md", "README.md"].filter((path) => path), // Filter out any empty paths
        };
    }
    isRAGEnabled() {
