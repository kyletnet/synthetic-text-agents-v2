...
export class PerformanceDashboard {
    ...
    alerts = [];
    alertThresholds;
    constructor(performanceMonitor, alertThresholds) {
    ...
    async getDashboardMetrics(timeWindow = 300000) {
        try {
    ...
    async getOverviewMetrics(timeWindow) {
    ...
    async getAgentMetrics(timeWindow) {
        ...
        return Object.entries(agentMetrics).map(([agentName, data]) => ({
            ...
            averageExecutionTime: data.executions.reduce((sum, exec) => sum + exec.duration, 0) /
                data.executions.length || 0,
            averageQualityScore: data.qualityScores.reduce((sum, score) => sum + score, 0) /
    ...
    async getTrendMetrics(timeWindow) {
        ...
        for (let i = buckets; i >= 0; i--) {
            ...
            // Response time trend
            const avgResponseTime = bucketMetrics.responseTimes.reduce((sum, rt) => sum + rt, 0) /
            ...
            // Quality score trend
            const avgQualityScore = bucketMetrics.qualityScores.reduce((sum, qs) => sum + qs, 0) /
    ...
    generateAlerts(overview, agents, system) {
        const now = new Date();
    ...
    addAlert(alertData) {
        ...
        const existingAlert = this.alerts.find((a) => a.title === alert.title &&
    ...
    getAgentPerformanceData(timeWindow) {
    ...
    getMetricsInTimeRange(start, end) {
...
export function initializePerformanceDashboard(performanceMonitor, alertThresholds) {
