...
export class ABTestManager {
    ...
    initializeDefaultVariants() {
        ...
        this.variants.set("specialist", {
            ...
            agentSelection: (_complexity, _domain) => {
                ...
                if (_complexity >= 8)
                    agents.push("psychology-specialist");
                return agents;
            },
            enabled: true,
        });
    ...
    calculateMetrics(response) {
        ...
        const qualityScore = response.metadata?.qualityScore || averageConfidence * 10; // Convert 0-1 to 0-10 scale
        // Calculate diversity (unique question types)
        const uniqueStarters = new Set(questions.map((q) => (q.question || "").split(" ").slice(0, 2).join(" "))).size;
