...
export class Orchestrator {
    ...
    async processRequest(request) {
        ...
        try {
            ...
            const promptSpec = context.sharedMemory["prompt-architect"]
                ?.result?.promptSpec;
            const baseGen = context.sharedMemory["qa-generator"]?.result || [];
            ...
            const auditor = results.find((r) => r.agentId === "quality-auditor");
            if (auditor?.result?.retrySuggested) {
                ...
                if (gen) {
                    ...
                    const beforeArr = context.sharedMemory["qa-generator"] || [];
                    const afterArr = retry || [];
    ...
    analyzeComplexity(request) {
        ...
        if (request.domainContext && request.domainContext !== "general")
            score += 2;
        if (request.qualityTarget && request.qualityTarget > 8)
            score += 1;
        return Math.min(10, Math.max(1, score));
    }
    selectAgents(complexity, domain) {
        const agents = ["meta-controller", "quality-auditor"];
        if (complexity >= 6)
            agents.push("prompt-architect", "qa-generator");
        if (complexity >= 8)
    ...
    async runCouncil(agentIds, request, context) {
        const results = [];
    ...
    compileResponse(results, startTime, agentsUsed) {
        ...
        for (const result of results) {
            if (typeof result === "object" && result !== null) {
                ...
                if (resultObj.agentId === "qa-generator" ||
