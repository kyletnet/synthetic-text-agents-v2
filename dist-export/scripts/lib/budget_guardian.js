...
const LEVEL = { none: 0, low: 1, medium: 2, high: 3, critical: 4 };
...
export class BudgetGuardian {
     ...
     */
    initializeRun(runId, sessionId, profile, budgetLimits, estimatedItems) {
        this.currentState = {
            run_id: runId,
     ...
     */
    checkBudget(estimatedCost = 0, estimatedTimeMs = 0, agentRole) {
        ...
        if (costUtilization > 0.9 || timeUtilization > 0.9) {
            ...
            warningLevel = maxLevel(warningLevel, "medium");
            ...
            warningLevel = maxLevel(warningLevel, "low");
        }
     ...
     */
    recordUsage(actualCost, actualTimeMs, agentRole, itemsProcessed = 1) {
        if (!this.currentState) {
    ...
    checkKillSwitch() {
        try {
            ...
            if (existsSync(this.killSwitchFile)) {
                ...
                const killSwitch = JSON.parse(content);
                const st = {
     ...
     */
    activateKillSwitch(reason, manualOverride = true) {
        const killSwitch = {
            enabled: true,
            triggered: true,
     ...
     */
    completeRun(finalStatus = "completed") {
        if (!this.currentState) {
            return null;
        }
     ...
     */
    generateRecommendations(costUtilization, timeUtilization, warnings) {
 ...
 */
export function createBudgetGuardian(baseDir) {
    return new BudgetGuardian(baseDir);
}
/**
 * Default budget limits based on profile
 */
export function getDefaultBudgetLimits(profile) {
    const profiles = {
        dev: {
    ...
    return profiles[profile] || profiles.dev;
}
//# sourceMappingURL=budget_guardian.js.map