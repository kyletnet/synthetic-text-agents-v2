...
export class DLQManager {
    ...
    toDLQ(runId, reason, exitCode, artifacts = [], options = {}) {
        ...
        try {
            ...
            if (typeof options.costUsd === "string")
                entry.cost_usd = options.costUsd;
            if (typeof options.sessionId === "string")
                entry.session_id = options.sessionId;
            if (options.notes)
                entry.notes = options.notes;
            const dlqEntry = entry;
     ...
     */
    extractFailReasons(reason, artifacts) {
        const reasons = [];
        // Primary reason
        reasons.push(reason);
...
if (typeof require !== "undefined" && require.main === module) {
    ...
    if (args[0] === "--to-dlq") {
        ...
        for (let i = 4; i < args.length; i += 2) {
            ...
            else if (key === "--budget-usd")
                options.budgetUsd = value;
            else if (key === "--cost-usd")
