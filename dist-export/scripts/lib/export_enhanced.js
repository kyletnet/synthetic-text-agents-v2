...
export class EnhancedExporter {
    ...
    async streamProcess(inputPath, outputPath, options) {
        ...
        try {
            ...
            // Setup pipeline
            await pipeline(createReadStream(inputPath, { highWaterMark: 64 * 1024 }), // 64KB chunks
            transformStream, createWriteStream(tempPath));
            // Atomic rename
    ...
    transformRecord(sourceRecord, options) {
        const record = {
            ...
            DUPLICATION_RATE: sourceRecord.duplication?.max_similarity ||
    ...
    determineResult(record) {
        ...
        if (alerts.includes("pii_license"))
            return "FAIL";
        if (alerts.length > 2)
            return "PARTIAL";
        if (alerts.length > 0)
    ...
    countP1Violations(record) {
        const flags = record.alert_flags || [];
        return flags.filter((flag) => ["hallucination"].includes(flag))
            .length;
    }
    countP2Violations(record) {
        const flags = record.alert_flags || [];
        return flags.filter((flag) => ["duplication", "missing_evidence", "low_quality"].includes(flag)).length;
    }
    /**
    ...
    recordToCsv(record, includeHeader) {
        ...
        let result = "";
        if (includeHeader) {
            const headers = Object.keys(record).map((key) => key.includes(",") ? `"${key}"` : key);
            result += headers.join(",") + "\n";
        }
        result += values.join(",") + "\n";
        return result;
...
export function validateExportOptions(options) {
    const errors = [];
    if (!options.source)
        errors.push("--source is required (baseline|session)");
    if (!options.format)
        errors.push("--format is required (csv|json)");
    if (!options.runId)
