#!/usr/bin/env tsx
import { EnhancedExporter, parseExportArgs, validateExportOptions, } from "./export_enhanced.js";
async function main() {
    ...
    try {
        ...
        console.log(`   Output: ${options.outputDir}/${options.runId}/${options.source}.${options.format}`);
...
function showUsage() {
    ...
    console.log("  --output-dir <DIR>            Output directory (default: reports/export)");
    console.log("  --dry-run                     Show what would be done without writing files");
    console.log("  --skip-duplicates             Skip if output file already exists");
    console.log("  --include-links               Include hyperlinks to reports (default: true)");
    ...
    console.log("  reports/export/{RUN_ID}/baseline-v2.csv  # Versioned if file exists");
}
...
//# sourceMappingURL=export_enhanced_cli.js.map