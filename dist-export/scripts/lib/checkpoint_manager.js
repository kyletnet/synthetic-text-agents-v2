...
export class CheckpointManager {
     ...
     */
    createCheckpoint(runId, sessionId, progress, state, processedItems, checkpointType = "incremental") {
    ...
    createEmergencyCheckpoint(runId, sessionId, currentState, errorDetails) {
        ...
        return this.createCheckpoint(runId, sessionId, progress, emergencyState, processedItems, "emergency");
    }
    /**
    ...
    loadLatestCheckpoint(runId) {
        try {
            ...
            const latestCheckpoint = checkpoints.sort((a, b) => new Date(b.created_timestamp).getTime() -
                new Date(a.created_timestamp).getTime())[0];
    ...
    cleanupOldCheckpoints(runId) {
        try {
            ...
            const sortedCheckpoints = checkpoints.sort((a, b) => new Date(b.created_timestamp).getTime() -
    ...
    calculateDataIntegrityHash(processedItems, state) {
        ...
        const dataToHash = {
            ...
            last_item_hash: processedItems.length > 0
    ...
    verifyCheckpointIntegrity(checkpoint) {
        try {
            const expectedHash = this.calculateDataIntegrityHash(checkpoint.results.processed_items, checkpoint.state);
            const integrityMatch = expectedHash === checkpoint.recovery_info.data_integrity_hash;
            // Additional consistency checks
            const itemsCountMatch = checkpoint.results.processed_items.length ===
    ...
    generateMetricsSummary(processedItems) {
        ...
        return {
            ...
            success_rate: processedItems.length > 0
    ...
    generateErrorSummary(processedItems) {
        ...
        return {
            ...
            failure_rate: processedItems.length > 0
    ...
    extractDependencies(state) {
        ...
        if (state.seed_value)
            dependencies.push(`seed:${state.seed_value}`);
        if (state.threshold_config)
            dependencies.push("threshold_config");
        if (state.budget_state)
            dependencies.push("budget_state");
        return dependencies;
    }
    ...
    calculateProgressFromItems(processedItems, totalItems) {
        ...
        const lastIndex = processedItems.length > 0
    ...
    cleanStateForPartialRestart(state, stablePoint) {
        ...
        if (cleanedState.processed_items) {
            cleanedState.processed_items = cleanedState.processed_items.filter((item) => item.item_index <= stablePoint);
        }
        return cleanedState;
    }
...
export const CheckpointUtils = {
    ...
    withCheckpointing: async (operation, checkpointManager, context) => {
        try {
            ...
            try {
                checkpointManager.createEmergencyCheckpoint(context.runId, context.sessionId, {
