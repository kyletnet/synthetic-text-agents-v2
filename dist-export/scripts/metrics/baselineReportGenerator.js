...
import { createThresholdManager, } from "./thresholdManager.js";
import Ajv from "ajv";
 ...
 */
function loadHistoricalTrends(outputDir, limit = 10) {
    const trends = {};
 ...
 */
function loadDLQSummary(outputDir) {
 ...
 */
function convertToExportFormat(records, summary, _gating) {
    return records.map((record, _index) => {
        ...
        const p0Violations = record.alert_flags.filter((flag) => flag === "pii_license").length;
        const p1Violations = record.alert_flags.filter((flag) => ["hallucination"].includes(flag)).length;
        const p2Violations = record.alert_flags.filter((flag) => ["duplication", "missing_evidence", "low_quality"].includes(flag)).length;
        return {
 ...
 */
function validateExportData(records, schemaPath) {
    try {
 ...
 */
function writeExportFiles(records, outputDir, profile) {
 ...
 */
function validateSchema(record, schemaPath) {
    try {
        const schemaContent = readFileSync(schemaPath, "utf-8");
        const schema = JSON.parse(schemaContent);
        const ajv = new Ajv();
 ...
 */
function extractThresholdMetrics(summary, records) {
    return {
        ...
        coverage_rate: summary.coverage?.entity_coverage_rate ||
 ...
 */
function _generateGateMappingBanner(profile, autocalibrationEnabled) {
    const lines = [];
    lines.push("---");
    lines.push("");
 ...
 */
function _generateDLQSummary(dlqSummary) {
    const lines = [];
 ...
 */
function generateThresholdSummary(gating, calibrationResults, profile, manager, summary, previousSummary) {
    const lines = [];
    ...
    for (const metric of metricsToTrack) {
        ...
        const priorityLabel = metric.source === "FIXED"
 ...
 */
function generateJsonlReport(records, outputPath) {
 ...
 */
function generateMarkdownReport(summary, records, options, gating, calibrationResults, thresholdManager, _trends, _dlqSummary, _previousSummary) {
    const lines = [];
    // Header
        ...
        const alertSections = [
            {
                ...
                message: "High duplication rate detected - review for content repetition",
            },
            {
            ...
            },
            {
                ...
                message: "Potential hallucinations detected - validate against source material",
            },
            {
            ...
            },
            {
                ...
                message: "Question type imbalance detected - diversify question patterns",
            },
        ];
 ...
 */
export async function generateBaselineReports(qaItems, options = {}) {
    const { outputDir = "reports", sessionId = `baseline_${Date.now()}`, budgetLimit = 0, sourceTexts = [], includeFullData = true, profile = "dev", enableAutocalibration = false, applyCalibration = false, enableSchemaValidation = true, enableExport = true, trendHistoryLimit = 10, } = options;
    ...
    const markdownContent = generateMarkdownReport(summary, records, options, gating, calibrationResults, thresholdManager, trends, dlqSummary, previousSummary);
    ...
    const out = { jsonlPath, markdownPath, summary, hash };
    if (gating)
        out.gating = gating;
    if (calibrationResults)
        out.calibrationResults = calibrationResults;
    if (schemaValidationResults)
        out.schemaValidationResults = schemaValidationResults;
    if (exportPaths)
        out.exportPaths = exportPaths;
    if (exportValidation)
        out.exportValidation = exportValidation;
    return out;
}
//# sourceMappingURL=baselineReportGenerator.js.map