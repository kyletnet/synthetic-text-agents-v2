 ...
 */
function extractKeyPhrases(text, minLength, maxLength) {
    // Normalize text
 ...
 */
function extractSourceEntities(sourceTexts, config) {
    const allEntities = new Set();
    const frequency = {};
    for (const text of sourceTexts) {
        const phrases = extractKeyPhrases(text, config.entity_extraction.min_phrase_length, config.entity_extraction.max_phrase_length);
    ...
    const sortedEntities = Array.from(allEntities).sort((a, b) => (frequency[b] || 0) - (frequency[a] || 0));
    return {
 ...
 */
function analyzeEntityCoverage(qaItems, sourceTexts, config) {
 ...
 */
function createSections(sourceTexts) {
    const sections = [];
    for (const text of sourceTexts) {
 ...
 */
function isSectionCovered(section, qaItems, overlapThreshold) {
    const sectionWords = new Set(section
        .toLowerCase()
        .replace(/[^\w\s가-힣]/g, " ")
        .split(/\s+/)
        .filter((w) => w.length > 1));
    for (const item of qaItems) {
        if (item.evidence) {
            const evidenceWords = new Set(item.evidence
                .toLowerCase()
                .replace(/[^\w\s가-힣]/g, " ")
                .split(/\s+/)
                .filter((w) => w.length > 1));
            // Calculate overlap
            const intersection = new Set([...sectionWords].filter((w) => evidenceWords.has(w)));
            const overlap = intersection.size / Math.min(sectionWords.size, evidenceWords.size);
 ...
 */
function analyzeSectionCoverage(qaItems, sourceTexts, config) {
    const sections = createSections(sourceTexts);
    let coveredCount = 0;
    const uncoveredSections = [];
    const sectionHistogram = {};
    for (const section of sections) {
        const sectionKey = `Section_${section.substring(0, 20)}...`;
        sectionHistogram[sectionKey] = section.length;
        if (isSectionCovered(section, qaItems, config.section_mapping.section_overlap_threshold)) {
            coveredCount++;
        }
        else {
 ...
 */
export function calculateCoverageMetrics(qaItems, sourceTexts, configPath = "baseline_config.json") {
    // Load configuration
    const configText = readFileSync(configPath, "utf-8");
    const fullConfig = JSON.parse(configText);
    const config = fullConfig.coverage_metrics;
    ...
    const overallScore = entityCoverage.coverage_rate * 0.6 + sectionCoverage.coverage_rate * 0.4;
    // Identify critical gaps
    const criticalGaps = [];
    if (entityCoverage.coverage_rate <
        config.alert_thresholds.entity_coverage_rate_min) {
        criticalGaps.push(`Low entity coverage: ${(entityCoverage.coverage_rate * 100).toFixed(1)}%`);
    }
    if (sectionCoverage.coverage_rate <
        config.alert_thresholds.section_coverage_rate_min) {
        criticalGaps.push(`Low section coverage: ${(sectionCoverage.coverage_rate * 100).toFixed(1)}%`);
    }
    if (entityCoverage.missed_entities.length >
        config.alert_thresholds.uncovered_important_entities_max) {
        criticalGaps.push(`Too many uncovered entities: ${entityCoverage.missed_entities.length}`);
    }
 ...
 */
export function generateCoverageReport(metrics) {
    const lines = [];
    lines.push("## Coverage Analysis");
    lines.push("");
    // Summary metrics
    lines.push("### Overall Coverage Summary");
    lines.push(`- **Overall Score**: ${(metrics.coverage_summary.overall_score * 100).toFixed(1)}%`);
    lines.push(`- **Alert Status**: ${metrics.alert_triggered ? "⚠️ ISSUES DETECTED" : "✅ NORMAL"}`);
    lines.push("");
    // Entity coverage
    lines.push("### Entity Coverage");
    lines.push(`- **Total Entities**: ${metrics.entity_coverage.total_entities}`);
    lines.push(`- **Covered Entities**: ${metrics.entity_coverage.covered_entities}`);
    lines.push(`- **Coverage Rate**: ${(metrics.entity_coverage.coverage_rate * 100).toFixed(1)}%`);
    ...
    lines.push(`- **Total Sections**: ${metrics.section_coverage.total_sections}`);
    lines.push(`- **Covered Sections**: ${metrics.section_coverage.covered_sections}`);
    lines.push(`- **Coverage Rate**: ${(metrics.section_coverage.coverage_rate * 100).toFixed(1)}%`);
    lines.push("");
    // Critical gaps
...
//# sourceMappingURL=coverageMetrics.js.map