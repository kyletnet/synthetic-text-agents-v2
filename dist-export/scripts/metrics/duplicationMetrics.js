 ...
 */
function calculateMaxNgramOverlap(text1, text2, ngramRange) {
    let maxOverlap = 0;
 ...
 */
function findDuplicatePairs(qaItems, config) {
    const pairs = [];
    for (let i = 0; i < qaItems.length; i++) {
        for (let j = i + 1; j < qaItems.length; j++) {
            ...
            const jaccardSim = jaccardSimilarity(new Set(text1.toLowerCase().split(/\s+/)), new Set(text2.toLowerCase().split(/\s+/)));
 ...
 */
function calculateNgramDistributions(qaItems, ngramRange) {
    const distributions = {};
    for (const n of ngramRange) {
 ...
 */
async function mockLlmJudgeSemanticSimilarity(pairs) {
 ...
 */
export async function calculateDuplicationMetrics(qaItems, configPath = "baseline_config.json") {
    // Load configuration
    const configText = readFileSync(configPath, "utf-8");
    const fullConfig = JSON.parse(configText);
    const config = fullConfig.duplication_metrics;
...
//# sourceMappingURL=duplicationMetrics.js.map