 ...
 */
function _calculateMean(values) {
    if (values.length === 0)
        return 0;
 ...
 */
function calculateOverallQualityScore(duplicateRate, evidencePresenceRate, hallucinationRate, piiViolations, qtypeBalance, coverageScore) {
    ...
    const overallScore = weights.duplication * duplicationScore +
 ...
 */
function checkReproducibility(currentMetrics, tolerancePct = 5) {
    const deviations = {};
 ...
 */
export async function calculateAllBaselineMetrics(qaItems, options = {}) {
    const { configPath = "baseline_config.json", sessionId = `baseline_${Date.now()}`, budgetLimit = 0, sourceTexts = [], } = options;
    ...
    const records = qaItems.map((item, index) => {
        // Find if this item is flagged in various metrics
        const duplicateFlag = duplicationMetrics.top_duplicate_pairs.some((pair) => pair.index1 === index || pair.index2 === index);
        const hallucinationFlag = hallucinationMetrics.flags.find((flag) => flag.index === index);
        const piiLicenseViolations = piiLicenseMetrics.matches.filter((match) => match.location.index === index);
        // Calculate individual quality score
        const itemQualityScore = calculateOverallQualityScore(duplicateFlag ? 1 : 0, item.evidence ? 1 : 0, hallucinationFlag ? 1 : 0, piiLicenseViolations.length, qtypeMetrics.imbalance_score, coverageMetrics.coverage_summary.overall_score);
        const alertFlags = [];
        if (duplicateFlag)
            alertFlags.push("duplication");
        if (hallucinationFlag)
            alertFlags.push("hallucination");
        if (piiLicenseViolations.length > 0)
            alertFlags.push("pii_license");
        if (!item.evidence)
            alertFlags.push("missing_evidence");
        return {
    ...
    const overallQualityScore = calculateOverallQualityScore(duplicationMetrics.duplication_rate, evidenceMetrics.evidence_presence_rate, hallucinationMetrics.hallucination_rate, piiLicenseMetrics.pii_hits, qtypeMetrics.imbalance_score, coverageMetrics.coverage_summary.overall_score);
    // Create summary
    const summary = {
        timestamp: new Date().toISOString(),
        session_id: sessionId,
        ...
        qtype_distribution: {
            distributions: Object.fromEntries(Object.entries(qtypeMetrics.distributions).map(([key, value]) => [
                key,
        ...
        reproducibility_check: checkReproducibility({}), // Would pass actual metrics
        total_alerts: [
        ...
        recommendation_level: overallQualityScore > 0.8
            ? "green"
...
//# sourceMappingURL=__all__.js.map