...
async function main() {
    ...
    await prewriteSessionMeta({
        ...
        mode: process.env.BASELINE_MODE ??
    ...
    try {
        ...
        if (result.calibrationResults && result.calibrationResults.length > 0) {
            const appliedCount = result.calibrationResults.filter((c) => c.applied).length;
            console.log(`📈 Calibration: ${appliedCount}/${result.calibrationResults.length} changes applied`);
        }
        if (result.schemaValidationResults) {
            const validCount = result.schemaValidationResults.filter((r) => r.valid).length;
...
//# sourceMappingURL=baselineCli.js.map