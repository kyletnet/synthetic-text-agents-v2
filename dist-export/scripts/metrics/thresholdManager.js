...
export class ThresholdManager {
    ...
    getP1Thresholds(profile = "dev") {
        ...
        return {
            ...
            failure_rate_warn: profileOverrides.failure_rate_warn ?? p1.failure_rate_warn ?? 0.1,
     ...
     */
    getP2Thresholds(profile = "dev") {
        const dxloop = this.baselineConfig.dxloop || {};
        const p2 = dxloop.thresholds?.p2 || {};
        // Apply profile-specific overrides if any
        const profileOverrides = dxloop.profile_overrides?.[profile]?.p2 || {};
        return {
            duplication_rate_warn: profileOverrides.duplication_rate_warn ??
                p2.duplication_rate_warn ??
                0.1,
            duplication_rate_fail: profileOverrides.duplication_rate_fail ??
                p2.duplication_rate_fail ??
                0.2,
            coverage_rate_warn: profileOverrides.coverage_rate_warn ?? p2.coverage_rate_warn ?? 0.7,
            coverage_rate_fail: profileOverrides.coverage_rate_fail ?? p2.coverage_rate_fail ?? 0.5,
            quality_score_warn: profileOverrides.quality_score_warn ?? p2.quality_score_warn ?? 0.7,
            quality_score_fail: profileOverrides.quality_score_fail ?? p2.quality_score_fail ?? 0.5,
        };
    }
    /**
     ...
     */
    async autoCalibrateThresholds(profile = "dev") {
        const autocalibConfig = this.getAutoCalibrationConfig();
     ...
     */
    calibrateP1Metrics(metrics, current, config) {
        const results = [];
        ...
        results.push(this.createCalibrationResult("cost_per_item_warn", costWarnThreshold, current.cost_per_item_warn, config.drift_guard_max_delta, config.percentile_warn));
        results.push(this.createCalibrationResult("cost_per_item_fail", costFailThreshold, current.cost_per_item_fail, config.drift_guard_max_delta, config.percentile_fail));
        ...
        results.push(this.createCalibrationResult("latency_p95_warn_ms", latencyWarnThreshold, current.latency_p95_warn_ms, config.drift_guard_max_delta, config.percentile_warn));
        results.push(this.createCalibrationResult("latency_p95_fail_ms", latencyFailThreshold, current.latency_p95_fail_ms, config.drift_guard_max_delta, config.percentile_fail));
        ...
        results.push(this.createCalibrationResult("failure_rate_warn", failureWarnThreshold, current.failure_rate_warn, config.drift_guard_max_delta, config.percentile_warn));
        results.push(this.createCalibrationResult("failure_rate_fail", failureFailThreshold, current.failure_rate_fail, config.drift_guard_max_delta, config.percentile_fail));
        return results;
    }
    /**
     * Calibrate P2 thresholds
     */
    calibrateP2Metrics(metrics, current, config) {
        const results = [];
        ...
        results.push(this.createCalibrationResult("duplication_rate_warn", dupWarnThreshold, current.duplication_rate_warn, config.drift_guard_max_delta, config.percentile_warn));
        results.push(this.createCalibrationResult("duplication_rate_fail", dupFailThreshold, current.duplication_rate_fail, config.drift_guard_max_delta, config.percentile_fail));
        ...
        results.push(this.createCalibrationResult("coverage_rate_warn", coverageWarnThreshold, current.coverage_rate_warn, config.drift_guard_max_delta, 100 - config.percentile_warn));
        results.push(this.createCalibrationResult("coverage_rate_fail", coverageFailThreshold, current.coverage_rate_fail, config.drift_guard_max_delta, 100 - config.percentile_fail));
        ...
        results.push(this.createCalibrationResult("quality_score_warn", qualityWarnThreshold, current.quality_score_warn, config.drift_guard_max_delta, 100 - config.percentile_warn));
        results.push(this.createCalibrationResult("quality_score_fail", qualityFailThreshold, current.quality_score_fail, config.drift_guard_max_delta, 100 - config.percentile_fail));
        return results;
    }
    /**
     * Create calibration result with drift guard protection
     */
    createCalibrationResult(metricName, newValue, currentValue, maxDelta, percentileSource) {
     ...
     */
    applyCalibrationResults(results, profile = "dev") {
     ...
     */
    evaluateGating(currentMetrics, profile = "dev") {
        ...
        if (currentMetrics.evidence_missing_rate >
     ...
     */
    evaluateP1Thresholds(metrics, thresholds, violations, warnings) {
        // Cost per item
     ...
     */
    evaluateP2Thresholds(metrics, thresholds, violations, issues) {
    ...
    calculateCoverageRate(summary) {
        return (summary.coverage?.entity_coverage_rate ||
            summary.coverage?.section_coverage_rate ||
            0);
    }
    calculateEvidenceMissingRate(lines) {
        let totalItems = 0;
    ...
    calculateHallucinationRate(lines) {
        let totalItems = 0;
    ...
    calculateLicenseViolations(lines) {
     ...
     */
    async getCalibrationStatus(profile) {
 ...
 */
export function createThresholdManager(configPath) {
    return new ThresholdManager(configPath);
}
/**
 * Convenience function to get all thresholds for a profile
 */
export function getAllThresholds(profile = "dev", configPath) {
