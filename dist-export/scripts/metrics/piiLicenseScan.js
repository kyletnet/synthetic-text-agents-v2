 ...
 */
function extractContext(text, matchIndex, matchLength, contextLength = 50) {
 ...
 */
function scanForPii(text, fieldName, itemIndex) {
    const matches = [];
 ...
 */
function scanForLicense(text, fieldName, itemIndex) {
    const matches = [];
 ...
 */
export function scanPiiAndLicense(qaItems, configPath = "baseline_config.json") {
    // Load configuration
    const configText = readFileSync(configPath, "utf-8");
    const fullConfig = JSON.parse(configText);
    const config = fullConfig.pii_license_scan;
    const allMatches = [];
    const summary = {
        email_addresses: 0,
        phone_numbers: 0,
    ...
    for (let i = 0; i < qaItems.length; i++) {
        ...
        let evidencePiiMatches = [];
        let evidenceLicenseMatches = [];
    ...
    const alertTriggered = piiHits > config.alert_thresholds.pii_hits_max ||
 ...
 */
export function generatePiiLicenseReport(metrics) {
    const lines = [];
    ...
    if (metrics.matches.length > 0) {
        ...
        for (const match of metrics.matches.slice(0, 10)) {
            const riskIcon = match.risk_level === "high"
                ? "🔴"
...
//# sourceMappingURL=piiLicenseScan.js.map