...
export class PreflightRunner {
    ...
    budgetGuardian = createBudgetGuardian();
    dlqManager = createDLQManager();
    gatingIntegrator = new GatingIntegrator();
     ...
     */
    async initializeAndVerifySystems() {
    ...
    async runPaidSmoke() {
        ...
        const budgetState = this.budgetGuardian.initializeRun(`smoke_${this.timestamp}`, `session_${this.timestamp}`, this.config.profile, budgetLimits, 5);
        try {
    ...
    async runFullExecution() {
        ...
        this.budgetGuardian.initializeRun(`full_${this.timestamp}`, `session_${this.timestamp}`, this.config.profile, budgetLimits, 100);
        try {
            ...
            this.checkpointManager.createCheckpoint(`full_${this.timestamp}`, `session_${this.timestamp}`, {
            ...
            this.checkpointManager.createEmergencyCheckpoint(`full_${this.timestamp}`, `session_${this.timestamp}`, { error_state: true }, { error: error, context: "full_run_execution" });
            throw error;
        }
    }
    /**
     ...
     * Generate observability artifacts
     */
    async generateObservabilityArtifacts() {
     ...
     * Helper methods
     */
    async simulateQuickSmoke(dryRun) {
    ...
    async simulateFullRun(startIndex) {
    ...
    generateObservabilityHTML(data) {
        return `<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    createFailedResult(reason, timing, costs) {
        return {
            smoke_passed: false,
...
//# sourceMappingURL=preflight_integration.js.map