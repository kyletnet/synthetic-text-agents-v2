...
export class QAGenerator extends BaseAgent {
    ...
    async handle(content, context) {
        ...
        const dryRun = flag("DRY_RUN", false) ||
            (!process.env.OPENAI_API_KEY && !process.env.ANTHROPIC_API_KEY);
        // Pull promptSpec if exists
        const promptSpec = context?.sharedMemory?.["prompt-architect"]?.result
            ?.promptSpec || null;
        let result = [];
        let reasoning = "";
        let tokensUsed = 0;
...
- confidence는 0.0~1.0 사이 실수로 자기평가하세요. domain에는 주제를 채우세요.`;
    ...
    extractQAFromText(text, topic, count) {
...
//# sourceMappingURL=qaGenerator.js.map