...
export class LinguisticsEngineer extends BaseAgent {
    constructor(logger) {
        super("linguistics-engineer", "llm_optimization_language_structure", [
            "llm-optimization",
            "language-quality",
            "prompt-engineering",
    ...
    async optimizeForLLM(request, _context) {
    ...
    analyzeLLMCharacteristics(targetLLM) {
        ...
        return (characteristics[targetLLM] ||
            characteristics.generic);
    }
    async optimizePromptStructure(request, modelChar) {
        // Structure optimization based on model capabilities
        let optimalFormat = "hierarchical-structured";
        let sectionOrganization = [];
        let instructionClarity = [];
        let examplePlacement = "before";
    ...
    async optimizeTokenUsage(request, promptStructure) {
        ...
        const maxReasonableTokens = request.targetLLM === "claude"
            ? 2000
    ...
    async designOutputParsing(request, modelChar) {
    ...
    async analyzeLanguageQuality(request) {
    ...
    async assessConsistency(request) {
        let score = 8.0; // Base consistency score
        const terminologyAlignment = [];
        const styleCoherence = [];
        // Domain terminology consistency
        if (request.terminologyRequirements &&
    ...
    async assessPrecision(request) {
        let score = 7.5; // Base precision score
        const specificityEnhancements = [];
    ...
    async assessNaturalness(request) {
    ...
    async developTerminologyFramework(request) {
    ...
    async buildDomainVocabulary(domain, terminologyRequirements) {
    ...
    async createUsageGuidelines(request, domainVocabulary) {
    ...
    async establishConsistencyRules(domainVocabulary) {
        const preferredTerms = {};
        const synonymHandling = [];
    ...
    async generateStructuralRecommendations(request, llmOptimization) {
        const promptArchitecture = [];
        const informationHierarchy = [];
        const coherenceStrategies = [];
        const diversityMechanisms = [];
    ...
    async predictPerformance(request, llmOptimization, languageQuality) {
        ...
        const avgLanguageScore = (languageQuality.clarity.score +
    ...
    async assessConfidence(result) {
        if (typeof result === "object" && result !== null) {
            ...
            const avgLanguageQuality = (output.languageQuality.clarity.score +
    ...
    async explainReasoning(input, output, context) {
        if (typeof output === "object" && output !== null) {
            ...
            const _avgLangScore = (result.languageQuality.clarity.score +
...
//# sourceMappingURL=linguisticsEngineer.js.map