...
export class PsychologySpecialist extends BaseAgent {
    constructor(logger) {
        super("psychology-specialist", "user_psychology_communication_strategy", [
            "psychology",
            "user-behavior",
            "communication",
            "motivation",
    ...
    async conductPsychologyAnalysis(request, _context) {
    ...
    async analyzeUserPsychology(request) {
    ...
    determineEmotionalState(domain, contextualFactors) {
    ...
    identifyMotivations(domain, taskDescription) {
        const domainMotivations = {
            customer_service: [
        ...
        const hasUrgency = urgentWords.some((word) => taskDescription.toLowerCase().includes(word));
        if (hasUrgency) {
            baseMotivations.unshift("achieve_immediate_resolution");
        }
        return baseMotivations;
    }
    assessCognitiveLoad(taskDescription, experienceLevel) {
        let loadScore = 0;
        ...
        loadScore += complexityWords.filter((word) => taskDescription.toLowerCase().includes(word)).length;
        ...
        loadScore += technicalWords.filter((word) => taskDescription.toLowerCase().includes(word)).length;
    ...
    analyzeDecisionMakingStyle(domain, contextualFactors) {
        const domainStyles = {
            finance: "analytical",
            legal: "analytical",
    ...
    identifyStressFactors(domain, contextualFactors) {
        const domainStressors = {
            customer_service: [
                "time_pressure",
                "angry_customers",
                "complex_issues",
    ...
    assessConfidenceLevels(domain, experienceLevel) {
        const baseLevels = {
    ...
    async developCommunicationStrategy(request, userPsychology) {
    ...
    determineTone(userPsychology, domain) {
        // High stress or anxiety = supportive tone
        if (userPsychology.stressFactors.length > 3 ||
    ...
    selectLanguageStyle(userPsychology, experienceLevel) {
        if (experienceLevel === "beginner" ||
        ...
        if (userPsychology.emotionalState.includes("frustrated") ||
            userPsychology.emotionalState.includes("anxious")) {
            return "conversational";
        }
        return "formal";
    }
    selectStructuralApproach(userPsychology) {
    ...
    determineEmpathyLevel(userPsychology) {
    ...
    developUrgencyHandling(userPsychology, urgencyLevel) {
    ...
    async analyzePersuasionFactors(request, userPsychology) {
    ...
    identifyInfluencers(domain, userPsychology) {
        const domainInfluencers = {
            customer_service: [
                "empathy",
                "quick_resolution",
    ...
    identifyResistancePoints(userPsychology, request) {
    ...
    developMotivationalFrames(primaryMotivations) {
        const frameMap = {
        ...
        return primaryMotivations.map((motivation) => frameMap[motivation] || "Value and benefit realization");
    }
    identifyTrustBuilders(domain, userPsychology) {
        const domainTrustBuilders = {
            customer_service: [
                "active_listening",
                "problem_ownership",
                "follow_through",
        ...
        if (userPsychology.emotionalState.includes("anxious") ||
    ...
    async analyzeCognitiveFactors(request, userPsychology) {
    ...
    analyzeInformationProcessing(userPsychology) {
        if (userPsychology.cognitiveLoad === "high") {
        ...
        if (userPsychology.emotionalState.includes("urgent") ||
            userPsychology.stressFactors.includes("time_pressure")) {
    ...
    assessAttentionSpan(userPsychology, contextualFactors) {
        if (userPsychology.stressFactors.length > 3 ||
            userPsychology.cognitiveLoad === "high") {
            return "short";
        }
    ...
    determineLearningStyle(domain, userPsychology) {
        const domainStyles = {
    ...
    identifyMemoryAids(userPsychology, learningStyle) {
        const memoryAids = [];
    ...
    async generateRecommendations(analysis, _request) {
    ...
    generateQADesignPrinciples(analysis) {
        const principles = [];
    ...
    generateCommunicationGuidelines(analysis) {
        const guidelines = [];
        // Tone guidelines
    ...
    generateEmotionalConsiderations(analysis) {
    ...
    generateAvoidanceStrategies(analysis) {
        const strategies = [];
    ...
    generateEngagementTactics(analysis) {
        const tactics = [];
    ...
    generateQualityIndicators(analysis) {
        const indicators = [];
    ...
    async createImplementationGuidance(analysis, _request) {
    ...
    generateQuestionFormulationGuidance(analysis) {
        const guidance = [];
        if (analysis.userPsychology.cognitiveLoad === "high") {
    ...
    generateAnswerToneGuidance(analysis) {
        const guidance = [];
    ...
    generateEmpathyIntegration(analysis) {
        const integration = [];
        if (analysis.communicationStrategy.empathyLevel === "high") {
    ...
    generateMotivationalElements(analysis) {
        const elements = [];
    ...
    async identifyRiskFactors(analysis, request) {
        const riskFactors = [];
...
//# sourceMappingURL=psychologySpecialist.js.map