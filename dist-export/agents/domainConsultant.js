...
export class DomainConsultant extends BaseAgent {
  ...
  async handle(content, context) {
    ...
    const domainExpertise = await this.assembleDomainExpertise(
      request,
      context,
    );
    const processFrameworks = await this.developProcessFrameworks(
      request,
      domainExpertise,
    );
  ...
  async assembleDomainExpertise(request, _context) {
    const USE_LLM =
      String(
        process.env.FEATURE_LLM_DOMAIN_CONSULTANT || "false",
      ).toLowerCase() === "true";
    if (USE_LLM) {
    ...
    const domainData =
      this.domainKnowledgeBase.get(request.domain) ||
      this.getGenericDomainData();
    const industryContext = await this.buildIndustryContext(
      request,
      domainData,
    );
    const professionalStandards = await this.compileProfessionalStandards(
      request,
      domainData,
    );
    const practicalKnowledge = await this.gatherPracticalKnowledge(
      request,
      domainData,
    );
  ...
  async buildIndustryContext(request, domainData) {
  ...
  analyzeCompetitiveLandscape(request) {
    const landscape = [];
    ...
    return landscape.length > 0
      ? landscape
      : ["moderate competition", "established practices", "gradual evolution"];
  }
  async compileProfessionalStandards(request, domainData) {
  ...
  identifyIndustryStandards(domain) {
    const standards = {
      customer_service: [
        "ISO 9001 quality management",
        "COPC customer operations standards",
        "HDI support center standards",
      ],
      sales: [
  ...
  gatherRegulatoryRequirements(request) {
    const requirements = [];
    if (request.contextualFactors.regulatoryEnvironment === "strict") {
    ...
    requirements.push(
      ...(domainRequirements[request.domain] || [
  ...
  establishEthicalGuidelines(domain) {
    const guidelines = {
      customer_service: [
  ...
  defineQualityBenchmarks(request) {
    const benchmarks = [];
  ...
  async gatherPracticalKnowledge(request, domainData) {
  ...
  identifyTypicalChallenges(request) {
    const challenges = [];
    ...
    challenges.push(
      ...(domainChallenges[request.domain] || [
        "operational efficiency",
        "quality maintenance",
  ...
  compileSolutionPatterns(request) {
    const patterns = [];
  ...
  async deriveExpertInsights(request, _domainData) {
  ...
  compileAdvancedTechniques(request) {
    const techniques = [];
    if (
      request.expertiseLevel === "expert" ||
      request.expertiseLevel === "specialist"
  ...
  suggestInnovativeApproaches(request) {
    const approaches = [];
    if (request.qualityTarget >= 9) {
  ...
  async developProcessFrameworks(request, domainExpertise) {
    const decisionFrameworks = this.createDecisionFrameworks(
      request,
      domainExpertise,
    );
    const methodologies = this.defineMethodologies(request, domainExpertise);
    const evaluationCriteria = this.establishEvaluationCriteria(
      request,
      domainExpertise,
    );
    return {
  ...
  createDecisionFrameworks(request, _domainExpertise) {
    const frameworks = [];
  ...
  defineMethodologies(_request, _domainExpertise) {
    const methodologies = [];
    // Generic methodology
    methodologies.push({
  ...
  establishEvaluationCriteria(request, domainExpertise) {
    const criteria = [];
    criteria.push({
      category: "Quality",
  ...
  async createQAGuidance(request, domainExpertise) {
    ...
    const contextualConsiderations = this.identifyContextualConsiderations(
      request,
      domainExpertise,
    );
    return {
  ...
  defineQuestionTypes(_request, _domainExpertise) {
    const questionTypes = [];
    questionTypes.push({
      category: "Procedural",
      characteristics: [
    ...
    questionTypes.push({
      ...
      examples: [
        "What approach should we take for...",
        "How should we position...",
        "What strategy works best when...",
      ],
      appropriateContexts: [
  ...
  createAnswerPatterns(_request, domainExpertise) {
    const patterns = [];
    patterns.push({
      scenario: "Complex professional decision",
      structure:
        "Context → Analysis → Options → Recommendation → Implementation",
      keyElements: [
      ...
      expertiseMarkers:
        domainExpertise.professionalStandards.bestPractices.slice(0, 3),
    });
    patterns.push({
      scenario: "Technical implementation",
      structure:
        "Overview → Prerequisites → Step-by-step process → Quality checks",
      keyElements: [
        "clear prerequisites",
        "detailed steps",
        "quality validation",
      ...
      expertiseMarkers:
        domainExpertise.practicalKnowledge.toolsAndMethods.slice(0, 3),
    });
    return patterns;
  }
  identifyContextualConsiderations(request, domainExpertise) {
    return {
  ...
  async generateImplementationRecommendations(_request, _domainExpertise) {
    return {
  ...
  async conductRiskAssessment(request, _domainExpertise) {
    const risks = [];
    ...
    if (
      request.expertiseLevel === "professional" &&
      request.qualityTarget >= 9
    ) {
      risks.push({
  ...
  async assessConfidence(result) {
    if (typeof result === "object" && result !== null) {
      ...
      const expertiseBonus =
        output.domainExpertise.expertInsights.advancedTechniques.length > 5
          ? 0.05
          : 0;
      const riskPenalty =
        output.riskAssessment.filter((r) => r.likelihood === "high").length *
        0.05;
  ...
  async explainReasoning(input, output, context) {
    if (typeof output === "object" && output !== null) {
      const result = output;
      const domain = JSON.stringify(input).includes("domain")
        ? "specified domain"
        : "general";
      const expertiseItems =
        result.domainExpertise.practicalKnowledge.commonScenarios.length;
      const frameworksCount =
        result.processFrameworks.decisionFrameworks.length;
...
//# sourceMappingURL=domainConsultant.js.map
