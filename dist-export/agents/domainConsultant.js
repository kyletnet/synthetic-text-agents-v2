...
export class DomainConsultant extends BaseAgent {
    domainKnowledgeBase;
    constructor(logger) {
        super("domain-consultant", "domain_expertise_professional_knowledge", [
            "domain-expertise",
            "industry-knowledge",
            "best-practices",
    ...
    async assembleDomainExpertise(request, _context) {
        ...
        const domainData = this.domainKnowledgeBase.get(request.domain) ||
    ...
    gatherRegulatoryRequirements(request) {
        ...
        requirements.push(...(domainRequirements[request.domain] || [
    ...
    identifyTypicalChallenges(request) {
        ...
        challenges.push(...(domainChallenges[request.domain] || [
            "operational efficiency",
            "quality maintenance",
    ...
    compileSolutionPatterns(request) {
        ...
        patterns.push(...(domainPatterns[request.domain] || [
    ...
    async conductRiskAssessment(request, _domainExpertise) {
        ...
        if (request.expertiseLevel === "professional" &&
    ...
    async assessConfidence(result) {
        if (typeof result === "object" && result !== null) {
            ...
            const domainKnown = knownDomains.some((domain) => JSON.stringify(result).toLowerCase().includes(domain.replace("_", " ")));
            const baseConfidence = domainKnown ? 0.85 : 0.75;
            const expertiseBonus = output.domainExpertise.expertInsights.advancedTechniques.length > 5
                ? 0.05
                : 0;
            const riskPenalty = output.riskAssessment.filter((r) => r.likelihood === "high").length *
                0.05;
