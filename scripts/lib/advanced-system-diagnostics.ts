...
export interface SystemIssue {
  id: string;
  category: 'typescript' | 'imports' | 'methods' | 'runtime' | 'node' | 'triggers' | 'performance';
...
export class AdvancedSystemDiagnostics {
      ...
      if (errorOutput.includes('error TS')) {
        ...
        errors.forEach((err, index) => {
          this.issues.push({
            id: `ts-${index + 1}`,
            category: 'typescript',
            severity: err.code.includes('2345') || err.code.includes('2339') ? 'critical' : 'high',
    ...
    for (const file of tsFiles) {
      ...
      for (const imp of imports) {
        ...
        if (targetFile && existsSync(targetFile)) {
          ...
          if (imp.type === 'default' && !targetExports.some(exp => exp.type === 'default')) {
            this.issues.push({
  ...
  private async predictRuntimeErrors(): Promise<void> {
    ...
    const contextualTriggerFile = join(this.projectRoot, 'scripts/lib/contextual-trigger-system.ts');
      ...
      globPatterns.forEach((pattern) => {
        if (pattern.includes('src/**') || pattern.includes('scripts/**')) {
          this.issues.push({
            ...
            suggestedFix: 'scripts/ 디렉토리 감시로 패턴 수정 또는 존재하는 디렉토리로 변경',
            autoFixable: true,
            evidence: [pattern],
    ...
    for (const file of tsFiles) {
      ...
      asyncFunctions.forEach((func) => {
        const promiseCalls = func.match(/(?<!await\s+)\w+\.[a-zA-Z]+\s*\([^)]*\)\s*(?!\.)/g) || [];
        promiseCalls.forEach((call) => {
          if (call.includes('execSync') || call.includes('readFileSync')) return; // sync 함수는 제외
    ...
    try {
      ...
      if (existsSync(packageJson)) {
        ...
        if (pkg.type !== 'module') {
          ...
          for (const file of tsFiles.slice(0, 5)) { // 샘플링
            const content = readFileSync(join(this.projectRoot, file), 'utf8');
            if (content.includes('import.meta') || content.includes('import(')) {
              this.issues.push({
                id: 'esm-mismatch',
                category: 'node',
                severity: 'high',
                title: 'ESM/CommonJS 혼재 문제',
                description: 'package.json에 "type": "module"이 없으나 ESM 구문 사용 중',
  ...
  private async checkTriggerSystemHealth(): Promise<void> {
    const triggerSystemFile = join(this.projectRoot, 'scripts/lib/contextual-trigger-system.ts');
    ...
    // 트리거 설정 파일 검사
    const triggerConfigPath = join(this.projectRoot, '.contextual-triggers.json');
  ...
  private async detectHeavySyncOperations(): Promise<void> {
    ...
    for (const file of tsFiles) {
      ...
      // 동기 파일 작업이 많은 파일 감지
      const syncOperations = (content.match(/readFileSync|writeFileSync|execSync/g) || []).length;
  ...
  private generateTypeScriptFix(error: any): string {
    ...
    if (error.code === 'TS2339') return 'Property 존재 여부 확인 또는 타입 정의 추가';
  ...
  private resolveImportPath(importPath: string, currentFile: string): string | null {
    if (importPath.startsWith('./') || importPath.startsWith('../')) {
      ...
      if (existsSync(resolvedPath + '/index.ts')) return resolvedPath + '/index.ts';
    }
  ...
  private generateDiagnosticResult(): DiagnosticResult {
    ...
    else if (this.issues.filter(i => i.severity === 'high').length > 0) systemHealth = 'degraded';
  ...
  private generateRecommendations(): string[] {
    ...
    if (this.issues.some(i => i.category === 'typescript' && i.severity === 'critical')) {
