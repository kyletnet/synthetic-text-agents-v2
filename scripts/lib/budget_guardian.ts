...
export class BudgetGuardian {
    ...
    try {
      ...
      if (existsSync(this.killSwitchFile)) {
        ...
        return {
          ...
          trigger_reason: killSwitch.trigger_reason || 'Unknown',
          trigger_timestamp: killSwitch.trigger_timestamp,
   ...
   */
  completeRun(finalStatus: 'completed' | 'killed' | 'budget_exceeded' = 'completed'): RunBudgetState | null {
    if (!this.currentState) {
      return null;
    }
