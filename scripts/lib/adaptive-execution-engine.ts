...
import { smartDecisionMatrix, ExecutionContext, DecisionResult } from './smart-decision-matrix.js';
...
export interface AdaptiveOperation {
  ...
  name: string;
  command: string;
  type: 'typecheck' | 'lint' | 'test' | 'audit' | 'evolution' | 'build' | 'analysis';
  priority: 'P0' | 'P1' | 'P2';
...
export class AdaptiveExecutionEngine extends EventEmitter {
  ...
  async execute(
    ...
    const decision = await smartDecisionMatrix.makeDecision(operation.name, fullContext);

    console.log(`🎯 Adaptive execution for ${operation.name}:`);
    console.log(`   Strategy: ${decision.execution}`);
    console.log(`   Reasoning: ${decision.reasoning}`);
    console.log(`   Trade-offs: P=${Math.round(decision.tradeoffs.performance*100)}% S=${Math.round(decision.tradeoffs.safety*100)}% U=${Math.round(decision.tradeoffs.usability*100)}%`);
    ...
    try {
      ...
      await safeGuard.recordAttempt(operation.command, false, duration, errorResult.error);
  ...
  async executeBatch(
    ...
    console.log(`   Estimated duration: ${Math.round(executionPlan.estimatedDuration / 1000)}s`);
      ...
      switch (executionPlan.strategy) {
        case 'sequential':
          for (const operation of executionPlan.sequence) {
            ...
            const result = await this.execute(operation, fullContext, progressCallback);
            results.push(result);
            completed++;
          }
          break;
        ...
        case 'parallel':
          const promises = executionPlan.sequence.map(async (operation, index) => {
            const result = await this.execute(operation, fullContext, (progress) => {
              progressCallback?.({
                ...progress,
                operationId: 'batch',
        ...
        case 'hybrid':
          ...
          const criticalOps = executionPlan.sequence.filter(op => op.priority === 'P0');
          ...
          const remainingOps = executionPlan.sequence.filter(op => op.priority !== 'P0');
  ...
  getStatus(): {
    ...
    if (this.activeOperations.size > 5) {
      recommendations.push('High number of active operations - consider queuing');
    }

    if (this.systemMetrics.queueLength > 10) {
      recommendations.push('Queue backup detected - consider increasing parallelism');
    }

    const recentRecommendations = smartDecisionMatrix.getOptimizationRecommendations();
  ...
  private async executeWithStrategy(
    ...
    if (decision.execution !== 'user-guided' && operation.type !== 'evolution') {
    ...
    switch (decision.execution) {
      case 'immediate':
        result = await this.executeImmediate(operation, decision, progressCallback);
        break;

      case 'optimized':
        result = await this.executeOptimized(operation, decision, progressCallback);
        break;

      case 'safe-mode':
        result = await this.executeSafeMode(operation, decision, progressCallback);
        break;

      case 'user-guided':
        result = await this.executeUserGuided(operation, decision, progressCallback);
    ...
    while (attempts < decision.configuration.retries + 1) {
      ...
      try {
        ...
        return {
          ...
          strategy: 'optimized',
          output: output.trim(),
          userInteraction: { approvalsRequested: 0, progressUpdates: attempts + 1 }
        };
  ...
  private async executeUserGuided(
    ...
    try {
      ...
      return {
        ...
        userInteraction: { approvalsRequested: 1, progressUpdates: 0, userSatisfaction: 0.9 }
      };
      ...
      return {
        ...
        strategy: 'user-guided',
        error: error instanceof Error ? error.message : String(error),
        userInteraction: { approvalsRequested: 1, progressUpdates: 0, userSatisfaction: 0.3 }
      };
    }
  }
  ...
  private async executeDeferred(
    ...
    return new Promise((resolve, reject) => {
      this.executionQueue.push({
        operation,
        context: { // Default context for deferred operations
          priority: operation.priority,
          userPresent: false,
          systemLoad: 'low',
  ...
  private getCurrentSystemLoad(): 'low' | 'medium' | 'high' {
    if (this.activeOperations.size > 5 || this.systemMetrics.cpuLoad > 80) {
      return 'high';
    } else if (this.activeOperations.size > 2 || this.systemMetrics.cpuLoad > 50) {
      return 'medium';
    }
    return 'low';
  }

  private getRiskLevel(operation: AdaptiveOperation): 'low' | 'medium' | 'high' | 'critical' {
    switch (operation.type) {
      case 'evolution': return 'critical';
      case 'audit': return 'medium';
      case 'build': return 'medium';
      case 'typecheck': return 'low';
      case 'lint': return 'low';
  ...
  private getCacheTTL(type: string): number {
    switch (type) {
      case 'typecheck': return 2 * 60 * 1000; // 2 minutes
      case 'lint': return 5 * 60 * 1000; // 5 minutes
      case 'test': return 10 * 60 * 1000; // 10 minutes
  ...
  private startSystemMonitoring(): void {
    setInterval(() => {
      ...
      this.systemMetrics.memoryUsage = process.memoryUsage().heapUsed / 1024 / 1024;
  ...
  private startQueueProcessor(): void {
    setInterval(async () => {
      if (this.executionQueue.length === 0 || this.getCurrentSystemLoad() === 'high') {
        return;
      }
      ...
      try {
        const result = await this.execute(queuedItem.operation, queuedItem.context);
        queuedItem.resolve(result);
      } catch (error) {
        queuedItem.reject(error instanceof Error ? error : new Error(String(error)));
      }
