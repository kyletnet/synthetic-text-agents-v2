...
class SimplifiedApprovalSystem {
   ...
   */
  async requestApproval(request: SimplifiedApprovalRequest, safeMode: boolean = false): Promise<ApprovalResult> {
    ...
    console.log(`🎯 위험도: ${this.getRiskEmoji(analysis.riskLevel)} ${analysis.riskLevel.toUpperCase()}`);
    ...
    if (safeMode) {
      ...
      console.log(`✅ 자동 승인: ${analysis.matchedCriteria?.description || '안전한 변경사항'}`);
  ...
  private async getUserDecision(
    ...
    return new Promise((resolve) => {
      ...
      if (timeoutMs > 0) {
        console.log(`⏱️ ${timeoutMs / 1000}초 후 자동으로 건너뛰기 처리됩니다.`);
      } else {
   ...
   * 상세 정보 표시
   */
  private showDetailedInfo(request: SimplifiedApprovalRequest, analysis: any): void {
    console.log('\n📋 상세 분석 정보:');
    ...
    if (analysis.matchedCriteria) {
      ...
      console.log(`🎯 위험도: ${this.getRiskEmoji(criteria.riskLevel)} ${criteria.riskLevel}`);
   ...
   * 배치 처리 (여러 항목을 한번에)
   */
  async processBatch(requests: SimplifiedApprovalRequest[], safeMode: boolean = false): Promise<{
   ...
   * 중요도별 타임아웃 (GPT 조언 반영)
   */
  private getTimeoutForRisk(riskLevel: string, safeMode: boolean = false): number {
   ...
   * 위험도를 우선순위로 변환
   */
  private riskToPriority(riskLevel: string): 'low' | 'medium' | 'high' | 'critical' {
  ...
  async handleRejectionRollback(
    ...
    if (result.matchedCriteria?.rollbackDescription) {
      console.log(`💡 롤백 이유: ${result.matchedCriteria.rollbackDescription}`);
    }
    ...
    try {
      switch (result.rollbackStrategy) {
        ...
        case 'graceful_abort':
          ...
          console.log('🔒 데이터 무결성 보호를 위해 전체 메인테넌스를 중단합니다.');
