...
class ApprovalQueueHandler {
  ...
  async processQueue(): Promise<void> {
    ...
    if (stats.oldestItem) {
      const daysSince = Math.floor((Date.now() - stats.oldestItem.getTime()) / (1000 * 60 * 60 * 24));
  ...
  async showQueueStatus(): Promise<void> {
    ...
    pendingItems.slice(0, 10).forEach((item, idx) => {
      ...
      console.log(`   ${idx + 1}. ${priority} ${item.title} (${item.attempts}회 시도)`);
    });
  ...
  async clearQueue(): Promise<void> {
    ...
    console.log(`⚠️ ${stats.total}개의 승인 대기 항목을 모두 삭제하시겠습니까?`);
    console.log('   (y/N): ');
