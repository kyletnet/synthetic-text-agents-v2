...
export function validateSeverity(value: string): CanonicalSeverity {
  if (!isCanonicalSeverity(value)) {
    throw new Error(`Invalid severity '${value}'. Must be one of: ${CANONICAL_SEVERITIES.join(', ')}`);
  }
  return value;
}
...
export function validateStage(value: string): CanonicalStage {
  if (!isCanonicalStage(value)) {
    throw new Error(`Invalid stage '${value}'. Must be one of: ${CANONICAL_STAGES.join(', ')}`);
  }
  return value;
}
