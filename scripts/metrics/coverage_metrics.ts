 ...
 */
function extractKeyPhrases(text: string, minLength: number, maxLength: number): string[] {
  // Normalize text
  ...
  for (let n = minLength; n <= maxLength; n++) {
    for (let i = 0; i <= words.length - n; i++) {
      ...
      if (phrase.length > 3) { // Minimum phrase length
        phrases.push(phrase);
      }
    }
  }
...
function extractSourceEntities(
  ...
  const sortedEntities = Array.from(allEntities)
...
function isEntityCovered(entity: string, qaItems: QAItem[]): boolean {
  ...
  for (const item of qaItems) {
    ...
    if (questionText.includes(entityLower) || answerText.includes(entityLower)) {
      return true;
    }
  }
...
export function calculateCoverageMetrics(
  ...
  if (entityCoverage.coverage_rate < config.alert_thresholds.entity_coverage_rate_min) {
    criticalGaps.push(`Low entity coverage: ${(entityCoverage.coverage_rate * 100).toFixed(1)}%`);
  }
  if (sectionCoverage.coverage_rate < config.alert_thresholds.section_coverage_rate_min) {
    criticalGaps.push(`Low section coverage: ${(sectionCoverage.coverage_rate * 100).toFixed(1)}%`);
  }
  if (entityCoverage.missed_entities.length > config.alert_thresholds.uncovered_important_entities_max) {
