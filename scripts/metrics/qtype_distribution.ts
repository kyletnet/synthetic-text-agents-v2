 ...
 */
function classifyQuestion(question: string, patterns: Record<string, string[]>): string | null {
  ...
  if (normalizedQuestion.includes('비교') || normalizedQuestion.includes('차이') ||
      normalizedQuestion.includes('다른') || normalizedQuestion.includes('같은')) {
    return 'comparison';
  }
  if (normalizedQuestion.includes('생각') || normalizedQuestion.includes('판단') ||
      normalizedQuestion.includes('결론') || normalizedQuestion.includes('추론')) {
    return 'inference';
  }
...
export function analyzeQuestionTypeDistribution(
  ...
  const distributions: Record<string, { count: number; ratio: number; examples: string[] }> = {};
  ...
  const imbalanceScore = calculateImbalanceScore(rawCounts, config.target_distribution, qaItems.length);
  const missingCategories = findMissingCategories(rawCounts, config.target_distribution);
  ...
  const unclassifiedRatio = qaItems.length > 0 ? unclassifiedCount / qaItems.length : 0;
...
export function generateQtypeReport(distribution: QtypeDistribution): string {
  ...
  for (const [qtype, data] of Object.entries(distribution.distributions)) {
    ...
    lines.push(`| ${qtype} | ${data.count} | ${(data.ratio * 100).toFixed(1)}% | ${examples} |`);
  }

  if (distribution.unclassified_count > 0) {
    lines.push(`| unclassified | ${distribution.unclassified_count} | ${(distribution.unclassified_ratio * 100).toFixed(1)}% | - |`);
  }
  ...
  lines.push(`- **Imbalance Score**: ${distribution.imbalance_score.toFixed(3)}`);
  lines.push(`- **Entropy**: ${distribution.entropy.toFixed(3)}`);
  lines.push(`- **Missing Categories**: ${distribution.missing_categories.length > 0 ? distribution.missing_categories.join(', ') : 'None'}`);
  lines.push(`- **Alert Status**: ${distribution.alert_triggered ? '⚠️ TRIGGERED' : '✅ NORMAL'}`);
...
if (require.main === module) {
  ...
  const sampleQA: QAItem[] = [
    ...
    { qa: { q: "왜 물이 얼까요?", a: "온도가 0도 이하로 내려가기 때문입니다." }, index: 1 },
    { qa: { q: "어떻게 물이 수증기가 되나요?", a: "가열하면 증발합니다." }, index: 2 },
    { qa: { q: "언제 얼음이 녹나요?", a: "온도가 0도 이상일 때 녹습니다." }, index: 3 },
    { qa: { q: "식물과 동물의 차이는 무엇인가요?", a: "식물은 광합성을 하고 동물은 호흡을 합니다." }, index: 4 }
  ];
