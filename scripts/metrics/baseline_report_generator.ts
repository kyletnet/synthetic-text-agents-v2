...
import { createThresholdManager, ThresholdManager, GatingResult, CalibrationResult } from './threshold_manager.js';
import Ajv from 'ajv';
...
function generateMarkdownReport(
  ...
  if (gating && calibrationResults && thresholdManager && options.profile) {
    const thresholdLines = generateThresholdSummary(gating, calibrationResults, options.profile, thresholdManager, summary, options as any);
  ...
  for (const [qtype, data] of Object.entries(summary.qtype_distribution.distributions)) {
    lines.push(`| ${qtype} | ${(data as any).count} | ${((data as any).ratio * 100).toFixed(1)}% |`);
  }
  lines.push('');
  ...
  for (const [level, count] of Object.entries(summary.hallucination.risk_distribution)) {
    const percentage = summary.total_items > 0 ? ((count as any) / summary.total_items * 100).toFixed(1) : '0.0';
  ...
  if (Object.keys(summary.reproducibility_check.deviations).length > 0) {
    ...
    for (const [metric, deviation] of Object.entries(summary.reproducibility_check.deviations)) {
      lines.push(`  - ${metric}: ${(deviation as any).toFixed(1)}%`);
    }
  }
  lines.push('');
...
export async function generateBaselineReports(
  ...
  // After data load:
  const casesTotal = Array.isArray(qaItems) ? qaItems.length : 0;
  try {
