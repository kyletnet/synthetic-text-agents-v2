...
class WorkflowPreventionSystem {
    ...
    this.preventionRules = [
      {
        ...
        fixSuggestion: 'Add missing steps to /sync workflow in scripts/slash-commands.sh',
        severity: 'CRITICAL'
      },
      {
        id: 'WF002',
  ...
  async runPreventionCheck(): Promise<PreventionReport> {
    ...
    const preventionScore = Math.max(0, 100 - (criticalViolations * 30 + highViolations * 20 + violations.length * 5));

    const overallStatus = criticalViolations > 0 ? 'FAIL' :
                         highViolations > 0 ? 'WARN' : 'PASS';
  ...
  private checkSecurityIntegration(): boolean {
    try {
      ...
      return slashCommandsContent.includes('security:audit:check') &&
  ...
  private checkFailureRecovery(): boolean {
    try {
      ...
      return slashCommandsContent.includes('sync:tx:rollback') &&
  ...
  private checkAuditTrail(): boolean {
    try {
      ...
      return existsSync(join(this.projectRoot, 'scripts/sync-health-reporter.ts')) &&
             existsSync(join(this.projectRoot, 'scripts/workflow-completeness-auditor.ts'));
    } catch (error) {
      return false;
    }
  }
  ...
  private printReport(report: PreventionReport): void {
    ...
    if (report.violations.length > 0) {
      ...
      report.violations.forEach((violation, i) => {
        ...
        console.log(`   ${i + 1}. ${icon} [${violation.ruleId}] ${violation.ruleName}`);
      ...
      if (criticalCount > 0) {
        console.log(`   1. 🚨 Fix ${criticalCount} CRITICAL violations immediately`);
      }
      if (highCount > 0) {
        console.log(`   ${criticalCount > 0 ? '2' : '1'}. 🔴 Address ${highCount} HIGH priority violations`);
      }
      console.log(`   ${criticalCount + highCount > 0 ? '3' : '1'}. 📋 Review remaining ${report.violations.length - criticalCount - highCount} violations`);
    } else {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    default:
      console.log('Usage: tsx workflow-prevention-system.ts <check|install-hooks>');
      process.exit(1);
  }
}
