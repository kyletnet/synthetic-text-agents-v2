...
class DryRunApprovalQueue {
  ...
  async queueDryRun(operation: AutoFixOperation): Promise<string> {
    ...
    const approvalScore = await this.calculateApprovalScore(
      operation,
      dryRunResult,
    );
    ...
    const approvalResult = await this.evaluateForAutoApproval(
      queuedItem,
      criteria,
    );
  ...
  private async calculateApprovalScore(
    operation: AutoFixOperation,
    dryRunResult: DryRunResult,
  ): Promise<number> {
    ...
    const estimatedLines = dryRunResult.previewDiff
      .split("\n")
  ...
  private async evaluateForAutoApproval(
    item: QueuedDryRun,
    criteria: ApprovalCriteria,
  ): Promise<{
    ...
    if (
      riskLevels[dryRunResult.impact.riskLevel] >
      riskLevels[criteria.maxRiskLevel]
    ) {
      return {
        status: "pending",
    ...
    if (autoApprovalScore >= 80 && allFilesAutoApprovable) {
      ...
      autoApprovalScore >= 70 &&
      dryRunResult.impact.riskLevel === "low"
    ) {
      return {
        status: "auto-approved",
  ...
  async executeAutoApprovedItems(): Promise<void> {
    ...
    const queue = await this.loadQueue();
    const autoApproved = queue.queue.filter(
      (item) => item.status === "auto-approved",
    );
  ...
  async displayQueueStatus(): Promise<void> {
    ...
    if (activeItems.length > 0) {
      ...
      activeItems.forEach((item, index) => {
        ...
        const ageHours =
          (Date.now() - new Date(item.queuedAt).getTime()) / (1000 * 60 * 60);
    ...
    console.log("\n💻 **Commands**:");
    console.log(
      "   npm run queue:execute              # Execute auto-approved items",
    );
    console.log(
      "   npm run queue:approve <id>         # Manually approve item",
    );
