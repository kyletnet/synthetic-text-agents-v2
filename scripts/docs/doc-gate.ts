...
const QUALITY_GATES: GateRule[] = [
  ...
  // 신선도 게이트
  {
    ...
    severity: 'blocking',
    checker: (auditResults) => {
      const staleDocs = auditResults.freshness?.filter((f: any) => f.warningLevel === 'critical') || [];
      return {
        passed: staleDocs.length === 0,
        score: staleDocs.length,
        message: `Critical stale docs: ${staleDocs.length}`,
        details: staleDocs.map((doc: any) => `${doc.docPath}: ${doc.stalenessInDays} days stale`)
      };
    }
  },
...
class DocumentGate {
  ...
  async runQualityGate(): Promise<GateReport> {
    ...
    // 먼저 전체 문서 감사 실행
    const auditor = new (await import('./doc-quality-auditor.js')).DocQualityAuditor(this.projectRoot);
  ...
  private async getAuditResults(): Promise<any> {
    ...
    const auditReportPath = join(this.projectRoot, 'reports/doc-audit-report.json');
  ...
  private printGateReport(report: GateReport): void {
    ...
    console.log(`📈 Rules passed: ${report.summary.passed}/${report.summary.totalRules}`);
    ...
    for (const rule of report.rules) {
      ...
      console.log(`   ${icon} ${rule.name}: ${rule.result.message}${blockText}`);
    ...
    if (failedRules.length > 0) {
      ...
      for (const rule of failedRules) {
        if (rule.name === 'coverage-threshold') {
          ...
          console.log('   📅 Update stale documentation to reflect recent code changes');
        } else if (rule.name === 'structure-violations') {
          console.log('   🏗️ Fix document structure violations using npm run docs:lint');
  ...
  async generateGitHubStatus(): Promise<{ state: string; description: string }> {
    try {
      const report = await this.runQualityGate();
      return {
...
async function checkLLMSignals(): Promise<GateResult> {
  ...
  try {
    ...
    return {
      ...
      details: signalRatio < 50 ? ['Consider adding LLM optimization tags to key documents'] : undefined
    };
  } catch (error) {
    return {
      passed: false,
