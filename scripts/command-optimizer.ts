...
class CommandOptimizer {
  ...
  private categorizeCommand(name: string, command: string): CommandAnalysis {
    ...
    if (coreCommands.includes(name)) {
      category = "core";
      usage =
        name.includes("ci:") || ["build", "dev", "test"].includes(name)
          ? "daily"
          : "weekly";
      ...
      name.includes(":") &&
      (name.includes("baseline") || name.includes("report"))
    ) {
      category = "specialized";
      usage = "rare";
  ...
  private async generateOptimizationReport(
    ...
    console.log(
      `   Reduction: ${((reduction / report.summary.total) * 100).toFixed(1)}%`,
    );
  }

  private async generateCleanPackageJson(
    analysis: CommandAnalysis[],
    ...
    // Core commands 유지
    const keepCommands = analysis.filter(
      (a) => a.recommendation === "keep" || a.category === "core",
    );
    ...
    const cleanPackageJson = {
      ...
      scripts: {
        ...
        ...Object.fromEntries(
          Object.entries(cleanScripts).filter(
            ([name]) =>
              ![
                "build",
                "dev",
