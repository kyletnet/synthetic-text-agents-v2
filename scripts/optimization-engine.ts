...
export class OptimizationEngine {
  ...
  constructor() {
    ...
    this.optimizationHistoryPath = join(
      this.projectRoot,
      "reports",
  ...
  private async collectMetrics(): Promise<OptimizationMetrics> {
    ...
    try {
      ...
      const gapResult = execSync(
        'npm run workflow:gaps 2>/dev/null || echo "0"',
        {
          encoding: "utf8",
          stdio: "pipe",
        },
      ...
      const registryResult = execSync(
        'npm run registry:summary 2>/dev/null || echo ""',
        {
          encoding: "utf8",
          stdio: "pipe",
        },
  ...
  private async identifyOptimizations(
    ...
    if (metrics.workflowGaps > 0) {
      opportunities.push({
        ...
        description:
          "Convert notification-only patterns to interactive workflows",
        impact: "high",
        effort: "medium",
    ...
    opportunities.push({
      ...
      description:
        "Run independent maintenance tasks in parallel for 50% speedup",
      impact: "high",
      effort: "low",
  ...
  private printOptimizationReport(
    opportunities: OptimizationOpportunity[],
  ): void {
  ...
  private calculateImprovement(
    ...
    const summary =
      [
        improvements.workflowGaps > 0
          ? `${improvements.workflowGaps} workflow gaps fixed`
          : null,
        improvements.governanceCoverage > 0
          ? `+${improvements.governanceCoverage}% governance coverage`
          : null,
        improvements.automationLevel > 0
          ? `+${improvements.automationLevel}% automation`
          : null,
      ]
        .filter(Boolean)
        .join(", ") || "System optimized";
  ...
  private saveOptimizationSession(
    ...
    if (existsSync(this.optimizationHistoryPath)) {
      try {
        history = JSON.parse(
          readFileSync(this.optimizationHistoryPath, "utf8"),
        );
      } catch (error) {
        history = [];
      }
  ...
  async getTrends(): Promise<void> {
    ...
    if (history.length > 1) {
      ...
      console.log(`\\n📊 Overall Progress:`);
      console.log(
        `   Workflow Gaps: ${first.beforeMetrics.workflowGaps} → ${last.afterMetrics.workflowGaps}`,
      );
      console.log(
        `   Governance Coverage: ${first.beforeMetrics.governanceCoverage}% → ${last.afterMetrics.governanceCoverage}%`,
      );
      console.log(
        `   Automation Level: ${first.beforeMetrics.automationLevel}% → ${last.afterMetrics.automationLevel}%`,
      );
    }
