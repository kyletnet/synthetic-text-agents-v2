...
import { DxThresholds, MetricsAnalysis, AnomalyDetection, ActionRecommendation, GatingResult, ConsistencyCheck } from './types.js';
...
function checkP0Violations(
  ...
  if (metrics.pii_license.pii_hits > thresholds.pii_hits_max) {
    violations.push(`PII detected: ${metrics.pii_license.pii_hits} hits (max: ${thresholds.pii_hits_max})`);
  }

  // License violations
  if (metrics.pii_license.license_violations > thresholds.license_violations_max) {
    violations.push(`License violations: ${metrics.pii_license.license_violations} (max: ${thresholds.license_violations_max})`);
  }
  ...
  if (evidenceMissingRate > thresholds.evidence_missing_rate_max) {
    violations.push(`Evidence missing rate: ${(evidenceMissingRate * 100).toFixed(1)}% (max: ${(thresholds.evidence_missing_rate_max * 100).toFixed(1)}%)`);
  }

  // Hallucination rate
  if (metrics.hallucination.rate > thresholds.hallucination_rate_max) {
    violations.push(`Hallucination rate: ${(metrics.hallucination.rate * 100).toFixed(1)}% (max: ${(thresholds.hallucination_rate_max * 100).toFixed(1)}%)`);
  }
...
function checkP1Warnings(
  ...
  if (metrics.cost_latency.cost_per_item > thresholds.cost_per_item_fail) {
    warnings.push(`Cost per item FAIL: $${metrics.cost_latency.cost_per_item.toFixed(3)} (threshold: $${thresholds.cost_per_item_fail})`);
  } else if (metrics.cost_latency.cost_per_item > thresholds.cost_per_item_warn) {
    warnings.push(`Cost per item WARN: $${metrics.cost_latency.cost_per_item.toFixed(3)} (threshold: $${thresholds.cost_per_item_warn})`);
  }

  // Latency P95
  if (metrics.cost_latency.latency_p95_ms > thresholds.latency_p95_fail_ms) {
    warnings.push(`P95 latency FAIL: ${metrics.cost_latency.latency_p95_ms}ms (threshold: ${thresholds.latency_p95_fail_ms}ms)`);
  } else if (metrics.cost_latency.latency_p95_ms > thresholds.latency_p95_warn_ms) {
    warnings.push(`P95 latency WARN: ${metrics.cost_latency.latency_p95_ms}ms (threshold: ${thresholds.latency_p95_warn_ms}ms)`);
  }

  // Failure rate
  if (metrics.failure_retry.failure_rate > thresholds.failure_rate_fail) {
    warnings.push(`Failure rate FAIL: ${(metrics.failure_retry.failure_rate * 100).toFixed(1)}% (threshold: ${(thresholds.failure_rate_fail * 100).toFixed(1)}%)`);
  } else if (metrics.failure_retry.failure_rate > thresholds.failure_rate_warn) {
    warnings.push(`Failure rate WARN: ${(metrics.failure_retry.failure_rate * 100).toFixed(1)}% (threshold: ${(thresholds.failure_rate_warn * 100).toFixed(1)}%)`);
  }
...
function checkP2Issues(
  ...
  if (metrics.duplication.rate > thresholds.duplication_rate_fail) {
    issues.push(`Duplication rate FAIL: ${(metrics.duplication.rate * 100).toFixed(1)}% (threshold: ${(thresholds.duplication_rate_fail * 100).toFixed(1)}%)`);
  } else if (metrics.duplication.rate > thresholds.duplication_rate_warn) {
    issues.push(`Duplication rate WARN: ${(metrics.duplication.rate * 100).toFixed(1)}% (threshold: ${(thresholds.duplication_rate_warn * 100).toFixed(1)}%)`);
  }

  // Coverage rate (lower is worse)
  if (metrics.coverage.entity_coverage_rate < thresholds.coverage_rate_fail) {
    issues.push(`Coverage rate FAIL: ${(metrics.coverage.entity_coverage_rate * 100).toFixed(1)}% (threshold: ${(thresholds.coverage_rate_fail * 100).toFixed(1)}%)`);
  } else if (metrics.coverage.entity_coverage_rate < thresholds.coverage_rate_warn) {
    issues.push(`Coverage rate WARN: ${(metrics.coverage.entity_coverage_rate * 100).toFixed(1)}% (threshold: ${(thresholds.coverage_rate_warn * 100).toFixed(1)}%)`);
  }
  ...
  if (qualityScore < thresholds.quality_score_fail) {
    issues.push(`Quality score FAIL: ${(qualityScore * 100).toFixed(1)}% (threshold: ${(thresholds.quality_score_fail * 100).toFixed(1)}%)`);
  } else if (qualityScore < thresholds.quality_score_warn) {
    issues.push(`Quality score WARN: ${(qualityScore * 100).toFixed(1)}% (threshold: ${(thresholds.quality_score_warn * 100).toFixed(1)}%)`);
  }
...
function applyAdditionalChecks(
  ...
  for (const spike of anomalies.spikes) {
    p1Warnings.push(`${spike.type} spike: ${spike.value.toFixed(3)} (threshold: ${spike.threshold.toFixed(3)})`);
  }
  ...
  for (const issue of consistency.issues) {
    if (issue.includes('CASES_TOTAL is 0') || issue.includes('Session report not found')) {
...
export function performGating(
  ...
  try {
    ...
    const gateDecision = determineGateStatus(p0Violations, p1Warnings, p2Issues, recommendations);
...
export function summarizeGating(result: GatingResult): {
  ...
  switch (result.gate_status) {
    ...
    case 'WARN':
      status_emoji = '⚠️';
      summary_text = 'Minor issues detected but within acceptable limits - proceed with monitoring';
      action_required = false;
    ...
    case 'PARTIAL':
      status_emoji = '🟡';
      summary_text = 'Significant warnings detected - proceed with caution and enhanced monitoring';
    ...
    case 'FAIL':
      status_emoji = '❌';
      summary_text = 'Critical issues detected - full run blocked until resolved';
      action_required = true;
  ...
  if (result.p0_violations.length > 0) {
    next_steps.push(`Resolve ${result.p0_violations.length} critical P0 violations`);
  }

  if (result.p1_warnings.length > 3) {
    next_steps.push(`Review ${result.p1_warnings.length} P1 performance warnings`);
  }

  if (result.p2_issues.length > 2) {
    next_steps.push(`Consider addressing ${result.p2_issues.length} P2 quality issues`);
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === 'gate') {
    ...
    const mockMetrics: MetricsAnalysis = {
      ...
      coverage: { entity_coverage_rate: 0.8, section_coverage_rate: 0.85, uncovered_entities: [] },
      evidence: { presence_rate: 0.9, alignment_mean: 0.75, alignment_p95: 0.6, missing_evidence_count: 1 },
      hallucination: { rate: 0.02, high_risk_cases: 0, confidence_scores: [0.8, 0.9, 0.85] },
      pii_license: { pii_hits: 0, license_violations: 0, risk_samples: [] },
      cost_latency: { cost_per_item: 0.12, latency_p50_ms: 1200, latency_p95_ms: 3500, budget_utilization: 0.6 },
      failure_retry: { failure_rate: 0.05, retry_count: 1, dlq_count: 1, top_error_classes: ['timeout'] }
    };

    const mockThresholds: DxThresholds = {
      p0: { pii_hits_max: 0, license_violations_max: 2, evidence_missing_rate_max: 0.3, hallucination_rate_max: 0.05 },
      p1: { cost_per_item_warn: 0.08, cost_per_item_fail: 0.15, latency_p95_warn_ms: 4000, latency_p95_fail_ms: 8000, failure_rate_warn: 0.1, failure_rate_fail: 0.25 },
      p2: { duplication_rate_warn: 0.1, duplication_rate_fail: 0.2, coverage_rate_warn: 0.7, coverage_rate_fail: 0.5, quality_score_warn: 0.7, quality_score_fail: 0.5 }
    };
    ...
    const result = performGating(mockMetrics, mockThresholds, mockAnomalies, mockRecommendations, mockConsistency);
