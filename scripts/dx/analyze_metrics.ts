...
function analyzeDuplication(baselineData: any): MetricsAnalysis['duplication'] {
  ...
  return {
    ...
    semantic_duplicates: baselineData.duplication_metrics?.semantic_duplicates || 0
  };
}
...
function analyzeCoverage(baselineData: any): MetricsAnalysis['coverage'] {
  ...
  return {
    entity_coverage_rate: baselineData.coverage_metrics?.entity_coverage_rate || 0,
    section_coverage_rate: baselineData.coverage_metrics?.section_coverage_rate || 0,
...
function analyzeEvidence(baselineData: any): MetricsAnalysis['evidence'] {
  ...
  return {
    ...
    missing_evidence_count: baselineData.evidence_metrics?.missing_evidence_count || 0
  };
}
 ...
 */
function analyzeHallucination(baselineData: any): MetricsAnalysis['hallucination'] {
  if (!baselineData) {
    return {
      rate: 0,
      high_risk_cases: 0,
  ...
  return {
    ...
    confidence_scores: baselineData.hallucination_metrics?.confidence_scores || []
  };
}
...
function analyzePiiLicense(baselineData: any): MetricsAnalysis['pii_license'] {
  ...
  return {
    ...
    license_violations: baselineData.pii_license_metrics?.license_violations || 0,
 ...
 */
function analyzeCostLatency(sessionData: any, llmData: any): MetricsAnalysis['cost_latency'] {
  const costPerItem = (sessionData?.cost_usd || 0) / Math.max(sessionData?.cases_total || 1, 1);
  const budgetUtilization = (sessionData?.cost_usd || 0) / Math.max(sessionData?.budget_usd || 1, 1);
 ...
 */
async function analyzeFailureRetry(sessionData: any): Promise<MetricsAnalysis['failure_retry']> {
  ...
  try {
    ...
    for (const entry of dlqEntries.slice(0, 10)) { // Limit to first 10 for performance
      try {
          ...
          try {
            ...
            for (const line of lines) {
              ...
              if (reasonMatch) {
                ...
                if (!topErrorClasses.includes(reason) && topErrorClasses.length < 5) {
...
export async function analyzeMetrics(): Promise<MetricsAnalysis> {
  try {
    ...
    const cost_latency = analyzeCostLatency(collection.session_report, collection.llm_analysis);
...
export function summarizeMetrics(metrics: MetricsAnalysis): {
  ...
  if (metrics.pii_license.license_violations > 2) {
    critical_issues.push(`License violations: ${metrics.pii_license.license_violations} detected`);
  }
  if (metrics.evidence.presence_rate < 0.7) {
    critical_issues.push(`Low evidence presence: ${(metrics.evidence.presence_rate * 100).toFixed(1)}%`);
  }
  if (metrics.hallucination.rate > 0.05) {
    critical_issues.push(`High hallucination rate: ${(metrics.hallucination.rate * 100).toFixed(1)}%`);
  }

  // Warning issues (P1 level)
  if (metrics.cost_latency.cost_per_item > 0.08) {
    warning_issues.push(`High cost per item: $${metrics.cost_latency.cost_per_item.toFixed(3)}`);
  }
  if (metrics.cost_latency.latency_p95_ms > 4000) {
    warning_issues.push(`High P95 latency: ${metrics.cost_latency.latency_p95_ms}ms`);
  }
  if (metrics.failure_retry.failure_rate > 0.10) {
    warning_issues.push(`High failure rate: ${(metrics.failure_retry.failure_rate * 100).toFixed(1)}%`);
  }

  // Informational items (P2 level)
  if (metrics.duplication.rate > 0.10) {
    info_items.push(`Moderate duplication: ${(metrics.duplication.rate * 100).toFixed(1)}%`);
  }
  if (metrics.coverage.entity_coverage_rate < 0.70) {
    info_items.push(`Low entity coverage: ${(metrics.coverage.entity_coverage_rate * 100).toFixed(1)}%`);
  }
  if (metrics.failure_retry.dlq_count > 0) {
    info_items.push(`DLQ entries: ${metrics.failure_retry.dlq_count} failed runs`);
  }
