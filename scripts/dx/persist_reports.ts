...
async function loadSchema(): Promise<any> {
  try {
    const schemaContent = await fs.readFile(
      "schema/dxloop_report.schema.json",
      "utf-8",
    );
    ...
    console.warn(
      "Could not load DxLoop schema, validation will be skipped:",
      error,
    );
...
`
    ...
    gating.p1_warnings.length > 0
      ? `#### ⚠️ P1 Warnings (Performance)
${gating.p1_warnings.map((w) => `- ${w}`).join("\n")}

`
      : ""
  }${
    gating.p2_issues.length > 0
      ? `#### ℹ️ P2 Issues (Quality)
...
${
  dlq_status.recent_failures.length > 0
    ? `### Recent Failures
...
`
    ...
    dlq_status.retry_candidates.length > 0
      ? `### Retry Candidates
          ...
          try {
            ...
            if (lastLine) {
              ...
              if (match) {
                ...
                dlqStatus.recent_failures.push(
                  `${runId}: ${match[2]} (${match[1]})`,
                );
...
function generateDiffSummary(changes: ThresholdCalibration[]): string {
  ...
  const highConfidenceChanges = changes.filter(
    (c) => c.confidence > 0.8,
  ).length;
...
export async function persistReport(
  ...
  try {
    ...
    if (
      report.calibration.changes.length > 0 &&
      !report.calibration.diff_summary
    ) {
      report.calibration.diff_summary = generateDiffSummary(
    ...
    if (
      ...
      try {
        ...
        console.log(
          `Applied ${report.calibration.changes.length} threshold calibrations`,
        );
      } catch (error) {
...
export async function loadReport(
  filePath: string = "reports/dxloop_report.jsonl",
): Promise<DxLoopReport | null> {
  try {
  ...
  if (command === "persist") {
    ...
    const mockReport: DxLoopReport = {
      ...
      metrics: {
        ...
        coverage: {
          entity_coverage_rate: 0.85,
          section_coverage_rate: 0.9,
          uncovered_entities: [],
        },
        evidence: {
          presence_rate: 0.95,
      ...
      summary: {
        overall_status: "PASS",
        top_issues: [],
        next_actions: ["Proceed with full run"],
