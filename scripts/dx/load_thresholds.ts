...
function mergeDxLoopConfig(baselineConfig: any, dxConfig: DxLoopConfig): any {
  ...
  if (baselineConfig.evidence_quality?.alert_thresholds) {
    merged.dxloop.thresholds.p0.evidence_missing_rate_max =
      1.0 -
      baselineConfig.evidence_quality.alert_thresholds
        .evidence_presence_rate_min;
  }
  ...
  try {
    ...
    if (baselineConfig.dxloop) {
      dxConfig = {
        ...
        thresholds: {
          p0: {
            ...dxConfig.thresholds.p0,
            ...baselineConfig.dxloop.thresholds?.p0,
          },
          p1: {
            ...dxConfig.thresholds.p1,
            ...baselineConfig.dxloop.thresholds?.p1,
          },
          p2: {
            ...dxConfig.thresholds.p2,
            ...baselineConfig.dxloop.thresholds?.p2,
          },
        },
      };
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === "load") {
    ...
    loadBaselineConfig(configPath)
      .then((baselineConfig) => {
        const mergedConfig = mergeDxLoopConfig(
          baselineConfig,
          DEFAULT_DXLOOP_CONFIG,
        );
