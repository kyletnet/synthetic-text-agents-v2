  ...
  try {
    ...
    for (const filePath of historyFiles) {
      try {
        ...
        console.warn(`Could not parse historical metric from ${filePath}:`, error);
      }
    }
...
function calculateStatistics(values: number[]): {
  ...
  if (values.length === 0) {
    return { mean: 0, median: 0, stdDev: 0, q1: 0, q3: 0, iqr: 0, min: 0, max: 0 };
  }
...
function detectMetricAnomalies(
  ...
  if (stats.iqr > 0) {
    ...
    if (currentValue < lowerBound || currentValue > upperBound) {
      const severity = currentValue < stats.q1 - 3 * stats.iqr || currentValue > stats.q3 + 3 * stats.iqr
        ? 'high' as const
        : 'medium' as const;
...
function detectSpikes(
  ...
  if (costHistory.length > 0) {
    const recentCostMean = costHistory.slice(0, 5).reduce((sum, m) => sum + m.value, 0) / Math.min(5, costHistory.length);
  ...
  if (latencyHistory.length > 0) {
    const recentLatencyMean = latencyHistory.slice(0, 5).reduce((sum, m) => sum + m.value, 0) / Math.min(5, latencyHistory.length);
  ...
  if (failureHistory.length > 0) {
    const recentFailureMean = failureHistory.slice(0, 5).reduce((sum, m) => sum + m.value, 0) / Math.min(5, failureHistory.length);
 ...
 */
export async function detectAnomalies(metrics: MetricsAnalysis): Promise<AnomalyDetection> {
  try {
    ...
    const metricNames = ['cost_per_item', 'latency_p95_ms', 'failure_rate', 'budget_utilization'];
    const historicalData: { [key: string]: HistoricalMetric[] } = {};

    for (const metricName of metricNames) {
      historicalData[metricName] = await loadHistoricalMetrics(metricName, lookbackDays);
    }
...
export function summarizeAnomalies(detection: AnomalyDetection): {
  ...
  const has_critical_anomalies = criticalAnomalies.length > 0 || spikes.length > 0;
  ...
  for (const spike of spikes.slice(0, 2)) {
    top_concerns.push(`${spike.type} spike: ${spike.value.toFixed(3)} (threshold: ${spike.threshold.toFixed(3)})`);
  }

  // Add medium anomalies if room
  for (const anomaly of mediumAnomalies.slice(0, Math.max(0, 5 - top_concerns.length))) {
    top_concerns.push(`${anomaly.metric}: ${anomaly.description}`);
  }

  let recommendation = '';
  if (has_critical_anomalies) {
    recommendation = 'Critical anomalies detected - investigate immediately before proceeding with full run';
  } else if (mediumAnomalies.length > 2) {
    recommendation = 'Multiple anomalies detected - review metrics and consider adjusting thresholds';
  } else if (anomalies.length > 0) {
    recommendation = 'Minor anomalies detected - monitor closely but safe to proceed';
  } else {
    recommendation = 'No significant anomalies detected - metrics are within normal ranges';
  }
