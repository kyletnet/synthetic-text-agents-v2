...
import { MetricsAnalysis, AnomalyDetection, ActionRecommendation } from './types.js';
...
const RECOMMENDATION_RULES = {
  ...
  high_duplication: {
    ...
    hypothesis: 'Input data contains repetitive patterns or insufficient diversity',
    action: 'Review data source for duplicates, increase sampling diversity, or implement deduplication preprocessing',
  ...
  low_entity_coverage: {
    category: 'data' as const,
    severity: 'medium' as const,
    hypothesis: 'Dataset lacks comprehensive coverage of domain entities or key concepts',
    action: 'Expand training data to include missing entities, review domain coverage requirements',
  ...
  low_section_coverage: {
    ...
    severity: 'medium' as const,
    hypothesis: 'Retrieval system not accessing all relevant document sections or contexts',
    action: 'Tune retrieval parameters, expand chunk overlap, or improve indexing strategy',
  ...
  missing_evidence: {
    ...
    hypothesis: 'Retrieval system failing to find relevant supporting evidence for responses',
    action: 'Improve retrieval recall, expand knowledge base, or tune similarity thresholds',
  ...
  poor_evidence_alignment: {
    ...
    severity: 'medium' as const,
    hypothesis: 'Agent not effectively utilizing retrieved evidence in response generation',
    action: 'Improve evidence integration prompts, add citation requirements, or enhance context utilization',
  ...
  high_hallucination: {
    ...
    hypothesis: 'Model generating information not supported by provided context or knowledge',
    action: 'Strengthen grounding instructions, add factual verification steps, or implement confidence thresholds',
  ...
  high_cost_per_item: {
    category: 'agent' as const,
    severity: 'medium' as const,
    hypothesis: 'Inefficient token usage, oversized context windows, or unnecessary API calls',
    action: 'Optimize prompt length, implement response caching, or use smaller models for simple tasks',
  ...
  high_latency: {
    ...
    severity: 'medium' as const,
    hypothesis: 'Network bottlenecks, sequential processing, or resource constraints',
    action: 'Implement parallel processing, optimize API calls, or upgrade infrastructure',
  ...
  budget_overrun: {
    category: 'system' as const,
    severity: 'high' as const,
    hypothesis: 'Cost controls insufficient or usage patterns exceeding estimates',
    action: 'Implement stricter budget controls, add early warning systems, or optimize expensive operations',
  ...
  high_failure_rate: {
    ...
    action: 'Implement robust retry logic, add circuit breakers, or review system dependencies',
  ...
  dlq_accumulation: {
    category: 'system' as const,
    severity: 'medium' as const,
    hypothesis: 'Persistent failures not being resolved or inadequate error handling',
    action: 'Review DLQ processing logic, improve error classification, or add automated recovery',
  ...
  pii_detected: {
    category: 'data' as const,
    severity: 'critical' as const,
    hypothesis: 'Input data contains personal identifiable information not properly sanitized',
    action: 'Implement PII detection and redaction pipeline, review data sources for sensitive content',
  ...
  license_violations: {
    category: 'data' as const,
    severity: 'critical' as const,
    hypothesis: 'Training or reference data may contain copyrighted or restricted content',
    action: 'Audit data sources for licensing compliance, implement content filtering, or seek proper permissions',
  ...
  cache_miss_pattern: {
    ...
    hypothesis: 'Caching strategy not aligned with usage patterns or cache size insufficient',
    action: 'Analyze cache hit patterns, optimize cache keys, or increase cache capacity',
...
function generateIssueRecommendations(
  ...
  if (metrics.coverage.entity_coverage_rate < 0.60) {
    recommendations.push({
  ...
  if (metrics.coverage.section_coverage_rate < 0.70) {
    recommendations.push({
  ...
  if (metrics.evidence.presence_rate < 0.80) {
    recommendations.push({
  ...
  if (metrics.evidence.alignment_mean < 0.50) {
    recommendations.push({
  ...
  if (metrics.cost_latency.cost_per_item > 0.10) {
    recommendations.push({
  ...
  for (const anomaly of anomalies.anomalies) {
    if (anomaly.severity === 'high') {
      let rule: typeof RECOMMENDATION_RULES[keyof typeof RECOMMENDATION_RULES] = RECOMMENDATION_RULES.cache_miss_pattern; // Default
 ...
 */
function prioritizeRecommendations(recommendations: ActionRecommendation[]): ActionRecommendation[] {
...
export function recommendActions(
  ...
  try {
    ...
    const prioritizedRecommendations = prioritizeRecommendations(uniqueRecommendations);

    console.log('Action recommendations generated:', {
      total_recommendations: prioritizedRecommendations.length,
      critical: prioritizedRecommendations.filter(r => r.severity === 'critical').length,
      high: prioritizedRecommendations.filter(r => r.severity === 'high').length,
      medium: prioritizedRecommendations.filter(r => r.severity === 'medium').length,
      low: prioritizedRecommendations.filter(r => r.severity === 'low').length
    });
 ...
 */
export function summarizeRecommendations(recommendations: ActionRecommendation[]): {
  ...
  const effort_breakdown = recommendations.reduce((acc, rec) => {
    acc[rec.effort_estimate] = (acc[rec.effort_estimate] || 0) + 1;
    return acc;
  }, {} as { [key: string]: number });

  const category_breakdown = recommendations.reduce((acc, rec) => {
    acc[rec.category] = (acc[rec.category] || 0) + 1;
    return acc;
  }, {} as { [key: string]: number });

  const quick_wins = recommendations.filter(r =>
    r.effort_estimate === 'low' && (r.severity === 'medium' || r.severity === 'high')
  );

  const major_initiatives = recommendations.filter(r =>
    r.effort_estimate === 'high' && (r.severity === 'critical' || r.severity === 'high')
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === 'recommend') {
    ...
    const mockMetrics: MetricsAnalysis = {
      ...
      coverage: { entity_coverage_rate: 0.55, section_coverage_rate: 0.68, uncovered_entities: ['concept_a', 'concept_b'] },
      evidence: { presence_rate: 0.75, alignment_mean: 0.45, alignment_p95: 0.30, missing_evidence_count: 8 },
      hallucination: { rate: 0.08, high_risk_cases: 2, confidence_scores: [0.6, 0.7, 0.5] },
      pii_license: { pii_hits: 1, license_violations: 3, risk_samples: ['email@example.com'] },
      cost_latency: { cost_per_item: 0.12, latency_p50_ms: 2000, latency_p95_ms: 6000, budget_utilization: 0.98 },
      failure_retry: { failure_rate: 0.30, retry_count: 5, dlq_count: 8, top_error_classes: ['timeout', 'rate_limit'] }
    };
