...
async function loadHistoricalRuns(
  ...
  try {
    ...
    console.log(
      `Loaded ${runs.length} historical runs from ${filesToProcess.length} files`,
    );
    return runs;
  } catch (error) {
 ...
 */
async function parseHistoricalRun(
  filePath: string,
): Promise<HistoricalRun | null> {
  try {
    ...
    const costPerItem =
      parseFloat(data.cost_usd || "0") /
      Math.max(parseInt(data.cases_total || "1"), 1);
    ...
    if (timestampMatch) {
      const baselineReportPath = filePath.replace(
        "session_report.md",
      ...
      try {
        ...
        coverageRate =
          baselineData.coverage_metrics?.entity_coverage_rate || 0.8;
...
function detectDrift(
  ...
  const relativeChange =
    Math.abs(recentMean - olderMean) / Math.max(olderMean, 0.001);
  const thresholdChange =
    Math.abs(recentMean - currentThreshold) / Math.max(currentThreshold, 0.001);
...
export async function autoCalibrateThresholds(
  ...
  try {
    ...
    const historicalRuns = await loadHistoricalRuns(
      config.autocalibration.lookback_runs,
    );

    if (historicalRuns.length < 5) {
      console.warn(
        `Insufficient historical data for calibration: ${historicalRuns.length} runs (minimum 5 required)`,
      );
      return calibrations;
    }

    // Filter runs by profile if specified
    const profileRuns = profile
      ? historicalRuns.filter((run) => run.profile === profile)
      : historicalRuns;

    if (profileRuns.length < 3) {
      console.warn(
        `Insufficient profile-specific data for calibration: ${profileRuns.length} runs`,
      );
      return calibrations;
    }

    console.log(
      `Using ${profileRuns.length} profile-matched runs for calibration`,
    );

    // Extract metric arrays
    const costPerItem = profileRuns
      .map((run) => run.metrics.cost_per_item)
      .filter((v) => v > 0);
    const latencyP95 = profileRuns
      .map((run) => run.metrics.latency_p95_ms)
      .filter((v) => v > 0);
    const failureRate = profileRuns
      .map((run) => run.metrics.failure_rate)
      .filter((v) => v >= 0);
    const duplicationRate = profileRuns
      .map((run) => run.metrics.duplication_rate)
      .filter((v) => v >= 0);
    const coverageRate = profileRuns
      .map((run) => run.metrics.coverage_rate)
      .filter((v) => v > 0);
    const qualityScore = profileRuns
      .map((run) => run.metrics.quality_score)
      .filter((v) => v > 0);

    // P1 Calibrations
    if (costPerItem.length >= 3) {
      const warnThreshold = calculatePercentile(
        costPerItem,
        config.autocalibration.percentile_warn,
      );
      const failThreshold = calculatePercentile(
        costPerItem,
        config.autocalibration.percentile_fail,
      );
      const drift = detectDrift(
        costPerItem,
        config.thresholds.p1.cost_per_item_warn,
        config.autocalibration.drift_guard_max_delta,
      );
      ...
      calibrations.push({
        ...
        confidence: Math.min(
          costPerItem.length / config.autocalibration.lookback_runs,
          1.0,
        ),
    ...
    if (latencyP95.length >= 3) {
      ...
      const drift = detectDrift(
        latencyP95,
        config.thresholds.p1.latency_p95_warn_ms,
        config.autocalibration.drift_guard_max_delta,
      );
      ...
      calibrations.push({
        ...
        confidence: Math.min(
          latencyP95.length / config.autocalibration.lookback_runs,
          1.0,
        ),
    ...
    if (failureRate.length >= 3) {
      ...
      const drift = detectDrift(
        failureRate,
        config.thresholds.p1.failure_rate_warn,
        config.autocalibration.drift_guard_max_delta,
      );
      ...
      calibrations.push({
        ...
        suggested_fail: Math.round(failThreshold * 1000) / 1000,
        confidence: Math.min(
          failureRate.length / config.autocalibration.lookback_runs,
          1.0,
        ),
    ...
    if (duplicationRate.length >= 3) {
      ...
      const drift = detectDrift(
        duplicationRate,
        config.thresholds.p2.duplication_rate_warn,
        config.autocalibration.drift_guard_max_delta,
      );
      ...
      calibrations.push({
        ...
        suggested_fail: Math.round(failThreshold * 1000) / 1000,
        confidence: Math.min(
          duplicationRate.length / config.autocalibration.lookback_runs,
          1.0,
        ),
    ...
    if (coverageRate.length >= 3) {
      ...
      const warnThreshold = calculatePercentile(
        coverageRate,
        100 - config.autocalibration.percentile_warn,
      );
      const failThreshold = calculatePercentile(
        coverageRate,
        100 - config.autocalibration.percentile_fail,
      );
      const drift = detectDrift(
        coverageRate,
        config.thresholds.p2.coverage_rate_warn,
        config.autocalibration.drift_guard_max_delta,
      );
      ...
      calibrations.push({
        ...
        suggested_fail: Math.round(failThreshold * 1000) / 1000,
        confidence: Math.min(
          coverageRate.length / config.autocalibration.lookback_runs,
          1.0,
        ),
    ...
    if (qualityScore.length >= 3) {
      ...
      const drift = detectDrift(
        qualityScore,
        config.thresholds.p2.quality_score_warn,
        config.autocalibration.drift_guard_max_delta,
      );
      ...
      calibrations.push({
        ...
        suggested_fail: Math.round(failThreshold * 1000) / 1000,
        confidence: Math.min(
          qualityScore.length / config.autocalibration.lookback_runs,
          1.0,
        ),
    ...
    console.log(
      `Auto-calibration completed: ${calibrations.length} calibrations generated`,
    );
    return calibrations;
  } catch (error) {
...
export function applyCalibrations(
  ...
  for (const calibration of calibrations) {
    ...
    if (driftGuardEnabled && calibration.drift_detected) {
      ...
      const deltaWarn =
        Math.abs(suggestedWarn - currentWarn) / Math.max(currentWarn, 0.001);
    ...
    if (calibration.metric in updatedConfig.thresholds.p1) {
      const p1Key =
        calibration.metric as keyof typeof updatedConfig.thresholds.p1;
      if (calibration.metric.includes("warn")) {
        (updatedConfig.thresholds.p1 as any)[p1Key] =
          calibration.suggested_warn;
      } else if (calibration.metric.includes("fail")) {
        (updatedConfig.thresholds.p1 as any)[p1Key] =
          calibration.suggested_fail;
      } else {
        // Update both warn and fail thresholds
        const warnKey =
          `${calibration.metric}_warn` as keyof typeof updatedConfig.thresholds.p1;
        const failKey =
          `${calibration.metric}_fail` as keyof typeof updatedConfig.thresholds.p1;
        if (warnKey in updatedConfig.thresholds.p1) {
          (updatedConfig.thresholds.p1 as any)[warnKey] =
            calibration.suggested_warn;
        }
        if (failKey in updatedConfig.thresholds.p1) {
          (updatedConfig.thresholds.p1 as any)[failKey] =
            calibration.suggested_fail;
        }
      }
    }
    ...
    if (calibration.metric in updatedConfig.thresholds.p2) {
      const p2Key =
        calibration.metric as keyof typeof updatedConfig.thresholds.p2;
      const warnKey =
        `${calibration.metric}_warn` as keyof typeof updatedConfig.thresholds.p2;
      const failKey =
        `${calibration.metric}_fail` as keyof typeof updatedConfig.thresholds.p2;
      if (warnKey in updatedConfig.thresholds.p2) {
        (updatedConfig.thresholds.p2 as any)[warnKey] =
          calibration.suggested_warn;
      }
      if (failKey in updatedConfig.thresholds.p2) {
        (updatedConfig.thresholds.p2 as any)[failKey] =
          calibration.suggested_fail;
      }
    }

    appliedChanges.push(
      `${calibration.metric}: ${calibration.current_warn}→${calibration.suggested_warn} (warn), ${calibration.current_fail}→${calibration.suggested_fail} (fail)`,
    );
  }
    ...
    const defaultConfig: DxLoopConfig = {
      profiles: {
        dev: {
          ...
          per_agent_limits: { answer_max_usd: 0.05, audit_max_ms: 6000 },
        },
        stage: {
          name: "stage",
          budget_max_usd: 2,
          timeout_max_ms: 45000,
          per_agent_limits: { answer_max_usd: 0.1, audit_max_ms: 10000 },
        },
        prod: {
          name: "prod",
          budget_max_usd: 5,
          timeout_max_ms: 60000,
          per_agent_limits: { answer_max_usd: 0.2, audit_max_ms: 15000 },
        },
      },
      thresholds: {
        p0: {
