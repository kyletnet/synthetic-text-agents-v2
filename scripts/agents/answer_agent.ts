...
export class AnswerAgent extends BaseAgent {
  ...
  async execute(input: AnswerInput): Promise<AgentResult> {
    ...
    try {
      ...
      const enhancedAnswer = await this.enhanceAnswer(input.question, answerResult, input.evidence);
    ...
    try {
      ...
      const output: AnswerOutput = {
        ...
        reasoning: {
          ...
          logical_chain: ['Template-based generation due to budget constraints'],
  ...
  private determineGenerationStrategy(input: AnswerInput): {
    ...
    const suggestedStrategy = input.evidence.next_stage_hints.answer_generation_strategy;

    const notes: string[] = [];

    // Choose strategy based on evidence quality and hints
    if (suggestedStrategy === 'comprehensive' && evidenceQuality > 0.8) {
      notes.push('High-quality evidence available, using comprehensive approach');
      return {
        name: 'comprehensive',
  ...
  private async generateAnswer(input: AnswerInput, strategy: any): Promise<{
    text: string;
    cost: number;
    ...
    const evidenceContext = relevantEvidence.map((e, idx) =>
...
Please provide a well-structured answer based on the evidence above. Include confidence assessment and cite your sources.`;
    ...
    return {
      ...
      raw_confidence: this.estimateConfidenceFromText(result.data.content[0].text)
    };
  }
...
- Avoid overstating confidence beyond what evidence supports`;
  ...
  private async enhanceAnswer(question: string, answerResult: any, evidence: EvidenceOutput): Promise<{
    ...
    const citations = this.extractCitations(answerResult.text, evidence.evidence_items);
    const confidenceScore = this.calculateConfidenceScore(answerResult.raw_confidence, evidence);
    const answerType = this.determineAnswerType(confidenceScore, citations.length);

    // Analyze reasoning chain
    const reasoning = await this.analyzeReasoning(question, answerResult.text, evidence);
  ...
  private extractCitations(answerText: string, evidenceItems: EvidenceOutput['evidence_items']): AnswerOutput['answer']['citations'] {
    ...
    evidenceItems.forEach(evidence => {
      ...
      const isReferenced = answerText.toLowerCase().includes(evidence.source.toLowerCase()) ||
      ...
      if (isReferenced) {
        citations.push({
          ...
          quote: evidence.text.substring(0, 100) + (evidence.text.length > 100 ? '...' : '')
        });
      }
    });
  ...
  private calculateConfidenceScore(rawConfidence: number, evidence: EvidenceOutput): number {
    const evidenceQuality = evidence.search_summary.search_confidence;
    const evidenceCount = evidence.evidence_items.length;
    const highQualityEvidence = evidence.evidence_items.filter(e => e.relevance_score > 0.7).length;
    ...
    return Math.min(rawConfidence * (evidenceFactor + qualityFactor + highQualityFactor), 1.0);
  }

  private determineAnswerType(confidenceScore: number, citationCount: number): AnswerOutput['answer']['answer_type'] {
  ...
  private async analyzeReasoning(question: string, answer: string, evidence: EvidenceOutput): Promise<AnswerOutput['reasoning']> {
    // Analyze which evidence was used and identify reasoning gaps
    const usedEvidence = evidence.evidence_items.filter(e =>
      this.findTextSimilarity(answer, e.text) > 0.2
    );

    const evidenceQuality = usedEvidence.length > 0 ?
      (usedEvidence.reduce((sum, e) => sum + e.relevance_score, 0) / usedEvidence.length) > 0.7 ? 'high' :
      (usedEvidence.reduce((sum, e) => sum + e.relevance_score, 0) / usedEvidence.length) > 0.5 ? 'medium' : 'low'
      : 'low';
  ...
  private generateFallbackAnswer(input: AnswerInput): { text: string; citations: AnswerOutput['answer']['citations'] } {
    ...
    if (topEvidence.length > 0) {
      answerText += topEvidence.map((e, idx) =>
        `${idx + 1}. ${e.text.substring(0, 200)}...`
      ).join('\n\n');

      answerText += '\n\nThis response is generated using available evidence, though with limited processing due to resource constraints.';
    } else {
      answerText += 'I apologize, but I cannot provide a comprehensive answer based on the available evidence.';
    }
  ...
  private estimateConfidenceFromText(text: string): number {
    ...
    const confidenceIndicators = [
      ...
      'evidence shows', 'data indicates', 'research confirms'
    ];
    const uncertaintyIndicators = [
      'might', 'could', 'possibly', 'uncertain', 'unclear',
      'appears to', 'seems to', 'may indicate'
    ];

    const lowerText = text.toLowerCase();
    const confidenceCount = confidenceIndicators.filter(indicator => lowerText.includes(indicator)).length;
    const uncertaintyCount = uncertaintyIndicators.filter(indicator => lowerText.includes(indicator)).length;

    return Math.max(0.3, Math.min(0.9, 0.6 + (confidenceCount * 0.1) - (uncertaintyCount * 0.1)));
  }
  ...
  private identifyLogicalChain(answer: string): string[] {
    ...
    return sentences.slice(0, 3).map((s, idx) => `Step ${idx + 1}: ${s.trim()}`);
  }
