...
export class EvidenceAgent extends BaseAgent {
  ...
  async execute(input: EvidenceInput): Promise<AgentResult> {
    ...
    try {
      ...
      const rankedEvidence = await this.scoreAndRankEvidence(
        input.question,
        evidenceItems,
      );

      // Step 4: Generate hints for next stage
      const nextStageHints = this.generateAnswerHints(
        input.question,
        rankedEvidence,
      );
    ...
    const result = await callAnthropic(
      {
        ...
        max_tokens: 300,
        system:
          "You are an expert research strategist. Provide concise, actionable search strategies.",
...
Provide only a number between 0.0 and 1.0 representing relevance.`;
        ...
        const result = await callAnthropic(
          {
            ...
            max_tokens: 50,
            system:
              "You are an expert at evaluating evidence relevance. Respond with only a number.",
            messages: [{ role: "user", content: prompt }],
          },
          {
            runId: this.context.run_id,
  ...
  private calculateSearchConfidence(
    ...
    return Math.min(
      avgRelevance + highQualityCount * 0.1 + diversityBonus,
      1.0,
    );
  }
