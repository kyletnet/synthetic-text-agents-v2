...
export interface OrchestrationOutput {
  ...
  resource_usage: {
    ...
    agent_breakdown: Record<string, { cost: number; latency: number; retries: number }>;
  };
  quality_metrics: {
...
export class MultiAgentOrchestrator {
  ...
  constructor(
    ...
    this.budgetGuard = new BudgetGuard(configPath, context.profile, context.run_id);
  ...
  async orchestrate(input: OrchestrationInput): Promise<OrchestrationOutput> {
    ...
    try {
      ...
      const budgetPlan = await this.budgetGuardianPlanning(input, subAgentDecisions);

      // Phase 2: Diversity Planner - Strategy selection
      const processingStrategy = await this.diversityPlannerSelection(input, budgetPlan, subAgentDecisions);

      // Phase 3: Evidence → Answer → Audit chain execution
      const chainResult = await this.executeProcessingChain(input, processingStrategy, subAgentDecisions);
      ...
      const resourceUsage = this.calculateResourceUsage(chainResult, orchestrationStart);
  ...
  private async budgetGuardianPlanning(
    ...
    if (!shouldProceed) {
      decisions.budget_guardian_actions.push('Kill switch active - blocking execution');
      return {
        allowed: false,
        budget_allocation: {},
        fallback_thresholds: {},
    ...
    const totalEstimatedCost = Object.values(costEstimates).reduce((sum, cost) => sum + cost, 0);

    // Check budget constraints
    const maxBudget = input.budget_constraints?.max_cost_usd || 0.20;

    if (totalEstimatedCost > maxBudget) {
      decisions.budget_guardian_actions.push(`Estimated cost $${totalEstimatedCost.toFixed(4)} exceeds budget $${maxBudget.toFixed(4)} - enabling fallback modes`);
    ...
    decisions.budget_guardian_actions.push(`Budget approved: $${totalEstimatedCost.toFixed(4)} within limit $${maxBudget.toFixed(4)}`);
  ...
  private async diversityPlannerSelection(
    ...
    const questionCharacteristics = this.analyzeQuestionCharacteristics(input.question);

    // Select strategies based on question type, budget, and diversity requirements
    let evidenceStrategy = input.processing_preferences?.evidence_depth || 'deep';
    let answerStrategy = input.processing_preferences?.answer_style || 'comprehensive';
    ...
    if (budgetPlan.fallback_thresholds.evidence < 0.02) {
      ...
      decisions.diversity_planner_choices.push('Evidence: Shallow search due to budget constraints');
    }

    if (budgetPlan.fallback_thresholds.answer < 0.05) {
      answerStrategy = 'conservative';
      decisions.diversity_planner_choices.push('Answer: Conservative generation due to budget constraints');
    }

    if (budgetPlan.fallback_thresholds.audit < 0.01) {
      auditStrategy = 'basic';
      decisions.diversity_planner_choices.push('Audit: Basic review due to budget constraints');
    }

    // Ensure diversity based on question characteristics
    if (questionCharacteristics.complexity === 'high' && answerStrategy === 'conservative') {
      answerStrategy = 'focused';
      decisions.diversity_planner_choices.push('Answer: Upgraded to focused for complex question');
    }

    if (questionCharacteristics.has_multiple_aspects && evidenceStrategy === 'shallow') {
      evidenceStrategy = 'deep';
      decisions.diversity_planner_choices.push('Evidence: Upgraded to deep for multi-aspect question');
    }

    // Calculate diversity score
    const strategyVariation = new Set([evidenceStrategy, answerStrategy, auditStrategy]).size;
    const diversityScore = Math.min(strategyVariation / 3, 1.0);

    decisions.diversity_planner_choices.push(`Final strategy: Evidence=${evidenceStrategy}, Answer=${answerStrategy}, Audit=${auditStrategy} (diversity=${diversityScore.toFixed(2)})`);
  ...
  private async executeProcessingChain(
    ...
    if (!evidenceResult.success) {
      throw new Error(`Evidence stage failed: ${evidenceResult.error?.message}`);
    }
    ...
    const answerInput: AnswerInput = {
      ...
      max_answer_length: strategy.answer_strategy === 'comprehensive' ? 1000 : 500,
  ...
  private async executeWithRetryRouter<T>(
    ...
    return await this.dlqHandler.executeWithRetry(
      operation,
      {
  ...
  private async performFinalGating(chainResult: any): Promise<{
    ...
    const metrics = {
      ...
      hallucination_rate_max: chainResult.audit_stage.dimension_scores.accuracy < 80 ? 0.1 : 0.0,
      evidence_missing_rate_max: chainResult.evidence_stage.evidence_items.length === 0 ? 1.0 : 0.0,

      // P1 metrics (performance)
      cost_per_item_warn: (chainResult.evidence_stage.cost_usd || 0) + (chainResult.answer_stage.cost_usd || 0) + (chainResult.audit_stage.cost_usd || 0),
      ...
      coverage_rate_warn: chainResult.answer_stage.reasoning.evidence_used > 0 ? 1.0 : 0.5
    };
  ...
  private calculateResourceUsage(chainResult: any, orchestrationStart: number): OrchestrationOutput['resource_usage'] {
  ...
  private async handleOrchestrationFailure(
    ...
    decisions.retry_router_decisions.push(`Orchestration failed: ${error.message}`);

    // Log failure for analysis
    console.error(`[ORCHESTRATOR] Orchestration failed for ${this.context.run_id}:${this.context.item_id} - ${error}`);
  }
  ...
  private analyzeQuestionCharacteristics(question: string): {
    ...
    const complexityIndicators = ['compare', 'analyze', 'evaluate', 'explain', 'describe'];

    const hasComplexityIndicators = complexityIndicators.some(indicator =>
      question.toLowerCase().includes(indicator)
    );

    const multipleQuestionWords = questionWords.filter(word =>
      question.toLowerCase().includes(word)
    ).length > 1;

    return {
      complexity: hasComplexityIndicators ? 'high' : multipleQuestionWords ? 'medium' : 'low',
      has_multiple_aspects: multipleQuestionWords,
      question_type: questionWords.find(word => question.toLowerCase().includes(word)) || 'general'
    };
  }
  ...
  private estimateEvidenceCost(input: OrchestrationInput): number {
    ...
    return searchDepth === 'deep' ? Math.min(documentCount * 0.005, 0.03) : 0.01;
  }

  private estimateAnswerCost(input: OrchestrationInput): number {
    const answerStyle = input.processing_preferences?.answer_style || 'comprehensive';

    return answerStyle === 'comprehensive' ? 0.05 : answerStyle === 'focused' ? 0.03 : 0.02;
  }
  ...
  private estimateAuditCost(input: OrchestrationInput): number {
    ...
    return auditDepth === 'comprehensive' ? 0.02 : auditDepth === 'standard' ? 0.01 : 0.005;
  }
}
