...
function sampleEvaluationData(inputPath, outputPath, sampleSize = 100) {
  ...
  if (sampleSize >= totalItems) {
    console.log(
      `⚠️  Sample size (${sampleSize}) >= total items (${totalItems}), using all items`,
    );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length < 2) {
    console.log(
      "Usage: node sample_evaluation_data.js <input.jsonl> <output.jsonl> [sample_size=100]",
    );
    console.log(
      "Example: node sample_evaluation_data.js evaluation_data.jsonl evaluation_sample.jsonl 50",
    );
    process.exit(1);
  }
