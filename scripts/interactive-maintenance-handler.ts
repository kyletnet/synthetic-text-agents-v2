...
class InteractiveMaintenanceHandler {
  ...
  private readonly riskProfiles: Record<string, RiskProfile> = {
    safe: {
      ...
      warningMessage: '이 작업은 완전히 안전하며 시스템에 영향을 주지 않습니다.',
    ...
    medium: {
      ...
      consequences: ['일부 기능에 영향 가능', '테스트 후 확인 권장', '백업본으로 복원 가능']
    },
    high: {
      emoji: '🟠',
      color: 'red',
    ...
    critical: {
      ...
      defaultChoice: 'n',
      requiresExpertConfirmation: true,
      warningMessage: '🚨 STOP! 이 작업은 시스템을 완전히 망가뜨릴 수 있습니다.',
      consequences: [
        '시스템 전체 중단 가능',
        '데이터 손실 위험',
        '복구 불가능할 수 있음',
  ...
  private async handleSafeActions(actions: MaintenanceAction[]): Promise<void> {
    ...
    console.log('이 작업들은 100% 안전하며 시스템에 전혀 영향을 주지 않습니다.');
  ...
  private async handleLowRiskActions(actions: MaintenanceAction[]): Promise<void> {
  ...
  private async handleMediumRiskActions(actions: MaintenanceAction[]): Promise<void> {
  ...
  private async handleHighRiskActions(actions: MaintenanceAction[]): Promise<void> {
  ...
  private async handleCriticalActions(actions: MaintenanceAction[]): Promise<void> {
  ...
  private async displayDetailedRiskInfo(action: MaintenanceAction): Promise<void> {
    ...
    console.log('\n⚠️  **주의사항**:');
    profile.consequences.forEach(consequence => console.log(`  • ${consequence}`));
  }

  private async displayCriticalWarning(action: MaintenanceAction): Promise<void> {
    const profile = this.riskProfiles[action.riskLevel];
    ...
    console.log(`⏰ **예상 시간**: ${action.estimatedTime}`);
    console.log(`🔄 **복구 가능성**: ${action.rollbackable ? '부분적으로 가능' : '매우 어려움'}`);
  ...
  private async promptUser(question: string, yesChar: string, noChar: string, hint: string): Promise<boolean> {
  ...
  private groupActionsByRisk(actions: MaintenanceAction[]): Record<string, MaintenanceAction[]> {
  ...
  const sampleActions: MaintenanceAction[] = [
    {
      ...
      userFriendlyExplanation: '사용하지 않는 코드를 정리해서 코드를 깔끔하게 만듭니다.',
    ...
    },
    {
      ...
      userFriendlyExplanation: 'TypeScript의 타입 검사를 더 정확하게 만들어 오류를 줄입니다.',
      requiresExpertReview: false,
      autoApprovable: true,
      estimatedTime: '1분',
      rollbackable: true
    },
    {
      ...
      riskLevel: 'high',
      impact: ['전체 시스템 동작 방식 변경', '성능에 영향 가능', '일부 기능 중단 가능'],
      technicalDetails: 'Update tsconfig.json and package.json configurations',
      userFriendlyExplanation: '시스템의 핵심 설정 파일을 수정합니다. 잘못되면 전체 시스템이 작동하지 않을 수 있습니다.',
