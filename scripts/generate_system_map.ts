...
function writeSystemMap(repoRoot: string) {
  ...
  const rag = has(["src/rag/chunk\\.ts", "src/rag/retriever\\.ts"], repoRoot)
    ? "ok"
    : "missing";
  const augParaphrase = has(["src/augmentation/"], repoRoot)
    ? "partial"
    : "missing";
  const docPath = has(["seed_doc_path"], path.join(repoRoot, "src"))
    ? "partial"
    : "missing";
  const pairsPath = has(["gold_pairs_path"], path.join(repoRoot, "src"))
    ? "partial"
    : "missing";
