...
function safeJson(p){
  ...
  try{ return { ok:true, value: JSON.parse(txt), path:p }; }
  catch(e){ return { ok:false, error:'json-parse-failed', path:p, detail:String(e) }; }
}
function findNeedle(txt, needles){
  const got = {};
  for(const n of needles){ got[n] = txt.includes(n); }
  return got;
}
...
(function main(){
  ...
  let smokeCmd = null, bcLoad = null;
  for(const p of bcs){
    const j = safeJson(p);
    if(j.ok){
      bcLoad = j;
      if(j.value && j.value.runners && typeof j.value.runners.smoke === 'string'){
  ...
  let runV3Exec = false;
  try{ fs.accessSync(runV3, fs.constants.X_OK); runV3Exec = true; } catch{ runV3Exec = false; }

  // Build the report
  const report = {
    workspace: wd,
    src: { preflight_ts: exists(srcPreflight), mtime: sStat?.mtime?.toISOString?.(), newerThanDist: srcNewer },
    dist: {
    ...
    product_plan: {
      root_exists: exists(ppRoot), root_header_ok: ppRootHdr.found, root_fields: ppRootHdr.fields,
      docs_exists: exists(ppDocs), docs_header_ok: ppDocsHdr.found, docs_fields: ppDocsHdr.fields
    },
    red_flags: []
  };
  ...
  if (needles['No smoke runner found, simulating'] && !needles['resolveSmokeCmd'])
  ...
  if (!smokeCmd && !envSmoke)
    report.red_flags.push('NO_SMOKE_RUNNER_CONFIG');
  ...
  lines.push(`- src/scripts/preflight_pack.ts: ${exists(srcPreflight)} mtime=${report.src.mtime}`);
  lines.push(`- dist/scripts/preflight_pack.js: ${exists(distPreflight)} mtime=${report.dist.mtime} hash=${report.dist.hash12}`);
  lines.push(`- src newer than dist? ${srcNewer === null ? 'n/a' : srcNewer}`);
  lines.push('');
  lines.push('2) Dist content markers');
  Object.entries(needles).forEach(([k,v])=>lines.push(`- contains("${k}") = ${v}`));
  lines.push('');
  lines.push('3) Manifest presence');
  lines.push(`- data_manifest.json exists? ${report.manifest.data_manifest_exists}`);
  lines.push('');
  lines.push('4) Smoke runner resolution');
  lines.push(`- ENV.SMOKE_RUN_CMD: ${envSmoke ? '(set)' : '(unset)'}`);
  lines.push(`- baseline_config: path=${report.smoke.baseline_config_path} ok=${report.smoke.baseline_config_ok}`);
  lines.push(`- runners.smoke: ${smokeCmd || '(none)'}`);
  lines.push(`- run_v3.sh present=${report.smoke.run_v3_present} executable=${report.smoke.run_v3_executable}`);
  lines.push('');
  lines.push('5) Product Plan detection');
  lines.push(`- ./PRODUCT_PLAN.md exists=${report.product_plan.root_exists} headerOk=${report.product_plan.root_header_ok}`);
  lines.push(`- ./docs/PRODUCT_PLAN.md exists=${report.product_plan.docs_exists} headerOk=${report.product_plan.docs_header_ok}`);
  if(report.product_plan.root_fields) lines.push(`- root fields: ${JSON.stringify(report.product_plan.root_fields)}`);
  if(report.product_plan.docs_fields) lines.push(`- docs fields: ${JSON.stringify(report.product_plan.docs_fields)}`);
  lines.push('');
  lines.push('RED FLAGS: ' + (report.red_flags.length ? report.red_flags.join(', ') : '(none)'));
  lines.push('');
