...
const CANONICAL_STAGES = new Set([
  'STEP_1_TYPESCRIPT', 'STEP_2_LINT', 'STEP_3_SANITY', 'STEP_4_SMOKE_PAID',
  'STEP_5_GATING', 'STEP_6_OBSERVABILITY', 'STEP_7_FULL_RUN'
]);
...
class TaxonomyChecker {
  ...
  async scanProject(roots: string[], sessionPath?: string, schemaPath?: string, obsGlob?: string): Promise<TaxonomyReport> {
  ...
  private async scanFile(filePath: string): Promise<void> {
    try {
      ...
      lines.forEach((line, index) => {
        ...
        const severityMatches = this.checkSeverities(filePath, index + 1, line, lines);
  ...
  private checkSeverities(file: string, lineNum: number, line: string, allLines: string[]): number {
...
async function main(): Promise<void> {
  const { values } = parseArgs({
    ...
    options: {
      roots: { type: 'string', default: 'src,scripts,validators,reports,.claude/commands,docs' },
      session: { type: 'string' },
      schema: { type: 'string' },
  ...
  const report = await checker.scanProject(roots, values.session, values.schema, values.obs_glob);
  ...
  const hasIssues = report.unknown_severities.length > 0 || report.unknown_stages.length > 0;
