...
class PerformanceAutoImprover {
  ...
  async analyzeAndTrigger(options: {
    dryRun?: boolean;
    forceGrade?: 'A' | 'B' | 'C' | 'D';
    autoExecute?: boolean;
  } = {}): Promise<void> {
    ...
    const currentGrade = forceGrade || performanceReport.metrics.trends.performanceGrade;
    ...
    const applicableActions = this.findApplicableActions(actions, performanceReport);
    ...
    applicableActions.forEach((action, index) => {
      ...
      console.log(`   ${index + 1}. ${autoEmoji} ${riskEmoji} **${action.name}**`);
      console.log(`      ${action.description}`);
      console.log(`      Expected: ${action.estimatedImpact}`);
      console.log(`      Risk: ${action.riskLevel.toUpperCase()} | Auto: ${action.autoExecute ? 'Yes' : 'Manual approval required'}`);
    ...
    if (autoExecute) {
      ...
      console.log('⏸️  Auto-execution disabled - actions queued for manual approval');
      this.displayApprovalCommands(queue);
    }
  }

  private findApplicableActions(actions: ImprovementAction[], report: any): ImprovementAction[] {
    return actions.filter(action => {
      const conditions = action.triggerConditions;
      const metrics = report.metrics;

      // Check performance grade condition
      if (conditions.performanceGrade &&
      ...
      if (conditions.latencyThreshold) {
        const avgLatency = (metrics.routingEfficiency.directMode.avgLatency +
                           metrics.routingEfficiency.hubMode.avgLatency) / 2;
      ...
      if (conditions.throughputThreshold) {
        const throughputNum = parseFloat(metrics.performance.throughputImprovement.replace('%', '')) / 100;
      ...
      if (conditions.failoverCountThreshold) {
        if (metrics.performance.failoverCount < conditions.failoverCountThreshold) {
          return false;
        }
      }
  ...
  private async queueActions(actions: ImprovementAction[], performanceReport: any): Promise<ImprovementQueue> {
  ...
  private async executeQueuedActions(queue: ImprovementQueue): Promise<void> {
    ...
    const autoActions = queue.actions.filter(action => action.autoExecute && action.riskLevel === 'low');
    ...
    for (const action of autoActions) {
      ...
      try {
        ...
        if (result.trim()) {
          console.log(`   Output: ${result.trim().substring(0, 200)}${result.length > 200 ? '...' : ''}`);
        }
    ...
    const manualActions = queue.actions.filter(action => !action.autoExecute || action.riskLevel !== 'low');

    if (manualActions.length > 0) {
      console.log(`\n👤 **Manual Approval Required** (${manualActions.length} actions):`);
      manualActions.forEach((action, index) => {
        console.log(`   ${index + 1}. ${action.name} (${action.riskLevel} risk)`);
      });
  ...
  private displayApprovalCommands(queue: ImprovementQueue): void {
    console.log('\n💻 **Approval Commands**:');
    console.log('   npm run improve:approve <action-id>     # Approve specific action');
    console.log('   npm run improve:approve-all            # Approve all pending');
    console.log('   npm run improve:reject <action-id>      # Reject specific action');
    console.log('   npm run improve:status                 # View queue status');
    console.log('   npm run improve:clear                  # Clear completed actions');
  }
  ...
  async displayQueueStatus(): Promise<void> {
    try {
      ...
      const statusCount = queue.actions.reduce((acc, action) => {
    ...
    switch (command) {
      case 'analyze':
        await improver.analyzeAndTrigger({
          ...
          forceGrade: (args.find(arg => arg.startsWith('--grade='))?.split('=')[1] as any),
      ...
      default:
        console.log('Usage: npm run improve:analyze [--dry-run] [--grade=D] [--no-auto]');
