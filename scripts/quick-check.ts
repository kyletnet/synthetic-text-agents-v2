...
export class QuickCheck {
  ...
  async checkTypeScript(): Promise<CheckResult> {
    ...
    try {
      ...
      const designPrinciple = this.designPrincipleMapper.enhanceIssueWithDesignPrinciple(mockIssue);
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          // 구식 2-파라미터 패턴 검사
          const oldPatternMatches = content.match(/requestApproval\s*\([^{,)]+,\s*[^{)]+\)/g);
      ...
      if (issues.length > 0) {
        ...
        designPrinciples.push(this.designPrincipleMapper.enhanceIssueWithDesignPrinciple(mockIssue));
      }
      ...
      if (result.designPrinciples && result.designPrinciples.length > 0) {
        result.designPrinciples.forEach(dp => {
          if (dp.designPrinciple) {
            console.log(`   🎯 Violates: ${dp.designPrinciple.id} - ${dp.designPrinciple.name}`);
            console.log(`      Impact: ${dp.designPrinciple.architecturalImpact}`);
          }
        });
      }
    });
  ...
  async runCommand(command: string): Promise<void> {
    ...
    switch (command) {
      ...
      default:
        console.log('❌ Unknown command. Available: typescript, signatures, P0-only, node, quick, all');
        return;
    }
