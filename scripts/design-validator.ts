...
class DesignValidator {
  ...
  async validate(): Promise<{
    ...
    console.log(
      `   🟢 P2 Medium: ${this.violations.length - p0Count - p1Count}`,
    );
  ...
  private async checkNoHardcodedForcedExecution(): Promise<void> {
    ...
    for (const file of files) {
      ...
      if (
        content.match(/if\s*\(\s*task(Name|\.name)\s*===\s*['"][^'"]+['"]\s*\)/)
      ) {
        this.addViolation({
          file,
          ...
          suggestion:
            "Use priority-based scheduling instead of hardcoding task names",
        });
      }
    ...
    for (const file of files) {
      ...
      if (
        ...
        this.addViolation({
          ...
          suggestion:
            "Use processLifecycleManager.spawnManaged() to prevent orphan processes",
        });
      }
    }
  }
    ...
    for (const file of files) {
      ...
      if (
        ...
        this.addViolation({
          ...
          suggestion:
            "All LLM calls must go through LLMExecutionAuthority.authorizeExecution()",
        });
      }
    }
  }
    ...
    for (const file of files) {
      ...
      if (
        ...
        this.addViolation({
          ...
          suggestion:
            "Critical priority tasks must always execute regardless of time filters",
        });
      }
    }
  }
        ...
        if (
          ...
          if (
            ...
            this.addViolation({
              ...
              suggestion:
                "Use stdio:inherit for user-facing commands or parse the output",
            });
          }
        }
      }
    ...
    for (const file of files) {
      ...
      if (
        ...
        this.addViolation({
          ...
          suggestion:
            "Add dormant mode check at start of performAutomaticHealing()",
        });
      }
    }
  }
