...
class AutoIntegrationGuard {
  ...
  private async analyzeIntegrationConcerns(changes: any): Promise<NewFeatureImpact['integration_concerns']> {
    const concerns: NewFeatureImpact['integration_concerns'] = [];

    // 1. 중복 기능 검사
    if (changes.added.some((f: string) => f.includes('report') || f.includes('track'))) {
      concerns.push({
        type: 'DUPLICATION',
        severity: 'HIGH',
    ...
    if (changes.added.some((f: string) => f.includes('reports/'))) {
      concerns.push({
        type: 'CONFLICT',
        severity: 'LOW',
        description: 'reports/ 디렉토리에 새 파일 추가로 기존 보고서와 충돌 가능성',
  ...
  private determineAutoActions(changes: any, concerns: NewFeatureImpact['integration_concerns']): NewFeatureImpact['auto_actions'] {
  ...
  private printImpactReport(impact: NewFeatureImpact): void {
    ...
    if (impact.integration_concerns.length > 0) {
      ...
      impact.integration_concerns.forEach((concern, i) => {
        const icon = concern.severity === 'HIGH' ? '🔴' :
  ...
  private saveImpact(impact: NewFeatureImpact): void {
    const reportPath = join(this.projectRoot, 'reports/feature-impact-analysis.json');
  ...
  static async quickIntegrationCheck(): Promise<boolean> {
    ...
    const hasHighConcerns = impact.integration_concerns.some(c => c.severity === 'HIGH');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    default:
      console.log('Usage: tsx auto-integration-guard.ts <analyze|execute|quick>');
      process.exit(1);
  }
}
