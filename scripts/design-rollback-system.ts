...
class DesignRollbackSystem {
      ...
      if (existsSync(sourcePath)) {
        try {
          if (file.endsWith("/")) {
            ...
            mkdirSync(
              join(snapshotDir, "..", file.split("/").slice(0, -1).join("/")),
              { recursive: true },
            );
    ...
    try {
      ...
      if (existsSync(scriptsDir)) {
        const files = execSync(
          `find "${scriptsDir}" -name "*.ts" -o -name "*.js" -o -name "*.sh" | wc -l`,
          { encoding: "utf8" },
        );
  ...
  async executeRollback(snapshotId?: string): Promise<void> {
    ...
    // 최신 스냅샷 찾기
    if (!snapshotId) {
      const snapshots = execSync(`ls -t ${this.backupDir}`, {
        encoding: "utf8",
      })
        .trim()
        .split("\n")
    ...
    console.log(
      `🔙 Git 커밋: ${snapshot.pre_transition_state.git_commit_hash}`,
    );
    ...
    if (snapshot.pre_transition_state.git_commit_hash !== "unknown") {
      try {
        ...
        execSync(
          `git reset --hard ${snapshot.pre_transition_state.git_commit_hash}`,
          { stdio: "inherit" },
        );
    ...
    if (existsSync(snapshot.pre_transition_state.package_json_backup)) {
      cpSync(
        snapshot.pre_transition_state.package_json_backup,
    ...
    if (existsSync(snapshot.pre_transition_state.scripts_directory_backup)) {
      ...
      cpSync(
        snapshot.pre_transition_state.scripts_directory_backup,
        scriptsDir,
        { recursive: true },
      );
    ...
    try {
      ...
      console.log(
        `📊 명령어 개수: ${currentCommandCount} (원본: ${snapshot.pre_transition_state.command_count})`,
      );
  ...
  listSnapshots(): void {
    ...
    try {
      ...
      snapshots.forEach((snapshot, i) => {
        ...
        if (existsSync(metadataPath)) {
          const metadata: SystemSnapshot = JSON.parse(
            readFileSync(metadataPath, "utf8"),
          );
          console.log(`   ${i + 1}. ${snapshot}`);
          console.log(
            `      📅 생성: ${new Date(metadata.timestamp).toLocaleString()}`,
          );
          console.log(
            `      📊 명령어: ${metadata.pre_transition_state.command_count}개`,
          );
        }
      });
    } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    default:
      console.log(
        "Usage: tsx design-rollback-system.ts <snapshot|rollback [id]|list>",
      );
      process.exit(1);
  }
}
