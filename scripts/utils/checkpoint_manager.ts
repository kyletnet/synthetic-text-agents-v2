...
export class CheckpointManager {
  ...
  constructor(runId: string, sessionId: string, checkpointDir: string = 'reports/checkpoints') {
    this.runId = runId;
   ...
   */
  async recordCheckpoint(checkpoint: Omit<CheckpointEntry, 'timestamp'>): Promise<void> {
    ...
    try {
      ...
      console.log(`[CHECKPOINT] Recorded: ${checkpoint.agent_role}:${checkpoint.item_id} (${checkpoint.stage}) - Success: ${checkpoint.success}`);
  ...
  getResumeInfo(): ResumeInfo | null {
    ...
    try {
      ...
      const checkpoints: CheckpointEntry[] = lines.map(line => JSON.parse(line));
      ...
      for (const checkpoint of checkpoints) {
        ...
        // Track all results
        if (checkpoint.success && checkpoint.resumable_state.intermediate_results) {
          allResults.push({
        ...
        if (checkpoint.success) {
          ...
          if (!existing || checkpoint.checkpoint_index > existing.checkpoint_index) {
      ...
      const lastSuccessfulIndex = successfulIndices.length > 0 ? Math.min(...successfulIndices) : -1;
      const totalItems = checkpoints.length > 0 ? Math.max(...checkpoints.map(cp => cp.total_items)) : 0;

      return {
        last_successful_index: lastSuccessfulIndex,
        total_completed: successfulIndices.length,
        remaining_items: totalItems > lastSuccessfulIndex + 1 ? totalItems - (lastSuccessfulIndex + 1) : 0,
   ...
   */
  streamCheckpoints(callback: (checkpoint: CheckpointEntry) => void): () => void {
    if (!fs.existsSync(this.checkpointPath)) {
    ...
    const watcher = fs.watchFile(this.checkpointPath, { interval: 1000 }, () => {
      try {
          ...
          for (const line of lines) {
            if (line.trim()) {
              try {
                ...
                console.warn(`[CHECKPOINT] Failed to parse checkpoint line: ${parseError}`);
              }
            }
          }
        });
  ...
  archiveCheckpoints(): void {
    ...
    try {
      const archiveDir = path.join(path.dirname(this.checkpointPath), 'archived');
      fs.mkdirSync(archiveDir, { recursive: true });

      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const archivePath = path.join(archiveDir, `checkpoint_${this.runId}_${timestamp}.jsonl`);
  ...
  generateSummaryReport(): {
    ...
    agents_summary: Record<string, { total: number; successful: number; avg_cost: number }>;
    ...
    try {
      ...
      const successfulCheckpoints = checkpoints.filter(cp => cp.success).length;
      const failedCheckpoints = totalCheckpoints - successfulCheckpoints;

      // Agent-level summaries
      const agentsSummary: Record<string, { total: number; successful: number; avg_cost: number }> = {};
  ...
  async resumeExecution<T>(
    items: T[],
    processItem: (item: T, index: number, resumeState?: any) => Promise<{
      success: boolean;
      result?: any;
      error?: any;
      checkpoint?: {
    ...
    console.log(`[CHECKPOINT] Resuming execution from index ${startIndex} (${items.length - startIndex} remaining)`);
      ...
      try {
        ...
        await this.recordCheckpoint({
          ...
          error_info: itemResult.error ? {
            ...
            error_message: itemResult.error.message || String(itemResult.error),
        ...
        failedCount++;
        console.error(`[CHECKPOINT] Unexpected error processing item ${i}: ${error}`);
  ...
  private findResumeStateForItem(itemIndex: number, agentRole: string): any | null {
    const resumeInfo = this.getResumeInfo();
    if (!resumeInfo) return null;

    const relevantResult = resumeInfo.partial_results.find(
      result => result.item_id === `item_${itemIndex}` && result.agent_role === agentRole
    );
  ...
  if (command === 'summary') {
    ...
    const processItem = async (item: string, index: number) => {
      ...
      return {
        ...
        error: success ? null : { type: 'processing_error', message: `Failed to process ${item}` },
        checkpoint: {
    ...
    checkpointManager.resumeExecution(testItems, processItem, 'test_agent')
      .then(result => {
    ...
    console.log('  summary [run_id] [session_id]     - Generate checkpoint summary');
    console.log('  resume-info [run_id] [session_id] - Show resume information');
