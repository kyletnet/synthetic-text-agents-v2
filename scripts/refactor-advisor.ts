...
class RefactorAdvisor {
  ...
  async provideSuggestions(): Promise<RefactorSuggestion[]> {
    try {
      ...
      if (daysSinceUpdate > 1) {
        console.log(`⚠️  Learning data is ${daysSinceUpdate} days old. Consider running npm run advisor:learn`);
      }
      ...
      console.log('📚 No learning data found. Run npm run advisor:learn first.');
      return [];
    }
  }
  ...
  private extractPatternsFromErrors(errors: any[], patterns: Map<string, ErrorPattern>): void {
  ...
  private extractPatternsFromFixAttempts(attempts: any[], patterns: Map<string, ErrorPattern>): void {
  ...
  private categorizeErrorType(errorCode: string): string {
    if (['TS2345', 'TS2339', 'TS2304'].includes(errorCode)) return 'type-system';
  ...
  private async generateSmartSuggestions(patterns: ErrorPattern[]): Promise<RefactorSuggestion[]> {
  ...
  private createPatternBasedSuggestion(pattern: ErrorPattern): RefactorSuggestion | null {
  ...
  private async analyzeCodebaseForImprovements(): Promise<RefactorSuggestion[]> {
    const suggestions: RefactorSuggestion[] = [];
  ...
  private async suggestStructuralImprovements(): Promise<RefactorSuggestion[]> {
    ...
    for (const file of tsFiles.slice(0, 10)) { // 샘플링
      try {
        const content = await fs.readFile(join(this.projectRoot, file), 'utf-8');
...
if (obj.config) {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          if (inFunction) {
            ...
            if (line.includes('}') && (functionLength > 50 || conditionCount > 5)) {
  ...
  private async calculateProjectStats(): Promise<LearningData['projectStats']> {
    ...
    try {
      const fixReport = JSON.parse(
        await fs.readFile(join(this.projectRoot, 'reports/ai-fix-report.json'), 'utf-8')
      );
  ...
  async generateReport(): Promise<void> {
    ...
    if (suggestions.length === 0) {
      console.log('📚 No suggestions available. Run npm run advisor:learn first.');
      return;
    }
    ...
    for (const [category, categorySuggestions] of Object.entries(byCategory)) {
      ...
      for (const suggestion of categorySuggestions.slice(0, 3)) {
        ...
        console.log(`   ${suggestion.confidence >= 0.7 ? '🎯' : suggestion.confidence >= 0.5 ? '💡' : '🤔'} ${suggestion.title}`);
        console.log(`      Confidence: ${confidenceBar} ${(suggestion.confidence * 100).toFixed(0)}%`);
        console.log(`      Effort: ${suggestion.effort}, Impact: ${suggestion.impact}`);
    ...
    for (let i = 0; i < topSuggestions.length; i++) {
      ...
      console.log(`   ${i + 1}. ${suggestion.title} (${(suggestion.confidence * 100).toFixed(0)}% confidence)`);
