...
class SmartApprovalSystem {
  ...
  async assessRisk(analysisResults: any): Promise<RiskAssessment> {
    ...
    const criticalFiles = [
      ...
      '.claude/system-mode.yaml', 'scripts/unified-dashboard.ts'
    ];
    if (analysisResults.changes?.files?.some((f: string) =>
  ...
  displayApprovalRequest(assessment: RiskAssessment): void {
    ...
    if (assessment.requiresExpertConsultation) {
      ...
      console.log('🔍 반드시 다른 LLM(Claude, GPT 등)이나 개발자에게 먼저 상의하세요.');
      console.log('📋 위의 이슈들을 복사해서 전문가에게 물어보신 후 진행하세요.');
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    }
