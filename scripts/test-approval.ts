...
function collectPendingApprovals(): PendingApproval[] {
  ...
  try {
    ...
    if (existsSync(evolutionReportPath)) {
      ...
      console.log('🔍 autoEvolutionCapabilities exists:', !!report.autoEvolutionCapabilities);
      console.log('🔍 needsApproval exists:', !!report.autoEvolutionCapabilities?.needsApproval);
      console.log('🔍 needsApproval length:', report.autoEvolutionCapabilities?.needsApproval?.length || 0);
