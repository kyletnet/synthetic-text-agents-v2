...
interface TrackedIssue {
  id: string;
  timestamp: string;
  category: 'PERFORMANCE' | 'ERROR' | 'WARNING' | 'WORKAROUND' | 'DEPENDENCY' | 'CONFIG';
...
class IssueTracker {
  ...
  trackIssue(issue: Omit<TrackedIssue, 'id' | 'timestamp' | 'occurrence' | 'firstSeen' | 'lastSeen'>): void {
  ...
  generateReport(): IssueReport {
    ...
    const highPriorityIssues = this.issues.filter(i => ['P0', 'P1'].includes(i.severity) && i.status === 'ACTIVE');
  ...
  printReport(): void {
    ...
    console.log('\n🔍 임시 처리 이슈 추적 보고서');
    console.log('==============================');
    console.log(`📊 총 이슈: ${report.totalIssues} | 활성: ${report.activeIssues} | 신규: ${report.newIssues} | 우선순위 높음: ${report.highPriorityIssues}`);
    ...
    if (report.issues.length > 0) {
      ...
      report.issues.forEach((issue, i) => {
        ...
        console.log(`   ${i + 1}. ${icon} [${issue.category}] ${issue.title}${freq}`);
      ...
      if (p2Count > 2) {
        console.log(`   2. 🟡 P2 이슈 ${p2Count}개 중 자주 발생하는 것부터 해결`);
      }
  ...
  autoDetectIssues(commandOutput: string, command: string): void {
    // SIGPIPE 오류 감지
    if (commandOutput.includes('SIGPIPE') || commandOutput.includes('Broken pipe')) {
      this.trackIssue({
        category: 'ERROR',
        severity: 'P2',
        title: 'SIGPIPE 오류 발생',
    ...
    // node_modules 처리 감지
    if (commandOutput.includes('node_modules') && (command.includes('find') || command.includes('grep'))) {
      this.trackIssue({
        category: 'PERFORMANCE',
        severity: 'P1',
        title: 'node_modules 불필요 처리',
        description: '스크립트가 node_modules 내부 파일들까지 처리하여 속도 저하',
        temporarySolution: 'node_modules 제외 옵션 수동 추가',
        properSolution: '모든 검색/처리 스크립트에 --exclude node_modules 자동 적용',
    ...
    if (commandOutput.includes('error TS') || commandOutput.includes('Found 0 errors')) {
    ...
    // 권한 오류 감지
    if (commandOutput.includes('Permission denied') || commandOutput.includes('EACCES')) {
      this.trackIssue({
        category: 'CONFIG',
        severity: 'P1',
        title: '권한 오류',
  ...
  listActiveIssues(): void {
    ...
    activeIssues.forEach((issue, i) => {
      console.log(`   ${i + 1}. [${issue.id}] ${issue.title} (${issue.severity})`);
    });
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    default:
      console.log('Usage: tsx issue-tracker.ts <report|list|resolve <id>|track <title> <description>>');
      process.exit(1);
  }
}
