...
class SecurityAuditChecker {
  ...
  async runSecurityAudit(): Promise<SecurityReport> {
    ...
    const report: SecurityReport = {
      ...
      overallStatus: blockingIssues.length > 0 ? 'FAIL' :
  ...
  private async checkForSecrets(): Promise<SecurityIssue[]> {
    ...
    try {
      ...
      const secretPatterns = [
        ...
        { pattern: 'xoxb-[0-9]{12}-[0-9]{12}-[a-zA-Z0-9]{24}', name: 'Slack Bot Token' },
      ...
      for (const { pattern, name } of secretPatterns) {
        try {
          const result = execSync(`rg "${pattern}" src/ scripts/ --type ts --type js --type json || true`,
            { encoding: 'utf8' });
            ...
            for (const line of lines) {
              if (line.includes(':')) {
                ...
                issues.push({
                  ...
                  file,
                  solution: 'Move to environment variables or secure secret manager',
                  blocking: true
                });
              }
            }
      ...
      if (audit.vulnerabilities) {
        Object.entries(audit.vulnerabilities).forEach(([pkg, vuln]: [string, any]) => {
          ...
          issues.push({
            ...
            solution: vuln.fixAvailable ? 'Run npm audit fix' : 'Update dependency manually',
  ...
  private async checkCodePatterns(): Promise<SecurityIssue[]> {
    ...
    try {
      ...
      const hardcodedEndpoints = execSync(`rg "https?://[a-zA-Z0-9.-]+\\.[a-zA-Z]{2,}" src/ --type ts --type js || true`,
        { encoding: 'utf8' });
  ...
  private printReport(report: SecurityReport): void {
    ...
    console.log(`📊 Overall Status: ${report.overallStatus === 'PASS' ? '✅ PASS' :
                                      report.overallStatus === 'WARN' ? '⚠️ WARN' :
                                      '❌ FAIL'}`);
