...
class TypeScriptCompileChecker {
  ...
  private generateSmartRecommendations(errors: TSError[]): string[] {
    ...
    for (const [pattern, patternErrors] of Object.entries(errorGroups)) {
      const refactorPrompt = this.generateRefactorPrompt(
        pattern,
        patternErrors,
      );
      if (refactorPrompt) {
  ...
  private generateRecommendations(errors: TSError[]): string[] {
      ...
      topErrors.forEach(([code, count]) => {
        ...
        if (code === "TS2345") {
          recommendations.push(
            "     💡 Fix: Check argument types and interface compatibility",
          );
        } else if (code === "TS2339") {
          recommendations.push(
            "     💡 Fix: Verify property names and interface definitions",
          );
        } else if (code === "TS2304") {
          recommendations.push(
            "     💡 Fix: Add missing imports or check variable declarations",
          );
        }
      });
    }
    ...
    if (errors.some((e) => e.code === "TS1005")) {
      recommendations.push(
        "🚨 Immediate: Add missing semicolons or fix syntax",
      );
    }
  ...
  private groupErrorsByPattern(errors: TSError[]): Record<string, TSError[]> {
    ...
    for (const error of errors) {
      ...
      if (error.code === "TS2345" && error.message.includes("not assignable")) {
        ...
        error.code === "TS2353" &&
        error.message.includes("unknown properties")
      ) {
  ...
  private generateRefactorPrompt(
   ...
   ${errors
     .map((e) => {
       const name =
         e.message.match(/Cannot find name '([^']+)'/)?.[1] || "UnknownType";
  ...
  async generateReport(report: CompileReport): Promise<void> {
    ...
    if (report.summary.criticalErrors > 0) {
      ...
      if (criticalErrors.length > 10) {
        console.log(
          `   ... and ${criticalErrors.length - 10} more critical errors`,
        );
      }
    }
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.log(
        "Usage: tsx scripts/ts-compile-checker.ts [check|refactor-issues]",
      );
  }
}
