...
export interface AuditInput {
  ...
  focus_areas?: Array<
    "accuracy" | "completeness" | "clarity" | "citations" | "bias"
  >;
}
      ...
      const output: AuditOutput = {
        ...
        recommendations: {
          ...
          evidence_gap_filling:
            fallbackAudit.result === "FAIL"
              ? ["Consider additional evidence"]
              : [],
          quality_improvement_notes: [
            "Audit performed in fallback mode with limited analysis",
          ],
        },
      };
...
Evidence Used:
${input.evidence.evidence_items
  .map((e, idx) => `${idx + 1}. ${e.text} (Relevance: ${e.relevance_score})`)
  .join("\n")}
...
Respond with only a number between 0 and 100.`;
      ...
      const result = await callAnthropic(
        {
          ...
          max_tokens: 50,
          system:
            "You are an expert fact-checker. Respond with only a number representing accuracy percentage.",
  ...
  private async auditCompleteness(input: AuditInput): Promise<number> {
    ...
    const answerCoverage = this.analyzeAnswerCoverage(
      input.answer.answer.text,
      questionComplexity,
    );
    const evidenceUtilization = this.analyzeEvidenceUtilization(
      input.answer,
      input.evidence,
    );
    ...
    return Math.round(
      answerCoverage * 0.5 +
        evidenceUtilization * 0.3 +
        questionComplexity.covered * 0.2,
    );
  }
  ...
  private async auditClarity(input: AuditInput): Promise<number> {
    ...
    if (
      answerText.toLowerCase().includes("in conclusion") ||
  ...
  private async auditBias(input: AuditInput): Promise<number> {
    ...
    const neutralCount = neutralLanguage.filter((indicator) =>
      answerText.includes(indicator),
    ).length;
  ...
  private async checkQualityGates(
    input: AuditInput,
  ): Promise<AuditOutput["quality_gates"]> {
    return {
      ...
      citation_integrity_gate: this.validateCitationIntegrity(
        input.answer,
        input.evidence,
      ),
      harmful_content_gate: await this.checkHarmfulContent(
        input.answer.answer.text,
      ),
    };
  }
  ...
  private async generateFindings(
    ...
    if (scores.accuracy < 70) {
      findings.push({
        ...
        suggested_improvement:
          "Review answer claims against provided evidence and strengthen factual support.",
      });
    }
    ...
    if (scores.completeness < 60) {
      findings.push({
        ...
        suggested_improvement:
          "Ensure all parts of the question are addressed comprehensively.",
      });
    }
    ...
    if (scores.citation_quality < 50) {
      findings.push({
        ...
        suggested_improvement:
          "Add more specific citations and ensure they support the claims made.",
      });
    }
    ...
    if (!gates.factual_accuracy_gate) {
      findings.push({
        ...
        severity: "high",
        description:
          "Answer confidence is too low, indicating potential factual issues.",
        suggested_improvement:
          "Verify facts and strengthen evidence base before publication.",
      });
    }
    ...
    if (!gates.evidence_support_gate) {
      findings.push({
        ...
        suggested_improvement:
          "Add citations to support all major claims in the answer.",
      });
    }
    ...
    if (scores.accuracy > 85) {
      findings.push({
        ...
        severity: "low",
        description:
          "Answer demonstrates high factual accuracy with strong evidence support.",
      });
    }
  ...
  private async generateRecommendations(
    ...
    return {
      answer_revision_needed:
        result === "FAIL" || highSeverityFindings.length > 0,
  ...
  private performFallbackAudit(input: AuditInput): {
    ...
    const citationScore = hasCitations
      ? Math.min(citations.length * 25, 80)
      : 20;
    ...
    const findings: AuditOutput["findings"] = [
      {
        ...
        severity: "low",
        description:
          "Audit performed in fallback mode with limited analysis capabilities.",
      },
    ];
  ...
  private analyzeQuestionComplexity(question: string): {
    aspects: string[];
    covered: number;
  } {
    const questionWords = [
    ...
    const aspects = questionWords.filter((word) =>
  ...
  private analyzeEvidenceUtilization(
    answer: AnswerOutput,
    evidence: EvidenceOutput,
  ): number {
  ...
  private validateCitationIntegrity(
    answer: AnswerOutput,
    evidence: EvidenceOutput,
  ): boolean {
  ...
  private estimateAuditCost(depth: string, dimensionCount: number): number {
    ...
    const depthMultiplier =
      depth === "comprehensive" ? 2 : depth === "standard" ? 1.5 : 1;
