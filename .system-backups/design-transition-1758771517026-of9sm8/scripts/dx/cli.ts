  ...
  try {
    ...
    try {
      ...
      if (!collection.session_report) {
        console.error(
          "❌ No session report found - ensure a run has completed first",
        );
        return false;
      }
      ...
      console.log(
        `✅ Found session data: ${collection.session_report.cases_total} cases`,
      );
    } catch (error) {
    ...
    if (budget > 0) {
      try {
        ...
        if (budget > profile_config.budget_max_usd) {
          console.error(
            `❌ Budget ${budget} exceeds profile limit ${profile_config.budget_max_usd} for ${profile}`,
          );
          return false;
        }
...
async function runDiagnosticLoop(options: {
  ...
  console.log(
    `Profile: ${options.profile} | Budget: $${options.budget} | Auto-cal: ${options.autocalib}`,
  );
  ...
  try {
    ...
    const { config: dxConfig, profile_config } = await loadThresholds(
      options.profile,
    );
    ...
    if (options.autocalib && dxConfig.autocalibration.enabled) {
      calibrationChanges = await autoCalibrateThresholds(
        dxConfig,
        options.profile,
      );
      ...
      if (calibrationChanges.length > 0) {
        console.log(
          `Found ${calibrationChanges.length} threshold calibration opportunities`,
        );

        if (options.approve) {
          const { updatedConfig: newConfig, appliedChanges } =
            applyCalibrations(
              dxConfig,
              calibrationChanges,
          ...
          console.log(
            "Threshold changes proposed but not approved (use --approve to apply)",
          );
        }
      }
    } else {
    ...
    if (calibrationChanges.length > 0) {
      ...
      console.log(
        `   ${options.approve ? "✅ Applied" : "⏳ Pending approval"}`,
      );
    }

    console.log(`\n🕐 Execution Time: ${(duration / 1000).toFixed(1)}s`);
    console.log(
      `🎯 Recommendation: ${gating.can_proceed ? "PROCEED" : "BLOCK"} full run`,
    );
...
async function reportMode(): Promise<void> {
  try {
    ...
    console.log(
      `${gatingSummary.status_emoji} Status: ${report.summary.overall_status}`,
    );
    ...
    if (report.summary.top_issues.length > 0) {
      console.log(`\n⚠️ Issues:`);
      report.summary.top_issues.forEach((issue) =>
        console.log(`   • ${issue}`),
      );
    }
