...
export class PromptArchitect extends BaseAgent {
  constructor() { 
    super('prompt-architect', 'PromptArchitect', ['prompt-engineering', 'rules-generation'], new Logger()); 
  }
  ...
  protected async handle(content: unknown, context?: AgentContext): Promise<unknown> {
    ...
    const topicTerms: string[] = (shared['topic-terms'] as any)?.result?.terms || []; // stub
    const fbText: string = (shared['feedback-latest'] as any)?.result?.text || '';    // stub
    ...
    const base = [
      ...
      { type:'tone_primary', value:'elementary', reason:'초기 기본값' } as const,
      { type:'vary_sentence_forms', value:true, reason:'초기 기본값' } as const,
      { type:'must_include_topic_terms', value: topicTerms.slice(0,3), reason:'예제 키워드 사용' } as const
    ] as any[];
