...
export class PsychologySpecialist extends BaseAgent {
  constructor(logger: Logger) {
    super(
      ...
      ['psychology', 'user-behavior', 'communication', 'motivation', 'empathy', 'persuasion'],
      logger
    );
  }

  protected async handle(content: unknown, context?: AgentContext): Promise<PsychologySpecialistOutput> {
    ...
    const implementationGuidance = await this.createImplementationGuidance(analysis, request);
  ...
  private async conductPsychologyAnalysis(
    ...
    const persuasionPsychology = await this.analyzePersuasionFactors(request, userPsychology);
    const cognitiveConsiderations = await this.analyzeCognitiveFactors(request, userPsychology);
  ...
  private async analyzeUserPsychology(request: PsychologyAnalysisRequest) {
    ...
    const emotionalState = this.determineEmotionalState(domain, contextualFactors);
    
    // Identify primary motivations
    const primaryMotivations = this.identifyMotivations(domain, request.taskDescription);
    
    // Assess cognitive load
    const cognitiveLoad = this.assessCognitiveLoad(request.taskDescription, contextualFactors.experienceLevel);
    
    // Determine decision-making style
    const decisionMakingStyle = this.analyzeDecisionMakingStyle(domain, contextualFactors);
    
    // Identify stress factors
    const stressFactors = this.identifyStressFactors(domain, contextualFactors);
    
    // Assess confidence levels
    const confidenceLevels = this.assessConfidenceLevels(domain, contextualFactors.experienceLevel);
  ...
  private determineEmotionalState(domain: string, contextualFactors: any): string {
    const domainEmotions: Record<string, string[]> = {
      'customer_service': ['frustrated', 'impatient', 'seeking_resolution', 'anxious'],
      'sales': ['skeptical', 'evaluating', 'pressured', 'hopeful'],
      'marketing': ['curious', 'evaluating_options', 'comparison_mode', 'research_focused'],
    ...
    const baseEmotions = domainEmotions[domain] || ['neutral', 'information_seeking'];
    
    // Modify based on contextual factors
    if (contextualFactors.urgency === 'high') {
      return baseEmotions.includes('anxious') ? 'highly_anxious' : 'urgent_and_focused';
    }
  ...
  private identifyMotivations(domain: string, taskDescription: string): string[] {
  ...
  private assessCognitiveLoad(taskDescription: string, experienceLevel?: string): 'low' | 'medium' | 'high' {
    let loadScore = 0;

    // Complexity indicators in task description
    const complexityWords = ['complex', 'multiple', 'various', 'comprehensive', 'detailed'];
    loadScore += complexityWords.filter(word => 
      taskDescription.toLowerCase().includes(word)
    ).length;

    // Technical terms increase cognitive load
    const technicalWords = ['implementation', 'integration', 'optimization', 'configuration'];
  ...
  private analyzeDecisionMakingStyle(domain: string, contextualFactors: any): PsychologyInsights['userPsychology']['decisionMakingStyle'] {
    const domainStyles: Record<string, PsychologyInsights['userPsychology']['decisionMakingStyle']> = {
      'finance': 'analytical',
      'legal': 'analytical',
  ...
  private identifyStressFactors(domain: string, contextualFactors: any): string[] {
    const domainStressors: Record<string, string[]> = {
      'customer_service': ['time_pressure', 'angry_customers', 'complex_issues', 'performance_metrics'],
      'sales': ['quota_pressure', 'rejection', 'competition', 'decision_delays'],
      'marketing': ['campaign_performance', 'budget_constraints', 'changing_trends', 'attribution_challenges'],
      'healthcare': ['patient_safety', 'regulatory_compliance', 'resource_constraints', 'life_impact'],
      'finance': ['regulatory_requirements', 'market_volatility', 'accuracy_demands', 'audit_pressure'],
    };

    const baseStressors = domainStressors[domain] || ['performance_pressure', 'complexity', 'time_constraints'];
  ...
  private assessConfidenceLevels(domain: string, experienceLevel?: string): string {
    ...
    let baseLevel = baseLevels[domain] || 'moderate_confidence_information_seeking';
  ...
  private async developCommunicationStrategy(
    ...
    const optimalTone = this.determineTone(userPsychology, request.targetDomain);
    const languageStyle = this.selectLanguageStyle(userPsychology, request.contextualFactors?.experienceLevel);
    ...
    const urgencyHandling = this.developUrgencyHandling(userPsychology, request.contextualFactors?.urgency);
  ...
  private determineTone(userPsychology: PsychologyInsights['userPsychology'], domain: string): PsychologyInsights['communicationStrategy']['optimalTone'] {
    // High stress or anxiety = supportive tone
    if (userPsychology.stressFactors.length > 3 || userPsychology.emotionalState.includes('anxious')) {
      return 'supportive';
    }
    ...
    const domainTones: Record<string, PsychologyInsights['communicationStrategy']['optimalTone']> = {
  ...
  private selectLanguageStyle(
    ...
    if (experienceLevel === 'beginner' || userPsychology.cognitiveLoad === 'high') {
      return 'simplified';
    }
    
    if (experienceLevel === 'expert' && userPsychology.decisionMakingStyle === 'analytical') {
      return 'technical';
    }
    
    if (userPsychology.emotionalState.includes('frustrated') || userPsychology.emotionalState.includes('anxious')) {
  ...
  private selectStructuralApproach(userPsychology: PsychologyInsights['userPsychology']): PsychologyInsights['communicationStrategy']['structuralApproach'] {
    if (userPsychology.cognitiveLoad === 'high' || userPsychology.decisionMakingStyle === 'directive') {
      return 'step-by-step';
    }
  ...
  private determineEmpathyLevel(userPsychology: PsychologyInsights['userPsychology']): PsychologyInsights['communicationStrategy']['empathyLevel'] {
    const stressScore = userPsychology.stressFactors.length;
    const hasEmotionalDistress = userPsychology.emotionalState.includes('anxious') || 
  ...
  private developUrgencyHandling(
    ...
    if (urgencyLevel === 'high' || userPsychology.emotionalState.includes('urgent')) {
      return 'Acknowledge urgency immediately, provide immediate actionable steps, include timeframes';
    }
    
    if (urgencyLevel === 'medium' || userPsychology.stressFactors.includes('time_pressure')) {
  ...
  private async analyzePersuasionFactors(
    ...
    const keyInfluencers = this.identifyInfluencers(request.targetDomain, userPsychology);
    const resistancePoints = this.identifyResistancePoints(userPsychology, request);
    const motivationalFrames = this.developMotivationalFrames(userPsychology.primaryMotivations);
    const trustBuilders = this.identifyTrustBuilders(request.targetDomain, userPsychology);
  ...
  private identifyInfluencers(domain: string, userPsychology: PsychologyInsights['userPsychology']): string[] {
    const domainInfluencers: Record<string, string[]> = {
      'customer_service': ['empathy', 'quick_resolution', 'competence_demonstration', 'follow_up_commitment'],
      'sales': ['social_proof', 'risk_reduction', 'value_demonstration', 'scarcity_urgency'],
      'marketing': ['data_evidence', 'case_studies', 'industry_benchmarks', 'thought_leadership'],
      'healthcare': ['clinical_evidence', 'safety_assurance', 'expert_endorsement', 'patient_testimonials'],
      'finance': ['regulatory_compliance', 'risk_mitigation', 'performance_data', 'expert_analysis'],
    };

    const baseInfluencers = domainInfluencers[domain] || ['expertise', 'evidence', 'credibility', 'practical_value'];
  ...
  private identifyResistancePoints(
    ...
    const domainResistance: Record<string, string[]> = {
      ...
      'customer_service': ['scripted_responses', 'deflection_attempts', 'impersonal_service'],
      'marketing': ['generic_advice', 'untested_strategies', 'resource_intensive_solutions'],
    };
  ...
  private developMotivationalFrames(primaryMotivations: string[]): string[] {
    ...
    return primaryMotivations.map(motivation => 
      frameMap[motivation] || 'Value and benefit realization'
    );
  }

  private identifyTrustBuilders(domain: string, userPsychology: PsychologyInsights['userPsychology']): string[] {
    const domainTrustBuilders: Record<string, string[]> = {
      'customer_service': ['active_listening', 'problem_ownership', 'follow_through', 'transparency'],
      'sales': ['honest_communication', 'customer_references', 'proven_track_record', 'no_pressure_approach'],
      'marketing': ['data_transparency', 'methodology_explanation', 'realistic_expectations', 'case_study_evidence'],
      'healthcare': ['credentials_display', 'evidence_based_recommendations', 'patient_safety_priority', 'informed_consent'],
      'finance': ['regulatory_compliance', 'risk_disclosure', 'track_record', 'third_party_validation'],
    };

    const baseTrustBuilders = domainTrustBuilders[domain] || ['expertise_demonstration', 'transparency', 'reliability', 'competence'];
    
    // Add psychology-specific trust builders
    if (userPsychology.emotionalState.includes('anxious') || userPsychology.emotionalState.includes('cautious')) {
  ...
  private async analyzeCognitiveFactors(
    ...
    const informationProcessing = this.analyzeInformationProcessing(userPsychology);
    const attentionSpan = this.assessAttentionSpan(userPsychology, request.contextualFactors);
    const learningStyle = this.determineLearningStyle(request.targetDomain, userPsychology);
  ...
  private analyzeInformationProcessing(userPsychology: PsychologyInsights['userPsychology']): string {
    if (userPsychology.cognitiveLoad === 'high') {
    ...
    if (userPsychology.emotionalState.includes('urgent') || userPsychology.stressFactors.includes('time_pressure')) {
  ...
  private assessAttentionSpan(
    ...
    if (userPsychology.stressFactors.length > 3 || userPsychology.cognitiveLoad === 'high') {
      return 'short';
    }
    ...
    if (userPsychology.decisionMakingStyle === 'analytical' && userPsychology.emotionalState.includes('focused')) {
      return 'extended';
    }
  ...
  private determineLearningStyle(domain: string, userPsychology: PsychologyInsights['userPsychology']): PsychologyInsights['cognitiveConsiderations']['learningStyle'] {
    const domainStyles: Record<string, PsychologyInsights['cognitiveConsiderations']['learningStyle']> = {
  ...
  private identifyMemoryAids(
    ...
    switch (learningStyle) {
      case 'visual':
        memoryAids.push('diagrams', 'flowcharts', 'bullet_points', 'visual_metaphors');
        break;
      case 'auditory':
        memoryAids.push('verbal_repetition', 'mnemonics', 'rhythm_patterns', 'discussion_points');
        break;
      case 'kinesthetic':
        memoryAids.push('step_by_step_actions', 'practice_examples', 'hands_on_exercises');
        break;
      case 'reading':
        memoryAids.push('written_summaries', 'key_point_lists', 'structured_outlines', 'reference_materials');
        break;
    }
    
    // Add cognitive load considerations
    if (userPsychology.cognitiveLoad === 'high') {
      memoryAids.push('chunking_strategies', 'simple_frameworks', 'repetition_emphasis');
    }
    
    // Add stress-based aids
    if (userPsychology.stressFactors.length > 2) {
      memoryAids.push('stress_reduction_cues', 'confidence_boosters', 'success_reminders');
    }
  ...
  private async generateRecommendations(
    ...
    const communicationGuidelines = this.generateCommunicationGuidelines(analysis);
    const emotionalConsiderations = this.generateEmotionalConsiderations(analysis);
  ...
  private generateQADesignPrinciples(analysis: PsychologyInsights): string[] {
    ...
    if (analysis.userPsychology.decisionMakingStyle === 'collaborative') {
      ...
      principles.push('Provide clear, actionable questions with definitive answers');
    ...
    if (analysis.userPsychology.cognitiveLoad === 'high') {
      principles.push('Break complex topics into smaller, manageable questions');
  ...
  private generateCommunicationGuidelines(analysis: PsychologyInsights): string[] {
    const guidelines: string[] = [];
    
    // Tone guidelines
    guidelines.push(`Maintain ${analysis.communicationStrategy.optimalTone} tone throughout responses`);
    guidelines.push(`Use ${analysis.communicationStrategy.languageStyle} language style`);
    
    // Empathy level
    if (analysis.communicationStrategy.empathyLevel === 'high') {
      guidelines.push('Include empathetic acknowledgment of challenges and concerns');
      guidelines.push('Use validating language that recognizes emotional state');
    }
    
    // Structure approach
    guidelines.push(`Organize responses using ${analysis.communicationStrategy.structuralApproach} structure`);
  ...
  private generateEmotionalConsiderations(analysis: PsychologyInsights): string[] {
    ...
    if (emotionalState.includes('anxious')) {
      considerations.push('Provide reassurance and confidence-building elements');
    ...
    for (const stressFactor of analysis.userPsychology.stressFactors) {
      if (stressFactor === 'time_pressure') {
        ...
        considerations.push('Include success indicators and confidence boosters');
      }
    }
    
    // Confidence level support
    if (analysis.userPsychology.confidenceLevels.includes('low_confidence')) {
      considerations.push('Provide step-by-step guidance with confidence checkpoints');
  ...
  private generateAvoidanceStrategies(analysis: PsychologyInsights): string[] {
    ...
    for (const resistancePoint of analysis.persuasionPsychology.resistancePoints) {
      if (resistancePoint === 'information_overwhelm') {
        ...
        strategies.push('Include evidence and reasoning for all recommendations');
      }
    }
    ...
    if (analysis.communicationStrategy.empathyLevel === 'high') {
      ...
      strategies.push('Avoid purely technical responses without emotional consideration');
    }
  ...
  private generateQualityIndicators(analysis: PsychologyInsights): string[] {
    ...
    indicators.push(`Response tone matches ${analysis.communicationStrategy.optimalTone} preference`);
    indicators.push(`Language complexity appropriate for ${analysis.userPsychology.cognitiveLoad} cognitive load`);
    indicators.push(`Structure follows ${analysis.communicationStrategy.structuralApproach} organization`);
    
    // Emotional resonance indicators
    if (analysis.communicationStrategy.empathyLevel === 'high') {
      indicators.push('Includes empathetic acknowledgment and emotional validation');
    }
    ...
    indicators.push(`Matches ${analysis.cognitiveConsiderations.learningStyle} learning style preferences`);
    indicators.push('Includes relevant memory aids and retention supports');
    indicators.push(`Respects ${analysis.cognitiveConsiderations.attentionSpan} attention span limitations`);
  ...
  private generateQuestionFormulationGuidance(analysis: PsychologyInsights): string[] {
    ...
    if (analysis.userPsychology.decisionMakingStyle === 'collaborative') {
      ...
      guidance.push('Frame as collaborative exploration rather than individual challenge');
    }
  ...
  private generateAnswerToneGuidance(analysis: PsychologyInsights): string[] {
    ...
    switch (tone) {
      ...
      case 'collaborative':
        ...
        guidance.push('Acknowledge multiple perspectives and stakeholder needs');
        break;
      case 'reassuring':
        guidance.push('Provide calm, steady guidance');
        guidance.push('Include safety and security emphasis');
        guidance.push('Use language that reduces anxiety and builds confidence');
        break;
      case 'direct':
  ...
  private generateEmpathyIntegration(analysis: PsychologyInsights): string[] {
    ...
    if (analysis.communicationStrategy.empathyLevel === 'high') {
      integration.push('Begin responses with acknowledgment of challenges or concerns');
      integration.push('Use validating language that shows understanding');
      integration.push('Include emotional support alongside practical guidance');
      ...
      integration.push('Include brief acknowledgment of situational challenges');
      integration.push('Show awareness of practical constraints and pressures');
    } else {
      integration.push('Focus primarily on practical solutions with minimal emotional content');
  ...
  private generateMotivationalElements(analysis: PsychologyInsights): string[] {
    ...
    for (const motivation of analysis.userPsychology.primaryMotivations) {
      if (motivation === 'resolve_issue_quickly') {
        ...
        elements.push('Frame solutions as professionally sophisticated approaches');
        ...
        elements.push('Connect solutions to business outcomes and success metrics');
  ...
  private async identifyRiskFactors(
    ...
    if (analysis.userPsychology.cognitiveLoad === 'high') {
      riskFactors.push({
        ...
        mitigation: 'Limit information density, use progressive disclosure, provide clear next steps',
      });
    }
    ...
    if (analysis.userPsychology.emotionalState.includes('anxious')) {
      riskFactors.push({
        ...
        impact: 'high' as const,
        mitigation: 'Use calming language, provide reassurance, break down complex tasks',
      });
    }
    
    // Resistance point risks
    if (analysis.persuasionPsychology.resistancePoints.includes('credibility_doubts')) {
      riskFactors.push({
        factor: 'Rejection of recommendations due to trust deficits',
        impact: 'medium' as const,
        mitigation: 'Include evidence, provide transparent reasoning, demonstrate expertise',
      });
    }
    
    // Communication mismatch risks
    if (analysis.communicationStrategy.empathyLevel === 'high' && request.contextualFactors?.urgency === 'high') {
      riskFactors.push({
        factor: 'Conflict between empathy needs and urgency demands',
        impact: 'medium' as const,
        mitigation: 'Balance empathetic acknowledgment with rapid solution provision',
      });
    }
    ...
    if (analysis.cognitiveConsiderations.attentionSpan === 'short') {
      riskFactors.push({
        ...
        impact: 'medium' as const,
        mitigation: 'Front-load key information, use bullet points, provide summaries',
      });
    }
