...
export class CognitiveScientist extends BaseAgent {
  constructor(logger: Logger) {
    super(
      ...
      ['cognitive-modeling', 'expertise-transfer', 'learning-design', 'knowledge-elicitation'],
      logger
    );
  }

  protected async handle(content: unknown, context?: AgentContext): Promise<CognitiveScientistOutput> {
  ...
  private identifyMentalModels(request: CognitiveAnalysisRequest) {
    ...
    const domainModels: Record<string, any> = {
      'customer_service': {
        ...
        components: ['touchpoints', 'emotional states', 'pain points', 'expectations', 'outcomes'],
        relationships: ['sequential progression', 'emotional transitions', 'satisfaction drivers', 'service recovery']
      },
      'sales': {
        name: 'Sales Process Mental Model',
        description: 'Cognitive framework for managing sales opportunities',
        components: ['prospect qualification', 'need identification', 'solution mapping', 'objection handling', 'closing'],
        relationships: ['pipeline progression', 'stakeholder influence', 'value proposition alignment', 'timing optimization']
      },
      'marketing': {
        name: 'Market Dynamics Mental Model',
        description: 'Cognitive representation of market forces and customer behavior',
        components: ['market segments', 'customer personas', 'competitive landscape', 'value propositions', 'channels'],
        relationships: ['segment interactions', 'competitive responses', 'channel synergies', 'customer lifecycle']
      }
    };
    ...
    models.push({
      ...
      components: ['problem definition', 'root causes', 'solution options', 'implementation plan', 'success metrics'],
      relationships: ['causal chains', 'solution mapping', 'risk-benefit analysis', 'stakeholder impact']
    });

    models.push({
      name: 'Stakeholder Ecosystem Model',
      description: 'Understanding of stakeholder relationships and influences',
      components: ['primary stakeholders', 'secondary influencers', 'decision makers', 'implementation agents'],
      relationships: ['power dynamics', 'influence networks', 'communication flows', 'alignment factors']
    });
    ...
    if (request.expertiseLevel === 'expert' || request.expertiseLevel === 'specialist') {
      models.push({
        ...
        components: ['system boundaries', 'feedback loops', 'leverage points', 'emergent properties'],
        relationships: ['system interactions', 'dynamic equilibrium', 'change propagation', 'unintended consequences']
      });
    }
  ...
  private analyzeReasoningPatterns(request: CognitiveAnalysisRequest) {
    ...
    patterns.push({
      ...
      triggerConditions: ['familiar situation encountered', 'similar cases available', 'pattern cues present'],
      steps: [
      ...
      outputCharacteristics: ['rapid recognition', 'experience-based confidence', 'analogical solutions', 'contextual adaptations']
    });

    // Analytical Decomposition
    patterns.push({
      pattern: 'Systematic Decomposition Reasoning',
      triggerConditions: ['complex novel problems', 'high stakes decisions', 'multiple variables present'],
      steps: [
      ...
      outputCharacteristics: ['thorough analysis', 'logical structure', 'comprehensive coverage', 'systematic approach']
    });
    ...
    if (request.expertDomain === 'customer_service') {
      patterns.push({
        ...
        triggerConditions: ['customer distress detected', 'emotional context present', 'relationship factors important'],
        steps: [
        ...
        outputCharacteristics: ['emotional intelligence', 'relationship preservation', 'holistic solutions', 'trust building']
      });
    } else if (request.expertDomain === 'sales') {
      patterns.push({
        pattern: 'Consultative Reasoning',
        triggerConditions: ['client needs assessment', 'solution design required', 'value demonstration needed'],
        steps: [
        ...
        outputCharacteristics: ['client-centric focus', 'value articulation', 'consultative approach', 'trust-based relationships']
      });
    }
    ...
    if (request.expertiseLevel === 'expert' || request.expertiseLevel === 'specialist') {
      patterns.push({
        ...
        triggerConditions: ['high complexity situations', 'time pressure', 'incomplete information'],
        steps: [
        ...
        outputCharacteristics: ['expert intuition', 'rapid processing', 'confident uncertainty handling', 'adaptive expertise']
      });
    }
  ...
  private catalogDecisionHeuristics(request: CognitiveAnalysisRequest) {
    ...
    heuristics.push({
      ...
      applicableContexts: ['strategic decisions', 'process changes', 'resource allocation'],
    ...
    heuristics.push({
      ...
      applicableContexts: ['investment decisions', 'change initiatives', 'solution selection'],
      reliability: 'high' as const,
    ...
    heuristics.push({
      ...
      applicableContexts: ['similar situations', 'proven approaches', 'low-risk decisions'],
      reliability: 'medium' as const,
      biasRisks: ['availability bias', 'confirmation bias', 'representativeness bias']
    });
      ...
      'customer_service': [
        {
          ...
          applicableContexts: ['process design', 'service delivery', 'problem resolution'],
          reliability: 'high' as const,
          biasRisks: ['oversimplification', 'capability underestimation']
        },
        {
          heuristic: 'Emotional State Prioritization',
          applicableContexts: ['upset customers', 'service recovery', 'escalation handling'],
          reliability: 'high' as const,
      ...
      'sales': [
        {
          ...
          applicableContexts: ['objection handling', 'pricing discussions', 'competitive situations'],
          reliability: 'high' as const,
          biasRisks: ['value overestimation', 'cost minimization']
        },
        {
          heuristic: 'Decision Maker Focus',
          applicableContexts: ['complex sales', 'organizational selling', 'proposal development'],
          reliability: 'medium' as const,
    ...
    if (request.cognitiveComplexity >= 7) {
      heuristics.push({
        ...
        applicableContexts: ['complex systems', 'multiple stakeholders', 'long-term consequences'],
        reliability: 'medium' as const,
        biasRisks: ['analysis paralysis', 'complexity bias', 'system justification']
      });
    }
  ...
  protected async assessConfidence(result: unknown): Promise<number> {
    if (typeof result === 'object' && result !== null) {
      ...
      const reasoningPatternsCount = output.expertThinkingModel.cognitiveArchitecture.reasoningPatterns.length;
  ...
  protected async explainReasoning(
    ...
    if (typeof output === 'object' && output !== null) {
      ...
      const reasoningPatterns = result.expertThinkingModel.cognitiveArchitecture.reasoningPatterns.length;
      const heuristics = result.expertThinkingModel.cognitiveArchitecture.decisionHeuristics.length;
