...
export class LogAggregator extends EventEmitter {
  ...
  async searchLogs(query: string, limit: number = 100): Promise<LogEntry[]> {
    const filter: LogFilter = { textSearch: query };
   ...
   */
  async getLogsByTrace(traceId: string): Promise<LogEntry[]> {
    const filter: LogFilter = { traceId };
   ...
   */
  async cleanup(): Promise<{ deletedCount: number; sizeFreed: number }> {
  ...
  private queryFromIndex(filter: LogFilter): LogEntry[] {
  ...
  private async cleanupFileStorage(cutoff: Date): Promise<number> {
    if (!this.config.storagePath) return 0;
  ...
  private async setupSplunk(): Promise<void> {
