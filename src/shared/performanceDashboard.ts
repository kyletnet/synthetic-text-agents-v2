...
export class PerformanceDashboard {
  ...
  async getDashboardMetrics(timeWindow: number = 300000): Promise<DashboardMetrics> {
    try {
  ...
  private async getOverviewMetrics(timeWindow: number): Promise<DashboardMetrics['overview']> {
    ...
    return {
      ...
      errorRate: summary.transactions.total > 0
  ...
  private async getAgentMetrics(timeWindow: number): Promise<DashboardMetrics['agents']> {
    ...
    return Object.entries(agentMetrics).map(([agentName, data]) => ({
      ...
      averageExecutionTime: data.executions.reduce((sum, exec) => sum + exec.duration, 0) / data.executions.length || 0,
      averageQualityScore: data.qualityScores.reduce((sum, score) => sum + score, 0) / data.qualityScores.length || 0,
      errorRate: data.errors.length / Math.max(data.executions.length, 1),
      lastExecution: data.executions.length > 0
        ? new Date(Math.max(...data.executions.map(e => e.timestamp.getTime())))
        : null
    }));
  }
  ...
  private async getTrendMetrics(timeWindow: number): Promise<DashboardMetrics['trends']> {
    ...
    for (let i = buckets; i >= 0; i--) {
      ...
      // Response time trend
      const avgResponseTime = bucketMetrics.responseTimes.reduce((sum, rt) => sum + rt, 0) / bucketMetrics.responseTimes.length || 0;
      ...
      // Error rate trend
      const bucketErrorRate = requestCount > 0 ? bucketMetrics.errorCount / requestCount : 0;
      errorRate.push({ timestamp, value: bucketErrorRate });

      // Quality score trend
      const avgQualityScore = bucketMetrics.qualityScores.reduce((sum, qs) => sum + qs, 0) / bucketMetrics.qualityScores.length || 0;
  ...
  private generateAlerts(
    ...
    if (overview.averageResponseTime > this.alertThresholds.responseTimeCritical) {
      this.addAlert({
        level: 'critical',
    ...
    } else if (overview.averageResponseTime > this.alertThresholds.responseTimeWarning) {
      this.addAlert({
        level: 'warning',
    ...
    for (const agent of agents) {
      if (agent.averageQualityScore < this.alertThresholds.qualityScoreCritical) {
        this.addAlert({
          level: 'critical',
      ...
      } else if (agent.averageQualityScore < this.alertThresholds.qualityScoreWarning) {
        this.addAlert({
          level: 'warning',
  ...
  private addAlert(alertData: Omit<Alert, 'id' | 'acknowledged'>): void {
    ...
    if (!existingAlert) {
      ...
      this.logger.warn(`Alert generated: ${alert.title} - ${alert.description}`);
    }
  }
  ...
  getActiveAlerts(): Alert[] {
    ...
    return this.alerts
      .filter(alert => !alert.acknowledged && alert.timestamp.getTime() > cutoff)
      .sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
  }

  private getAgentPerformanceData(timeWindow: number): Record<string, {
  ...
  private getMetricsInTimeRange(start: number, end: number): {
...
export function initializePerformanceDashboard(
  ...
  globalDashboard = new PerformanceDashboard(performanceMonitor, alertThresholds);
