...
export class MetaController extends BaseAgent {
  ...
  protected async handle(
  ...
  ): Promise<MetaControllerDecision> {
    ...
    const agentSelection = await this.selectOptimalAgents(
      complexityAnalysis,
      taskRequest,
    );
    ...
    const executionStrategy = await this.planExecutionStrategy(
      agentSelection,
      complexityAnalysis,
    );
    const expectedOutcome = await this.projectOutcome(
      complexityAnalysis,
      agentSelection,
    );
  ...
  private generateComplexityReasoning(
    ...
    const reasoning =
      `Complexity score: ${score}/10 (${complexity}). ` +
  ...
  private async projectOutcome(
    ...
    const complexity = analysis.score;
    const agentCount =
      selection.coreAgents.length + selection.expertCouncil.length;
