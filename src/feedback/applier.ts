...
export function conservativeMerge<T extends Record<string, any>>(
  orig: T,
...
export function applyPatchCard<T extends Record<string, any>>(
  obj: T,
  card: PatchCard,
): { result: T; changed: string[] } {
  if (!card.applies) return { result: obj, changed: [] };
  const before = JSON.stringify(obj);
  const merged = conservativeMerge(obj, card.deltas);
  const changed =
    before === JSON.stringify(merged) ? [] : Object.keys(card.deltas || {});
