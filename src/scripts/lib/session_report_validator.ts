...
export class SessionReportValidator {
  ...
  private async parseReportData(content: string): Promise<any> {
  ...
  private extractDataFromMarkdown(content: string): any {
  ...
  private extractFromMarkdownPatterns(content: string): any {
    const data: any = {};
  ...
  private extractValue(content: string, regex: RegExp): string | null {
  ...
  private extractArray(content: string, regex: RegExp): string[] {
    const match = content.match(regex);
    if (!match) return [];
