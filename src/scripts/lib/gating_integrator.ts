...
export class GatingIntegrator {
  ...
  private extractJSONFromMarkdown(content: string): SessionReportData {
  ...
  private extractNumericValue(content: string, regex: RegExp): number | null {
    const match = content.match(regex);
    return match ? parseFloat(match[1]) : null;
  }

  private extractStringValue(content: string, regex: RegExp): string | null {
    const match = content.match(regex);
