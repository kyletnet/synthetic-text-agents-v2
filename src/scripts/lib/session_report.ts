...
export class SessionReportManager {
   ...
   */
  static enforceCasesTotal(fields: Partial<SessionReportFields>): Partial<SessionReportFields> {
    if (fields.CASES_TOTAL === 0 && fields.MODE === 'smoke' && fields.RESULT === 'PASS') {
   ...
   */
  static validateFields(fields: Partial<SessionReportFields>): Partial<SessionReportFields> {
  ...
  private getGitStatus(): string {
    try {
      if (typeof require !== 'undefined') {
        ...
        return output.trim().split('\n').filter((line: string) => line.trim()).length.toString();
      }
      return 'unknown';
    } catch (e) {
      return 'unknown';
  ...
  private getDLQStatus(dlqCount: number): string {
    ...
    try {
      if (fs.existsSync(dlqDir)) {
        const entries = fs.readdirSync(dlqDir)
          .filter(name => name !== 'index.jsonl' && fs.statSync(path.join(dlqDir, name)).isDirectory())
          .sort((a, b) => {
            try {
...
if (typeof require !== 'undefined' && require.main === module) {
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    if (key && value !== undefined) {
      ...
      } else if (['DURATION_MS', 'PANEL_SIZE', 'TOKENS_EST', 'CASES_TOTAL', 'CASES_PASSED', 'WARNINGS', 'RETRY_COUNT'].includes(key)) {
