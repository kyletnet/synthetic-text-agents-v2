...
export class AgentLogger {
  ...
  createChildTrace(parent: TraceContext): TraceContext {
    ...
    if (typeof parent.session_id === "string") ctx.session_id = parent.session_id;
    return ctx;
  }
  ...
  logOperationStart(
    ...
    if (typeof context.session_id === "string") logEntry.session_id = context.session_id;
    if (typeof context.correlation_id === "string") logEntry.parent_trace_id = context.correlation_id;
    if (inputData) {
  ...
  logOperationComplete(
    ...
    if (typeof context.session_id === "string") logEntry.session_id = context.session_id;
    if (typeof context.correlation_id === "string") logEntry.correlation_id = context.correlation_id;
    if (typeof context.parent_trace_id === "string") logEntry.parent_trace_id = context.parent_trace_id;
    if (typeof result.tokens_in === "number") logEntry.tokens_in = result.tokens_in;
    if (typeof result.tokens_out === "number") logEntry.tokens_out = result.tokens_out;
    if (result.output_data) {
      logEntry.output_data = this.sanitizeData(result.output_data);
      logEntry.output_hash = this.calculateHash(result.output_data);
    }
    if (typeof result.quality_score === "number") logEntry.quality_score = result.quality_score;
    if (typeof result.confidence_score === "number") logEntry.confidence_score = result.confidence_score;
    if (result.validation_result) logEntry.validation_result = result.validation_result;
  ...
  logWorkflowStep(
    ...
    if (typeof context.session_id === "string") logEntry.session_id = context.session_id;
    if (typeof context.correlation_id === "string") logEntry.correlation_id = context.correlation_id;
    if (stepResult.output_data) logEntry.output_data = this.sanitizeData(stepResult.output_data);
