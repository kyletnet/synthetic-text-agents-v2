...
export async function calculateAllBaselineMetrics(
  ...
  const records: BaselineMetricsRecord[] = qaItems.map((item, index) => {
    ...
    const hallucinationFlag = hallucinationMetrics.flags.find((flag: any) => flag.index === index);
    ...
    return {
      ...
      duplication: {
        ...
        max_similarity: duplicateFlag ?
          Math.max(...duplicationMetrics.top_duplicate_pairs
            ...
            .map((pair: any) => pair.jaccard_similarity)) : 0,
        similar_to_indices: duplicationMetrics.top_duplicate_pairs
          .filter((pair: any) => pair.index1 === index || pair.index2 === index)
          .map((pair: any) => pair.index1 === index ? pair.index2 : pair.index1)
      },
      ...
      pii_license: {
        pii_violations: piiLicenseViolations.filter((v: any) => v.type === 'pii').length,
        license_violations: piiLicenseViolations.filter((v: any) => v.type === 'license').length,
        clean: piiLicenseViolations.length === 0,
        violation_types: [...new Set(piiLicenseViolations.map((v: any) => v.pattern as string))] as string[]
      },
  ...
  const summary: BaselineMetricsSummary = {
    ...
    duplication: {
      ...
      ...(typeof duplicationMetrics.semantic_duplication_rate === "number" ? { semantic_duplication_rate: duplicationMetrics.semantic_duplication_rate } : {}),
      alert_triggered: duplicationMetrics.alert_triggered
    },

    qtype_distribution: {
      distributions: Object.fromEntries(
        Object.entries(qtypeMetrics.distributions).map(([key, value]: [string, any]) => [
