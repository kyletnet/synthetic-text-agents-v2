...
export class PreflightPack {
  ...
  private async runStage(stageName: string, stageFunction: () => Promise<any>): Promise<StageResult> {
  ...
  private async validateLint(): Promise<{ outputs: string[] }> {
    ...
    const hasFlat = await fs.access(resolve(process.cwd(), 'eslint.config.cjs')).then(() => true).catch(() => false) ||
                    await fs.access(resolve(process.cwd(), 'eslint.config.js')).then(() => true).catch(() => false);
  ...
  private async evaluateGating(): Promise<{ canProceed: boolean; reason: string; outputs: string[] }> {
    const outputs: string[] = [];
  ...
  private async exportObservability(): Promise<{ outputs: string[] }> {
    ...
    try {
      ...
      // Export trace data
      const opts: any = { format: 'json', includeMetrics: true, includeTimeline: true };
  ...
  private async generateHandoffBundle(stages: StageResult[], success: boolean): Promise<HandoffBundle> {
    ...
    bundle.bundle_hash = createHash('sha256')
      .update(JSON.stringify({
        ...
        stages: stages.map(s => ({ stage: s.stage, success: s.success, duration_ms: s.duration_ms }))
      }))
      .digest('hex')
      .slice(0, 16);
  ...
  private async collectConfigSnapshot(): Promise<HandoffBundle['config_snapshot']> {
