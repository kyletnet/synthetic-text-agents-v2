...
export class PreflightRunner {
  ...
  constructor(config: Partial<PreflightConfig> = {}) {
    this.config = {
      profile: 'stage',
      budget_smoke_usd: 2.00,
      budget_full_usd: 50.00,
      run_tags: 'preflight',
  ...
  private setupDirectories(): void {
    for (const dir of [this.reportDir, this.runLogsDir, this.dlqDir, this.obsDir]) {
      if (!existsSync(dir)) {
  ...
  async execute(): Promise<PreflightResult> {
    ...
    console.log(`💰 Budget: Smoke $${this.config.budget_smoke_usd}, Full $${this.config.budget_full_usd}`);
    ...
    try {
      ...
      if (!smokeResult.passed) {
        return this.createFailedResult('Smoke run failed validation', {
      ...
      return this.createFailedResult(
        ...
        {
          smoke_duration_ms: smokeStartTime > 0 ? Date.now() - smokeStartTime : 0,
  ...
  private async initializeAndVerifySystems(): Promise<void> {
    ...
    if (latestManifest) {
      const validation = await this.manifestManager.validateManifest(latestManifest.manifest_id);
      if (!validation.valid) {
    ...
    console.log(`   P0 (fixed): PII max ${thresholds.p0.pii_hits_max}, License max ${thresholds.p0.license_violations_max}`);
    console.log(`   P1 (${this.config.profile}): Cost warn $${thresholds.p1.cost_per_item_warn}, fail $${thresholds.p1.cost_per_item_fail}`);
    console.log(`   P2 (${this.config.profile}): Quality warn ${thresholds.p2.quality_score_warn}, fail ${thresholds.p2.quality_score_fail}`);
  ...
  private async runDryRunSmoke(): Promise<void> {
    ...
    try {
      ...
      console.log('⚠️  Dry-run smoke failed, proceeding to paid smoke anyway\n');
    }
  }
   ...
   * Step 3: Paid smoke run (critical validation)
   */
  private async runPaidSmoke(): Promise<{ passed: boolean; cost_usd: number; session_data: any }> {
    ...
    const budgetLimits = {
      ...
      per_agent_limits: {
        evidence: { max_cost_usd: this.config.budget_smoke_usd * 0.2, max_time_ms: 60000 },
        answer: { max_cost_usd: this.config.budget_smoke_usd * 0.5, max_time_ms: 120000 },
        audit: { max_cost_usd: this.config.budget_smoke_usd * 0.3, max_time_ms: 60000 }
      }
    };
    ...
    try {
      ...
      // Record actual usage
      this.budgetGuardian.recordUsage(smokeResult.cost_usd, smokeResult.duration_ms, 'smoke_orchestrator');
      ...
      const passed = smokeResult.success &&
                    smokeResult.cases_processed >= 5 &&
                    smokeResult.cost_usd > 0 &&
                    smokeResult.cost_usd <= this.config.budget_smoke_usd;

      if (passed) {
        console.log(`✅ Paid smoke passed: ${smokeResult.cases_processed} cases, $${smokeResult.cost_usd.toFixed(3)}`);
      } else {
        console.log(`❌ Paid smoke failed: Success=${smokeResult.success}, Cases=${smokeResult.cases_processed}, Cost=$${smokeResult.cost_usd.toFixed(3)}`);
      }

      return { passed, cost_usd: smokeResult.cost_usd, session_data: sessionData };
   ...
   */
  private async runFullExecution(): Promise<{ cost_usd: number; session_data: any }> {
    console.log('🚀 Running full execution...');

    // Initialize budget guardian for full run
    const budgetLimits = {
      max_cost_per_run: this.config.budget_full_usd,
      max_cost_per_item: Math.min(0.20, this.config.budget_full_usd / 50), // Conservative estimate
      max_time_per_run_ms: 3600000, // 1 hour
      max_time_per_item_ms: 60000,   // 1 minute
      per_agent_limits: {
        evidence: { max_cost_usd: this.config.budget_full_usd * 0.2, max_time_ms: 600000 },
        answer: { max_cost_usd: this.config.budget_full_usd * 0.5, max_time_ms: 1800000 },
        audit: { max_cost_usd: this.config.budget_full_usd * 0.3, max_time_ms: 600000 }
      }
    };
    ...
    try {
      ...
      const checkpointPlan = this.checkpointManager.analyzeRecoveryOptions(`full_${this.timestamp}`);
      let startIndex = 0;

      if (checkpointPlan.can_recover) {
        console.log(`🔄 Found checkpoint, can recover ${checkpointPlan.estimated_time_saved_ms}ms`);
        const recovery = this.checkpointManager.executeRecovery(`full_${this.timestamp}`, checkpointPlan);
      ...
      // Record usage
      this.budgetGuardian.recordUsage(fullResult.cost_usd, fullResult.duration_ms, 'full_orchestrator');
      ...
      console.log(`✅ Full run completed: ${fullResult.items_processed} items, $${fullResult.cost_usd.toFixed(3)}`);
  ...
  private async postProcessing(): Promise<void> {
    ...
    if (dlqStats.pending_retries > 0) {
      ...
      for (const item of pendingItems.slice(0, 10)) { // Limit to 10 items
        try {
      ...
      console.log(`✅ Reprocessed ${reprocessedCount}/${pendingItems.length} DLQ items`);
    }
  ...
  private async generateObservabilityArtifacts(): Promise<void> {
    ...
    const observabilityData = {
      ...
      dlq_stats: dlqStats,
      checkpoints: this.checkpointManager.listCheckpoints(`full_${this.timestamp}`).length,
      final_status: 'SUCCESS'
    };
  ...
  private async simulateFullRun(startIndex: number): Promise<{
    ...
    await new Promise(resolve => setTimeout(resolve, Math.min(estimatedDuration, 5000))); // Cap simulation time
...
`);
  ...
  try {
    ...
    if (result.artifacts.observability_path) {
      console.log(`Observability Report: ${result.artifacts.observability_path}`);
    }
