...
describe('CircuitBreaker', () => {
  ...
  describe('Circuit Opening (CLOSED -> OPEN)', () => {
    ...
    it('should reject requests immediately when circuit is open', async () => {
      ...
      await expect(circuitBreaker.execute(newOperation)).rejects.toThrow(CircuitBreakerError);
    ...
    it('should include retry time in circuit breaker error', async () => {
      ...
      try {
        ...
        expect((error as CircuitBreakerError).message).toContain('Next attempt at');
      }
    });
  });
  ...
  describe('Circuit Recovery (OPEN -> HALF_OPEN -> CLOSED)', () => {
    ...
    it('should reopen circuit if failure occurs in HALF_OPEN', async () => {
      ...
      const failOperation = vi.fn().mockRejectedValue(new Error('failed again'));
      await expect(circuitBreaker.execute(failOperation)).rejects.toThrow('failed again');
