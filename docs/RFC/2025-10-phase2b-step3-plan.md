...
export class QAFeedbackManager {
  ...
  async runFeedbackLoop(): Promise<FeedbackResult> {
    ...
    const baseline = await this.metricsService.getBaselineMetrics("integration-base");
...
export class GovernanceKernel {
   ...
   */
  async evaluateExternalPolicy(policyDoc: ExternalPolicy): Promise<PolicyEvaluation> {
    ...
    return { approved: true, flag, recommendations: validation.recommendations };
  }
